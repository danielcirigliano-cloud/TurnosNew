...
<head>
    ...
    <style>
        ...
        .cap-parent-row:hover { background: rgba(15, 23, 42, 0.9); }
        .cap-child-row { background: rgba(15, 23, 42, 0.35); }
        .cap-child-row.hidden-row { display: none; }
        .expander-btn { width: 18px; height: 18px; display: inline-flex; align-items: center; justify-content: center; border-radius: 4px; border: 1px solid #334155; background: #0f172a; color: #38bdf8; font-weight: 800; line-height: 1; margin-right: 8px; cursor: pointer; }
...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s8" class="tab-content mt-8 w-full">
        ...
        <p class="text-sm text-slate-400 mb-6">Vista productiva alineada a <span class="text-sky-400">capacidad_turnos_por_planta.csv</span>. La fila principal resume la planta y, con el símbolo <strong>+</strong>, despliega el detalle exacto por <strong>Tipo_Carga</strong> como en el CSV.</p>
...
function toggleDetalleCapacidad(plantaKey) {
    const rows = document.querySelectorAll(`tr[data-parent-planta="${CSS.escape(plantaKey)}"]`);
    const icon = document.querySelector(`[data-expander-planta="${CSS.escape(plantaKey)}"]`);
    if (!rows.length) return;
    const expanded = Array.from(rows).every(r => !r.classList.contains('hidden-row'));
        ...
        configPlantas.forEach(p => {
            ...
            TIPOS_ZONA_CALCULO.forEach(tipo => {
                ...
                html += `<div class="flex justify-between gap-1 py-0.5 border-b border-slate-800/50">
                    ...
                    <span class="text-slate-400 shrink-0">${fmtNum(teo, 0)}|${ocup}|<span class="text-emerald-400 font-bold">${fmtNum(disp, 0)}</span></span>
                </div>`;
            });
        });
    ...
    function renderizarABMHorarios() {
        ...
        if (capacidadPorPlantaTipoCache && capacidadPorPlantaTipoCache.length) {
            const grupos = Object.values(capacidadPorPlantaIndex)
                .sort((a, b) => String(a.resumen?.Planta || '').localeCompare(String(b.resumen?.Planta || '')));

            const htmlRows = grupos.map(g => {
                const r = g.resumen || {};
                const plantaKey = String(r.Planta || '').replace(/"/g, '&quot;');
                const parent = `<tr class="text-sm cap-parent-row border-b border-slate-700/50 hover:bg-slate-800/30">
                    <td class="py-3 px-4 font-bold text-sky-400 whitespace-nowrap">
                        <button type="button" class="expander-btn" data-expander-planta="${plantaKey}" onclick="toggleDetalleCapacidad('${plantaKey.replace(/'/g, "\\'")}')">+</button>${escapeHtml(r.Planta || '-')}
                    </td>
                    <td class="px-4 text-slate-400 font-semibold">-</td>
                    <td class="px-4 text-center text-slate-400">-</td>
                    <td class="px-4 text-center text-slate-400">-</td>
                    <td class="px-4 text-center text-emerald-400 font-bold mono-num">${fmtNum(r.Turnos_Calc || r.TotalDia, 0)}</td>
                    <td class="px-4 text-center text-amber-400 font-bold mono-num">${fmtNum(r.Turnos_Sabado, 0)}</td>
                    <td class="px-4 text-center text-slate-300 mono-num">${fmtNum(r.Min_LV, 0)}</td>
                    <td class="px-4 text-center text-slate-300 mono-num">${fmtNum(r.Min_SAB, 0)}</td>
                    <td class="px-4 text-center text-slate-300 mono-num">${fmtNum(r.Minutos_Turno, 0)}</td>
                    <td class="px-4 text-center text-sky-300 font-bold mono-num">${fmtNum(r.TotalDia || r.Turnos_Calc, 0)}</td>
                    <td class="px-4 text-center font-bold ${String(r.Estatus || 'OK').toUpperCase() === 'OK' ? 'text-emerald-400' : 'text-amber-400'}">${escapeHtml(r.Estatus || 'OK')}</td>
                </tr>`;

                const children = (g.filas || [])
                    .sort((a, b) => String(a.Tipo_Carga || '').localeCompare(String(b.Tipo_Carga || '')))
                    .map(row => {
                        const colorHab = Number(row.Habilitado || 0) === 1 ? 'text-emerald-400' : 'text-rose-400';
                        const colorEst = String(row.Estatus || 'OK').toUpperCase() === 'OK' ? 'text-emerald-400' : 'text-amber-400';
                        return `<tr class="text-sm cap-child-row hidden-row border-b border-slate-800/60" data-parent-planta="${plantaKey}">
                            <td class="py-2.5 px-4 text-slate-500"><span class="cap-child-indent">└</span></td>
                            <td class="px-4 text-white font-semibold">${escapeHtml(row.Tipo_Carga || '-')}</td>
                            <td class="px-4 text-center font-bold ${colorHab} mono-num">${fmtNum(row.Habilitado, 0)}</td>
                            <td class="px-4 text-center text-slate-200 mono-num">${fmtNum(row.Turnos_Cargados, 2)}</td>
                            <td class="px-4 text-center text-emerald-400 font-bold mono-num">${fmtNum(row.Turnos_Calc, 2)}</td>
                            <td class="px-4 text-center text-amber-400 font-bold mono-num">${fmtNum(row.Turnos_Sabado, 2)}</td>
                            <td class="px-4 text-center text-slate-300 mono-num">${fmtNum(row.Min_LV, 0)}</td>
                            <td class="px-4 text-center text-slate-300 mono-num">${fmtNum(row.Min_SAB, 0)}</td>
                            <td class="px-4 text-center text-slate-300 mono-num">${fmtNum(row.Minutos_Turno, 0)}</td>
                            <td class="px-4 text-center text-sky-300 font-bold mono-num">${fmtNum(row.TotalDia, 2)}</td>
                            <td class="px-4 text-center font-bold ${colorEst}">${escapeHtml(row.Estatus || 'OK')}</td>
                        </tr>`;
                    }).join('');
            ...
            tb.innerHTML = htmlRows || '<tr><td colspan="11" class="p-6 text-center text-slate-500 text-sm">Sin datos en capacidad_turnos_por_planta.csv.</td></tr>';
            return;
        }
