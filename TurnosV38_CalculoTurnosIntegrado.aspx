...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s8" class="tab-content mt-8 w-full">
        ...
        <p class="text-sm text-slate-400 mb-6">Ajuste las ventanas operativas y duración de turnos. El total diario se calcula como turnos base × cantidad de tipos de descarga habilitados (Insumos / Packaging / No Productivo).</p>
...
<script>
    ...
    function contarZonasActivasPorTipo(plantaId, tipo) {
        ...
        // la cantidad de turnos base se multiplica por la cantidad de TIPOS DE CARGA habilitados,
        // no por la cantidad de zonas ni por la capacidad individual de cada zona.
        ...
        return activas.length > 0 ? 1 : 0;
    }

    function obtenerTiposHabilitadosPlanta(plantaId) {
        const tipos = TIPOS_ZONA_CALCULO.filter(tipo => contarZonasActivasPorTipo(plantaId, tipo) === 1);
        // Compatibilidad con la version actual: si aun no hay zonas configuradas para una planta,
        // se mantiene la disponibilidad para los 3 tipos base.
        return tipos.length > 0 ? tipos : [...TIPOS_ZONA_CALCULO];
    }
        const slots = calcularTurnosTeoricos(plantaConfig, esSabado);
        const res = {};
        const tiposHabilitados = obtenerTiposHabilitadosPlanta(plantaConfig.id);
        TIPOS_ZONA_CALCULO.forEach(tipo => {
            res[tipo] = tiposHabilitados.includes(tipo) ? slots : 0;
        });
        const total = slots * tiposHabilitados.length;
        return { slots, porTipo: res, total, tiposHabilitados };
    }

    function calcularCantidadTurnosBase(plantaConfig, esSabado) {
        return calcularTurnosTeoricos(plantaConfig, esSabado);
    }

    function calcularTotalDiaPorTipos(plantaConfig, esSabado) {
        const base = calcularCantidadTurnosBase(plantaConfig, esSabado);
        const tipos = obtenerTiposHabilitadosPlanta(plantaConfig.id).length;
        return base * tipos;
    }
