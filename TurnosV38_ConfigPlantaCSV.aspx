...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s8" class="tab-content mt-8 w-full">
        ...
        <p class="text-sm text-slate-400 mb-6">Ajuste las ventanas operativas y duración de turnos. La capacidad total se calcula como turnos base × cantidad de tipos de descarga habilitados (Insumos / Packaging / No Productivo).</p>
    ...
    <section id="sFlota" class="tab-content mt-8 w-full">
        ...
        <p class="text-xs text-slate-400 mb-4">Conectado a: <span class="text-sky-400">/sites/GestiondeTurnos/Lists/Vehiculos</span></p>
        <div class="glass p-6 rounded-xl mb-6 border border-slate-700 w-full text-sm">
...
<script>
    ...
    async function actualizarCSVSharePoint(tipo) {
        ...
        try {
            const digest = await getDigest();
            let fileName = tipo === 'prov' ? 'proveedores.csv' : (tipo === 'usu' ? 'Usuarios.csv' : 'ConfiguracionPlanta.csv');
...
`; });
                ...
                csvContent += csvConfigPlantasHeader + "
";
                configPlantas.forEach(p => {
                    const cantidadTurnos = calcularCantidadTurnosBase(p, false) + calcularCantidadTurnosBase(p, true);
                    const totalDia = calcularTotalDiaPorTipos(p);
                    csvContent += `${p.id};${p.hInLV};${p.hOutLV};${p.sabado ? 1 : 0};${p.hInS};${p.hOutS};${p.duracion};${p.insumos ? 1 : 0};${p.packaging ? 1 : 0};${p.noProductivo ? 1 : 0};${cantidadTurnos};${totalDia};${p.estatus || 'OK'}
`;
                });
            } else {
...
`; });
    ...
    const TIPOS_ZONA_CALCULO = ['Insumos', 'Packaging', 'No Productivo'];
    ...
    function calcularCantidadTurnosBase(plantaConfig, esSabado = false) {
        return calcularTurnosTeoricos(plantaConfig, esSabado);
    }

    function obtenerTiposHabilitadosPlanta(plantaConfig) {
        const tipos = [];
        if (Number(plantaConfig.insumos) === 1) tipos.push('Insumos');
        if (Number(plantaConfig.packaging) === 1) tipos.push('Packaging');
        if (Number(plantaConfig.noProductivo) === 1) tipos.push('No Productivo');
        return tipos;
    }

    function calcularTotalDiaPorTipos(plantaConfig) {
        const turnosBase = calcularCantidadTurnosBase(plantaConfig, false) + calcularCantidadTurnosBase(plantaConfig, true);
        const tipos = obtenerTiposHabilitadosPlanta(plantaConfig).length;
        return turnosBase * tipos;
    }

    function contarZonasActivasPorTipo(plantaId, tipo) {
        const plantaCfg = configPlantas.find(p => String(p.id).toLowerCase() === String(plantaId).toLowerCase());
        if (!plantaCfg) return 0;
        return obtenerTiposHabilitadosPlanta(plantaCfg).includes(tipo) ? 1 : 0;
    }

    function calcularCapacidadPlantaPorTipo(plantaConfig, esSabado) {
        const slots = calcularTurnosTeoricos(plantaConfig, esSabado);
        const tipos = obtenerTiposHabilitadosPlanta(plantaConfig);
        const res = {};
        TIPOS_ZONA_CALCULO.forEach(tipo => {
            res[tipo] = tipos.includes(tipo) ? slots : 0;
        });
        const total = slots * tipos.length;
        return { slots, porTipo: res, total, tiposHabilitados: tipos };
    }

    function parsearCSVConfigPlantas(text) {
        const lines = text.split(/?
|/).filter(l => l.trim());
        if (lines.length < 2) return;
        const header = lines[0].replace(/^﻿/, '').split(';').map(h => h.trim().toLowerCase());
        const idx = {
            planta: header.findIndex(h => h === 'planta'),
            lvDesde: header.findIndex(h => h === 'lv_desde'),
            lvHasta: header.findIndex(h => h === 'lv_hasta'),
            sabHab: header.findIndex(h => h === 'sab_habilitado'),
            sabDesde: header.findIndex(h => h === 'sab_desde'),
            sabHasta: header.findIndex(h => h === 'sab_hasta'),
            minutos: header.findIndex(h => h === 'minutos_turno'),
            insumos: header.findIndex(h => h === 'insumos'),
            packaging: header.findIndex(h => h === 'packaging'),
            np: header.findIndex(h => h === 'no_productivo'),
            estatus: header.findIndex(h => h === 'estatus')
        };
        const result = [];
        for (let i = 1; i < lines.length; i++) {
            const cols = lines[i].split(';');
            const planta = (cols[idx.planta] || '').trim();
            if (!planta) continue;
            const obj = {
                id: planta,
                hInLV: parseInt(cols[idx.lvDesde]) || 0,
                hOutLV: parseInt(cols[idx.lvHasta]) || 0,
                sabado: String(cols[idx.sabHab] || '0').trim() === '1',
                hInS: parseInt(cols[idx.sabDesde]) || 0,
                hOutS: parseInt(cols[idx.sabHasta]) || 0,
                duracion: parseInt(cols[idx.minutos]) || 40,
                insumos: parseInt(cols[idx.insumos]) || 0,
                packaging: parseInt(cols[idx.packaging]) || 0,
                noProductivo: parseInt(cols[idx.np]) || 0,
                estatus: (cols[idx.estatus] || 'OK').trim() || 'OK'
            };
            obj.cantidadTurnos = calcularCantidadTurnosBase(obj, false) + calcularCantidadTurnosBase(obj, true);
            obj.totalDia = calcularTotalDiaPorTipos(obj);
            result.push(obj);
        }
        if (result.length > 0) {
            configPlantas = result;
            csvConfigPlantasHeader = lines[0].replace(/^﻿/, '');
            cargarSelectPlantasFisicas();
            renderizarABMHorarios();
            renderizarResumenCapacidad();
            logLog(`[ETL CSV] ConfiguracionPlanta.csv cargado: ${configPlantas.length} plantas.`);
        }
    }
        ...
        configPlantas.forEach(p => {
            ...
            TIPOS_ZONA_CALCULO.forEach(tipo => {
                ...
                const ocup = listaTurnosCache.filter(t =>
                    ...
                    String(t.Planta || '').toLowerCase() === tipo.toLowerCase() &&
                ...
                html += `<div class="flex justify-between gap-1 py-0.5 border-b border-slate-800/50">
                    <span class="text-sky-300 truncate">${escapeHtml(p.id)} / ${tipo}</span>
    ...
    let configPlantas = [
        { id: "Parque Canuelas", hInLV: 7, hOutLV: 15, hInS: 0, hOutS: 0, duracion: 40, sabado: false, insumos: 1, packaging: 1, noProductivo: 1, cantidadTurnos: 12, totalDia: 36, estatus: 'OK' },
        { id: "Predio Canuelas", hInLV: 10, hOutLV: 17, hInS: 7, hOutS: 11, duracion: 40, sabado: true, insumos: 1, packaging: 1, noProductivo: 1, cantidadTurnos: 16, totalDia: 48, estatus: 'OK' },
        { id: "Spegazzini", hInLV: 8, hOutLV: 16, hInS: 8, hOutS: 12, duracion: 45, sabado: true, insumos: 1, packaging: 1, noProductivo: 1, cantidadTurnos: 15, totalDia: 45, estatus: 'OK' },
        { id: "Almar", hInLV: 7, hOutLV: 14, hInS: 7, hOutS: 11, duracion: 30, sabado: true, insumos: 0, packaging: 1, noProductivo: 1, cantidadTurnos: 22, totalDia: 44, estatus: 'OK' },
        { id: "Metro Mataderos", hInLV: 7, hOutLV: 15, hInS: 0, hOutS: 0, duracion: 40, sabado: false, insumos: 1, packaging: 0, noProductivo: 1, cantidadTurnos: 12, totalDia: 24, estatus: 'OK' }
    ];
    let csvConfigPlantasHeader = 'Planta;LV_Desde;LV_Hasta;Sab_Habilitado;Sab_Desde;Sab_Hasta;Minutos_Turno;INSUMOS;PACKAGING;NO_PRODUCTIVO;Cantidad_Turnos;TotalDia;Estatus';
    ...
    // ---- ABM FLOTA -- Lista SharePoint: Vehiculos ----
    const LISTA_FLOTA   = 'Vehculos';
    ...
    async function cargarFlotaSP() {
        try {
            const res = await fetchSPLista(LISTA_FLOTA, 'items?$select=Id,Patente,TipoVehiculo,Empresa,CUITEmpresa,Chofer,Estado&$orderby=Patente&$top=500');
            ...
            renderizarBaseFlota();
            actualizarDatalistFlota();
            logLog(`[ETL] Flota: ${listaFlotaCache.length} vehiculos cargados desde SP.`);
        } catch(e) {
            logLog(`[WARN] Flota SP no disponible: ${e.message}. Usando cache local.`);
            renderizarBaseFlota();
        }
    }
    ...
    function renderizarBaseFlota() {
        ...
        if(listaFlotaCache.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="p-6 text-center text-slate-500 text-sm">Sin vehiculos. Complete el formulario para agregar el primero.</td></tr>';
            return;
        }
    ...
    async function guardarVehiculo() {
        ...
        const payload = {
            Patente:      pat,
            TipoVehiculo: document.getElementById('fl_tipo').value,
            Empresa:      document.getElementById('fl_empresa').value.trim(),
            CUITEmpresa:  document.getElementById('fl_cuit').value.replace(/[^0-9\-]/g,''),
            Chofer:       chofer,
            Estado:       document.getElementById('fl_estado').value,
            '__metadata': { 'type': 'SP.Data.VehiculosListItem' }
        };
        try {
            const digest = await getDigest();
            const isEdit = spId && isNaN(parseInt(spId)) === false && listaFlotaCache.find(v => String(v.spId) === String(spId));
            ...
            const res = await fetchSPLista(LISTA_FLOTA, query, { method, headers, body: JSON.stringify(payload) });
            if(!res.ok && res.status !== 204) throw new Error('HTTP ' + res.status);
            showToast('Vehiculo guardado en SharePoint.', 'ok');
            limpiarFormularioFlota();
            await cargarFlotaSP();
            logLog(`[ETL] Flota: vehiculo ${pat} guardado en SP.`);
        } catch(e) {
            showToast('Error al guardar vehiculo: ' + e.message, 'err');
    ...
    async function initApp() {
        try {
            ...
            Promise.all([
                ...
                importarCSVSharePoint('cfg', true)
            ]).then(async () => {
                logLog("[ETL] Maestros sincronizados. Cargando Flota, Zonas y ConfiguracionPlanta.csv...");
    ...
    async function importarCSVSharePoint(tipo, isSilent = false) {
        ...
        try {
            ...
            else parsearCSVConfigPlantas(text);
    ...
    function renderizarABMHorarios() {
        ...
        configPlantas.forEach((p, idx) => {
            ...
            const capLV = calcularCapacidadPlantaPorTipo(p, false);
            const capSab = calcularCapacidadPlantaPorTipo(p, true);
            const totalDia = calcularTotalDiaPorTipos(p);
            const desglose = `${capLV.porTipo['Insumos']||0}/${capLV.porTipo['Packaging']||0}/${capLV.porTipo['No Productivo']||0}`;
            h += `<tr class="text-sm"><td class="py-3 px-4 font-bold text-sky-400">${p.id}</td><td class="px-4"><input type="number" id="cfg_inlv_${idx}" value="${p.hInLV}" oninput="previewHorariosConfig()" class="w-14 bg-slate-900 border border-slate-700 text-center rounded p-1 text-white"> a <input type="number" id="cfg_outlv_${idx}" value="${p.hOutLV}" oninput="previewHorariosConfig()" class="w-14 bg-slate-900 border border-slate-700 text-center rounded p-1 text-white"></td><td class="px-4"><label class="mr-3"><input type="checkbox" id="cfg_sab_${idx}" ${chkSab} onchange="toggleSabado(${idx}); previewHorariosConfig();"> Hab.</label><input type="number" id="cfg_ins_${idx}" value="${p.hInS}" oninput="previewHorariosConfig()" class="w-14 bg-slate-900 border border-slate-700 text-center rounded p-1 text-white ${!p.sabado ? 'opacity-30' : ''}" ${!p.sabado ? 'disabled' : ''}> a <input type="number" id="cfg_outs_${idx}" value="${p.hOutS}" oninput="previewHorariosConfig()" class="w-14 bg-slate-900 border border-slate-700 text-center rounded p-1 text-white ${!p.sabado ? 'opacity-30' : ''}" ${!p.sabado ? 'disabled' : ''}></td><td class="px-4"><input type="number" id="cfg_dur_${idx}" value="${p.duracion}" oninput="previewHorariosConfig()" class="w-16 bg-slate-900 border border-slate-700 text-center rounded p-1 text-white"></td><td class="px-4 text-center text-emerald-400 font-bold" id="cfg_prev_lv_${idx}">${capLV.slots}</td><td class="px-4 text-center text-amber-400 font-bold" id="cfg_prev_sab_${idx}">${capSab.slots}</td><td class="px-4 text-center text-slate-300 text-xs" id="cfg_prev_tip_${idx}">${desglose}</td><td class="px-4 text-center text-sky-300 font-bold" id="cfg_prev_tot_${idx}">${totalDia}</td><td class="px-4 text-center text-slate-500" id="cfg_prev_est_${idx}">${escapeHtml(p.estatus || 'OK')}</td></tr>`;
        });
        tb.innerHTML = h;
    }
        ...
        configPlantas.forEach((p, idx) => {
            ...
            if (elTot) elTot.textContent = calcularTotalDiaPorTipos(tmp);
        });
    }
    ...
    function guardarConfiguracionHorarios() {
        configPlantas.forEach((p, idx) => {
            ...
            p.cantidadTurnos = calcularCantidadTurnosBase(p, false) + calcularCantidadTurnosBase(p, true);
            p.totalDia = calcularTotalDiaPorTipos(p);
            p.estatus = p.estatus || 'OK';
        });
