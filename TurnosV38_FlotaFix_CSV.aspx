...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="sFlota" class="tab-content mt-8 w-full">
        ...
        <div class="flex gap-2 mb-4">
            <button onclick="importarCSVFlota()" class="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 text-xs font-bold rounded shadow">Subir CSV Flota</button>
            <input type="file" id="file_flota_csv" accept=".csv" style="display:none" onchange="procesarCSVFlota(event)">
        </div>
...
<script>
    ...
    function calcularCapacidadPlantaPorTipo(plantaConfig, esSabado) {
        ...
        TIPOS_ZONA_CALCULO.forEach(tipo => {
            const mult = contarZonasActivasPorTipo(plantaConfig.id, tipo);
            const cap = slots * mult;
            res[tipo] = cap;
            total += cap;
        });
    ...
    // IMPORTANTE: el link compartido usa la URL interna /Lists/Vehculos,
    // pero la API getbytitle() necesita el TITULO EXACTO de la lista.
    // Se prueban variantes solo para FLOTA, sin tocar la conectividad global.
    const NOMBRES_LISTA_FLOTA = ['Vehículos', 'Vehiculos', 'Vehculos'];
    ...
    async function resolverListaFlota() {
        for (const nombre of NOMBRES_LISTA_FLOTA) {
            try {
                const r = await fetchSPLista(nombre, 'items?$top=1');
                if (r.ok) {
                    LISTA_FLOTA = nombre;
                    logLog(`[ETL] Lista Flota resuelta: ${nombre}`);
                    return;
                }
            } catch(e) {}
        }
        logLog('[WARN] No se pudo resolver el titulo exacto de la lista Flota. Se usa fallback.');
    }
    async function cargarFlotaSP() {
        try {
            const res = await fetchSPLista(LISTA_FLOTA, 'items?$select=Id,Title,TipoVehiculo,Empresa,CUITEmpresa,Chofer,Estado&$orderby=Title&$top=500');
            if(!res.ok) {
            ...
            renderizarBaseFlota();
            actualizarDatalistFlota();
            logLog(`[ETL] Flota (${LISTA_FLOTA}): ${listaFlotaCache.length} vehiculos cargados desde SP.`);
        } catch(e) {
    ...
    async function guardarVehiculo() {
        ...
        try {
            ...
            await cargarFlotaSP();
            logLog(`[ETL] Flota: vehiculo ${pat} guardado en SP (${LISTA_FLOTA}).`);
        } catch(e) {
    ...
    async function initApp() {
        try {
            ...
            Promise.all([
                ...
                logLog("[ETL] Maestros sincronizados. Cargando Flota y Zonas...");
    ...
    function importarCSVFlota() {
        document.getElementById('file_flota_csv').click();
    }
    function procesarCSVFlota(event) {
        const file = event.target.files[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = function(e) { parsearCSVFlota(e.target.result); };
    ...
    function parsearCSVFlota(text) {
        ...
        const idx = {
            patente: header.findIndex(h => h.includes('patente') || h.includes('title')),
            tipo: header.findIndex(h => h.includes('tipo')),
            empresa: header.findIndex(h => h.includes('empresa') || h.includes('proveedor')),
            cuit: header.findIndex(h => h.includes('cuit')),
            chofer: header.findIndex(h => h.includes('chofer')),
            estado: header.findIndex(h => h.includes('estado'))
        };
        for (let i = 1; i < lines.length; i++) {
            const row = lines[i].trim();
            if (!row) continue;
            const cols = row.split(';');
            const patente = (cols[idx.patente] || '').toUpperCase().trim();
            if (!patente) continue;
            if (patentesSet.has(patente)) { duplicados.push(patente); continue; }
            patentesSet.add(patente);
            if (listaFlotaCache.find(v => String(v.patente).toUpperCase() === patente)) { duplicados.push(patente); continue; }
            nuevaLista.push({
                spId: null,
                patente,
                tipo: cols[idx.tipo] || 'Camion',
                empresa: cols[idx.empresa] || '',
                cuit: cols[idx.cuit] || '',
                chofer: cols[idx.chofer] || '',
                estado: cols[idx.estado] || 'Habilitado'
            });
        }
        listaFlotaCache = [...listaFlotaCache, ...nuevaLista];
        renderizarBaseFlota();
        actualizarDatalistFlota();
        let msg = `Flota cargada: ${nuevaLista.length} nuevos registros`;
        if (duplicados.length > 0) {
            msg += ` | Duplicados omitidos: ${duplicados.length}`;
            logLog(`[WARN] Duplicados CSV Flota: ${duplicados.join(', ')}`);
        }
        showToast(msg, 'ok');
        logLog(`[ETL] Flota CSV importada: ${nuevaLista.length} nuevos registros`);
    }
    // --- ABM USUARIOS ---
