...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s8" class="tab-content mt-8 w-full">
        ...
        <p class="text-sm text-slate-400 mb-6">Ventanas operativas y capacidad diaria cargadas desde <span class="text-sky-400">capacidad_turnos_por_planta.csv</span>. TotalDia representa la capacidad LV sin multiplicar por tipo de carga; sabado se calcula por separado.</p>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-sm uppercase text-slate-400 font-bold border-b border-slate-700">
                        <tr><th class="py-4 px-4">Planta Fisica</th><th class="px-4">Lunes a Viernes</th><th class="px-4">Sabados</th><th class="px-4">Minutos x Turno</th><th class="px-4 text-center">Turnos LV</th><th class="px-4 text-center">Turnos Sab</th><th class="px-4 text-center">Tipos habilitados</th><th class="px-4 text-center">TotalDia (LV)</th><th class="px-4 text-center">Estatus</th></tr>
                    </thead>
...
<script>
    ...
    function previewHorariosConfig() {
        configPlantas.forEach((p, idx) => {
            ...
            const slotsLV = calcularTurnosTeoricos(tmp, false);
            const slotsSab = calcularTurnosTeoricos(tmp, true);
            const idxCap = capacidadPorPlantaIndex[p.id] || null;
            const tiposHab = TIPOS_ZONA_CALCULO
                .filter(tipo => idxCap?.tipos?.[tipo]?.Habilitado === 1)
                .join(' / ') || '-';
            ...
            if (elLv) elLv.textContent = Number(slotsLV.toFixed(2));
            if (elSab) elSab.textContent = Number(slotsSab.toFixed(2));
            if (elTip) elTip.textContent = tiposHab;
            if (elTot) elTot.textContent = Number(slotsLV.toFixed(2));
        });
    }
