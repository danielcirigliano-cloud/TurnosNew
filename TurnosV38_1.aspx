            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-xs text-slate-400">Lists/Vehculos</div>
...
<script>
    ...
    const LISTA_FLOTA   = 'Vehículos'; // <-- Tilde agregada para evitar el 404
    ...
    async function cargarFlotaSP() {
        try {
            // Se quita el $select restrictivo para traer todos los campos nativos de la lista
            const res = await fetchSPLista(LISTA_FLOTA, 'items?$top=1000');
            if(!res.ok) throw new Error('HTTP ' + res.status);
            const data = await res.json();
            
            listaFlotaCache = (data.value || []).map(v => {
                // Mapeo exhaustivo tolerando los nombres internos de SharePoint basados en tu CSV
                return {
                    spId:         v.Id,
                    
                    // Campos del CSV
                    patente:      v.Title || v.OData__x0043__x00f3_digo_x0020_del_x0020 || v.Codigo_x0020_del_x0020_Vehiculo || v.Codigo_del_Vehiculo || v.Patente || '',
                    descripcion:  v.Descripci_x00f3_n || v.Descripcion || '',
                    nombre:       v.Nombre || '',
                    apellido:     v.Apellido || '',
                    correo:       v.Correo || '',
                    celular:      v.Celular || '',
                    capacidad1:   v.Capacidad_x0020_1 || v.Capacidad1 || v.Capacidad || '',
                    capacidad2:   v.Capacidad_x0020_2 || v.Capacidad2 || '',
                    capacidad3:   v.Capacidad_x0020_3 || v.Capacidad3 || '',
                    detalle:      v.Detalle || '',
                    tipoDisp:     v.Tipo_x0020_Dispositivo || v.TipoDispositivo || '',
                    numDisp:      v.N_x00fa_mero_x0020_Dispositivo || v.Numero_x0020_Dispositivo || v.NumeroDispositivo || '',
                    etiqCostos:   v.Etiqueta_x0020_Costos || v.EtiquetaCostos || '',
                    flotas:       v.Flotas || '',
                    empleador:    v.Empleador || v.Empresa || v.Proveedor || '',
                    tipoVehiculo: v.Tipo_x0020_Veh_x00ed_culo || v.Tipo_x0020_Vehiculo || v.TipoVehiculo || v.Tipo || '',
                    dni:          v.DNI || '',
                    ultLatitud:   v.OData__x00da_ltima_x0020_latitud || v.Ultima_x0020_latitud || v.UltimaLatitud || '',
                    ultLongitud:  v.OData__x00da_ltima_x0020_longitud || v.Ultima_x0020_longitud || v.UltimaLongitud || '',
                    
                    // Adaptadores de compatibilidad para que no se rompa la grilla visual actual de la App
                    empresa:      v.Empleador || v.Empresa || v.Proveedor || '',
                    cuit:         v.CUITEmpresa || '',
                    tipo:         v.Tipo_x0020_Veh_x00ed_culo || v.Tipo_x0020_Vehiculo || v.TipoVehiculo || v.Tipo || '',
                    capacidad:    v.Capacidad_x0020_1 || v.Capacidad1 || v.Capacidad || '',
                    chofer:       (v.Nombre || v.Apellido) ? `${v.Nombre || ''} ${v.Apellido || ''}`.trim() : (v.Chofer || ''),
