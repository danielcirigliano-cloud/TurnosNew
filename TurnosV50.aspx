    ...
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <div class="w-full grid grid-cols-2 gap-8">
            ...
            <div class="glass p-8 rounded-xl flex flex-col h-[600px]">
                ...
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-4 text-xs overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit v9.5...</div></div>
            </div>
        </div>
    </section>
...
<script>
    ...
    async function sincronizarTodo() {
        ...
        await sincronizarCSVConFeedback('flota');
        await sincronizarCSVConFeedback('zonas');
    ...
    function actualizarEstadoCSVVisual(tipo, titulo, detalle) {
        ...
        if (!box) return; box.className = `csv-status ${tipo}`; box.innerHTML = `<div class="csv-title">${escapeHtml(titulo)}</div><div class="csv-detail">${escapeHtml(detalle)}</div>`;
    }
    ...
    function recomputarResumenGrupo(g) {
        ...
        g.resumen = {
            Planta: base.Planta, Turnos_Calc: filas.length ? Math.max(...filas.map(r => toNum(r.Turnos_Calc, 2))) : 0, Turnos_Sabado: filas.length ? Math.max(...filas.map(r => toNum(r.Turnos_Sabado, 2))) : 0,
            Min_LV: filas.length ? Math.max(...filas.map(r => toNum(r.Min_LV, 2))) : 0, Min_SAB: filas.length ? Math.max(...filas.map(r => toNum(r.Min_SAB, 2))) : 0,
            Minutos_Turno: filas.length ? Math.max(...filas.map(r => toNum(r.Minutos_Turno, 2))) : 0, TotalDia: filas.length ? Math.max(...filas.map(r => toNum(r.TotalDia, 2))) : 0, Estatus: base.Estatus || 'OK'
        };
    }
    ...
    function collectCapacidadRowsFromDom() {
        ...
        return rows.map(tr => ({
            Planta: tr.dataset.planta || '', Tipo_Carga: tr.dataset.tipo || '', Habilitado: Number(tr.querySelector('[data-field="Habilitado"]')?.value || 0), Turnos_Cargados: toNum(tr.querySelector('[data-field="Turnos_Cargados"]')?.value, 2),
            Turnos_Calc: toNum(tr.querySelector('[data-field="Turnos_Calc"]')?.value, 2), Turnos_Sabado: toNum(tr.querySelector('[data-field="Turnos_Sabado"]')?.value, 2), Min_LV: toNum(tr.querySelector('[data-field="Min_LV"]')?.value, 2),
            Min_SAB: toNum(tr.querySelector('[data-field="Min_SAB"]')?.value, 2), Minutos_Turno: toNum(tr.querySelector('[data-field="Minutos_Turno"]')?.value, 2), TotalDia: toNum(tr.querySelector('[data-field="TotalDia"]')?.value, 2), Estatus: String(tr.querySelector('[data-field="Estatus"]')?.value || 'OK').trim()
        }));
    }
    ...
    async function cargarCapacidadTurnosCSV() {
        try {
            ...
            actualizarEstadoCSVVisual('ok', 'CSV válido', `Ruta: ${rutaUsada} | Registros: ${parsed.length} | Columnas: ${headersRaw.join(', ')}`); return parsed;
        } catch (e) {
    ...
    function poblarSelectZonasPorPlanta(plantaFisica, valorSeleccionado) {
        ...
        else { zonas.forEach(z => { const label = z.nombre || z.tipo; const val = z.tipo || z.nombre; if (val && !Array.from(sel.options).some(o => o.value === val)) { sel.add(new Option(label, val)); } }); }
    ...
    function calcularCapacidadPlantaPorTipo(plantaConfig, esSabado) {
        ...
        TIPOS_ZONA_CALCULO.forEach(tipo => {
            ...
            if (reg && reg.Habilitado === 1) cap = esSabado ? toNum(reg.Turnos_Sabado, 2) : toNum(reg.Turnos_Cargados || reg.TotalDia || reg.Turnos_Calc, 2); else if (!reg) cap = slots; res[tipo] = cap;
        });
    ...
    function autocompletarPatenteFlota() { const pat = document.getElementById('f_patente')?.value.trim().toUpperCase(); if (!pat) return; document.getElementById('f_patente').value = pat; const v = listaFlotaCache.find(x => String(x.patente).toUpperCase() === pat); if (v && v.empresa && !document.getElementById('f_proveedor').value) { showToast(`Patente ${pat} encontrada en flota (${v.empresa}).`, 'ok'); } }
    ...
    function mapearFilaFlota(cols) {
        ...
        return { spId: null, patente, tipo, empresa, cuit: '', chofer, estado: patente ? 'Habilitado' : 'Deshabilitado', descripcion, nombre, apellido, correo, celular, capacidad1, capacidad2, capacidad3, detalle, tipoDispositivo, numeroDispositivo, etiquetaCostos, flotas, dni, ultimaLatitud, ultimaLongitud, raw: cols };
    }
    ...
    function parsearCSVFlota(text) {
        ...
        let idxHeader = 0; if (String(lines[0] || '').startsWith('ListSchema=')) { flotaSchemaLine = lines[0].replace(/^\uFEFF/, ''); idxHeader = 1; }
        ...
        for (let i = idxHeader + 1; i < lines.length; i++) {
            const cols = parseCsvLineSeguro(lines[i]); if (!cols.length) continue; const item = mapearFilaFlota(cols); if (!item.patente) continue; rows.push(item);
        }
        listaFlotaCache = rows; listaFlotaRawRows = rows.map(r => r.raw);
        renderizarBaseFlota(); actualizarDatalistFlota(); 
    }

    async function cargarFlotaSP() { try { await importarCSVSharePoint('flota', true); } catch(e) { renderizarBaseFlota(); } }
    function renderizarBaseFlota() {
        const tb = document.getElementById('tabla-flota'); if(!tb) return;
        if(listaFlotaCache.length === 0) { tb.innerHTML = '<tr><td colspan="7" class="p-6 text-center text-slate-500 text-sm">Sin vehiculos cargados.</td></tr>'; return; }
        tb.innerHTML = listaFlotaCache.map((v, i) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm cursor-pointer" onclick="cargarFlotaForm(${i})">
                <td class="py-3 px-3 font-bold text-amber-300">${escapeHtml(v.patente)}</td><td class="px-3 text-slate-300">${escapeHtml(v.tipo || v.descripcion || '-')}</td><td class="px-3 text-white font-semibold">${escapeHtml(v.empresa || v.flotas || '-')}</td><td class="px-3 text-slate-400 text-xs">${escapeHtml(v.cuit || '-')}</td><td class="px-3 text-slate-300 text-xs">${escapeHtml(v.chofer || '-')}</td><td class="px-3 font-bold ${v.estado==='Habilitado'?'text-emerald-400':v.estado==='Vencido'?'text-amber-400':'text-rose-400'}">${escapeHtml(v.estado)}</td><td class="px-3 text-center"><button class="text-sky-400 text-xs hover:underline font-bold">Editar</button></td>
            </tr>`).join('');
    }
    ...
    function cargarFlotaForm(idx) {
        const v = listaFlotaCache[idx]; const ch = splitChofer(v.chofer);
        document.getElementById('fl_item_id').value = idx; document.getElementById('fl_patente').value = v.patente || ''; document.getElementById('fl_tipo').value = v.tipo || ''; document.getElementById('fl_empresa').value = v.empresa || ''; document.getElementById('fl_cuit').value = v.cuit || ''; document.getElementById('fl_apellido').value = ch.apellido; document.getElementById('fl_nombre').value = ch.nombre; document.getElementById('fl_estado').value = v.estado || '';
    ...
    function limpiarFormularioFlota() {
        document.getElementById('fl_item_id').value = ''; ['fl_patente','fl_empresa','fl_cuit','fl_apellido','fl_nombre'].forEach(id => document.getElementById(id).value = '');
    ...
    async function guardarEdicionFlota() {
        const pat = document.getElementById('fl_patente').value.trim().toUpperCase(); if(!pat) return showToast('Patente requerida.', 'err');
        const tipo = document.getElementById('fl_tipo').value; const estado = document.getElementById('fl_estado').value;
        if(!tipo) return showToast("Debe seleccionar un Tipo", "err"); if(!estado) return showToast("Debe seleccionar un Estado", "err");
        const idx = document.getElementById('fl_item_id').value; const apellido = document.getElementById('fl_apellido').value.trim(); const nombre = document.getElementById('fl_nombre').value.trim(); const chofer = [apellido, nombre].filter(Boolean).join(' ');
        const existente = idx !== '' && listaFlotaCache[parseInt(idx,10)] ? listaFlotaCache[parseInt(idx,10)] : listaFlotaCache.find(v => v.patente === pat);
        const headerLen = flotaHeaderCols.length || 19; const raw = existente && Array.isArray(existente.raw) ? [...existente.raw] : Array(headerLen).fill('');
        while (raw.length < headerLen) raw.push('');
        raw[0] = pat; raw[2] = nombre; raw[3] = apellido; raw[14] = document.getElementById('fl_empresa').value.trim(); raw[15] = tipo;
        const obj = mapearFilaFlota(raw); obj.cuit = document.getElementById('fl_cuit').value.replace(/[^0-9\-]/g,''); obj.estado = estado; obj.chofer = chofer;
        if (idx !== '' && listaFlotaCache[parseInt(idx,10)]) listaFlotaCache[parseInt(idx,10)] = obj; else if (!existente) listaFlotaCache.push(obj); else { const pos = listaFlotaCache.findIndex(v => v.patente === pat); if (pos >= 0) listaFlotaCache[pos] = obj; }
        renderizarBaseFlota(); actualizarDatalistFlota(); limpiarFormularioFlota(); showToast('Vehiculo guardado. Sincronizando Flota.csv...', 'ok'); await actualizarCSVSharePoint('flota');
    }
    ...
    async function eliminarFlota() {
        ...
        listaFlotaCache.splice(idx, 1); renderizarBaseFlota(); actualizarDatalistFlota(); limpiarFormularioFlota();
    ...
    function parsearCSVZonas(text) {
        ...
        for(let i=1; i<lines.length; i++) {
            ...
            if(cols.length >= 6) { listaZonasCache.push({ id: cols[0].trim(), planta: cols[1].trim(), nombre: cols[2].trim(), tipo: cols[3].trim(), capacidad: cols[4].trim(), estado: cols[5].trim() || 'Activo' }); }
        }
        renderizarBaseZonas();
    }

    async function cargarZonasSP() { try { await importarCSVSharePoint('zonas', true); poblarSelectsABMInicial(); } catch(e) { renderizarBaseZonas(); } }

    function renderizarBaseZonas() {
        const tb = document.getElementById('tabla-zonas'); if(!tb) return;
        if(listaZonasCache.length === 0) { tb.innerHTML = '<tr><td colspan="6" class="p-6 text-center text-slate-500 text-sm">Sin zonas de descarga cargadas.</td></tr>'; return; }
        tb.innerHTML = listaZonasCache.map((z, idx) => `<tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm cursor-pointer" onclick="cargarZonaForm(${idx})"><td class="p-3 font-semibold text-amber-400">${escapeHtml(z.nombre)}</td><td class="p-3 text-white font-semibold">${escapeHtml(z.planta)}</td><td class="p-3 text-slate-400">${escapeHtml(z.tipo)}</td><td class="p-3 text-slate-400 text-center">${escapeHtml(z.capacidad)}</td><td class="p-3 font-bold ${z.estado === 'Activo' || z.estado === 'Habilitado' ? 'text-emerald-400' : 'text-rose-400'}">${escapeHtml(z.estado)}</td><td class="p-3 text-sky-400 underline text-center">Editar</td></tr>`).join('');
    }
    ...
    function limpiarFormularioZona() {
        ...
        if(document.getElementById('z_planta').options.length > 0) document.getElementById('z_planta').selectedIndex = 0; document.getElementById('z_tipo').value = ''; document.getElementById('z_estado').value = '';
    ...
    async function guardarEdicionZona() {
        ...
        const obj = { id: `Z-${Date.now()}`, planta: zPlanta, nombre: document.getElementById('z_nombre').value.trim(), tipo: zTipo, capacidad: document.getElementById('z_capacidad').value, estado: zEstado };
        if(idx === '') listaZonasCache.push(obj); else { obj.id = listaZonasCache[idx].id; listaZonasCache[idx] = obj; }
    ...
    async function eliminarZona() {
        ...
        listaZonasCache.splice(idx, 1); renderizarBaseZonas(); limpiarFormularioZona();
