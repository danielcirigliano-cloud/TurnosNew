...
/).filter(l=>l.trim());
 ...
 const idx={planta:header.indexOf('planta'),lvDesde:header.indexOf('lv_desde'),lvHasta:header.indexOf('lv_hasta'),sabHab:header.indexOf('sab_habilitado'),sabDesde:header.indexOf('sab_desde'),sabHasta:header.indexOf('sab_hasta'),min:header.indexOf('minutos_turno'),ins:header.indexOf('insumos'),pack:header.indexOf('packaging'),np:header.indexOf('no_productivo'),est:header.indexOf('estatus')};
 const result=[];
 for(let i=1;i<lines.length;i++){const c=lines[i].split(';'); if(!c[idx.planta]) continue;
 result.push({id:c[idx.planta],hInLV:parseInt(c[idx.lvDesde])||0,hOutLV:parseInt(c[idx.lvHasta])||0,sabado:(c[idx.sabHab]==='1'),hInS:parseInt(c[idx.sabDesde])||0,hOutS:parseInt(c[idx.sabHasta])||0,duracion:parseInt(c[idx.min])||40,insumos:parseInt(c[idx.ins])||0,packaging:parseInt(c[idx.pack])||0,noProductivo:parseInt(c[idx.np])||0,estatus:c[idx.est]||'OK'});
 }
 if(result.length){configPlantas=result;renderizarABMHorarios();renderizarResumenCapacidad();logLog('[ETL CSV] ConfiguracionPlanta cargada '+result.length);} }

function obtenerTiposHabilitadosPlanta(p){let n=0;if(p.insumos==1)n++;if(p.packaging==1)n++;if(p.noProductivo==1)n++;return n;}
function calcularTotalDiaPorTipos(p){const base=calcularTurnosTeoricos(p,false)+calcularTurnosTeoricos(p,true);return base*obtenerTiposHabilitadosPlanta(p);} 
