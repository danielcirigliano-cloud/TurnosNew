...
async function resolverCamposFlota() {
    ...
    async function cargarFlotaSP() {
        try {
            // Seleccionamos Id + campos simples. Si la lista tiene columnas con
            // nombres diferentes, el error aparecera en el log con detalle completo.
            const res = await fetchSPLista(LISTA_FLOTA,
                'items?$select=Id,Patente,Tipo,Empresa,CUIT,Chofer,Estado&$orderby=Patente asc&$top=500');
            if (!res.ok) {
