...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="sFlota" class="tab-content mt-8 w-full">
        <h2 class="text-2xl font-bold mb-2 text-sky-400">Flota</h2><div class="flex gap-2 mb-4">
...
<script>
    ...
    async function cargarFlotaSP() {
        try {
            const res = await fetchSPLista(LISTA_FLOTA, 'items?$select=Id,Title,TipoVehiculo,Empresa,CUITEmpresa,Chofer,Estado&$orderby=Patente&$top=500');
...
function procesarCSVFlota(event) {
    ...
    reader.onload = function(e) {
        parsearCSVFlota(e.target.result);
    };
...
|/);
    ...
    const idx = {
        ...
        empresa: header.findIndex(h => h.includes('empresa')),
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (!row) continue;
        const cols = row.split(';');
        let patente = (cols[idx.patente] || '').toUpperCase().trim();
        ...
        nuevaLista.push({ spId:null, patente:patente, tipo:cols[idx.tipo]||'Camion', empresa:cols[idx.empresa]||'', cuit:cols[idx.cuit]||'', chofer:cols[idx.chofer]||'', estado:cols[idx.estado]||'Habilitado'});
    }
    listaFlotaCache = [...listaFlotaCache, ...nuevaLista];
    renderizarBaseFlota();
    actualizarDatalistFlota();
    let msg = `Flota cargada: ${nuevaLista.length}`;
    if (duplicados.length>0){ msg+=` | duplicados omitidos: ${duplicados.length}`; logLog(`Duplicados: ${duplicados.join(', ')}`);}    
    showToast(msg,'ok');
}
