...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s8" class="tab-content mt-8 w-full">
        ...
        <p class="text-sm text-slate-400 mb-4">Vista productiva alineada a <span class="text-sky-400">capacidad_turnos_por_planta.csv</span>. La fila principal resume la planta y, con el símbolo <strong>+</strong>, despliega el detalle exacto por <strong>Tipo_Carga</strong> tal como está en el CSV. Los cambios guardados desde esta pantalla actualizan el CSV y refrescan toda la información consultada en <strong>Turnos Diarios</strong> y pantallas relacionadas.</p>
    ...
    function renderizarABMHorarios(preserveExpanded = true) {
        ...
        if (capacidadPorPlantaTipoCache && capacidadPorPlantaTipoCache.length) {
            ...
            const rows = grupos.map(g => {
                ...
                const isOpen = expandedPlants.includes(plantaKey);
                const claseOpen = isOpen ? 'open' : '';
                const simbolo = isOpen ? '−' : '+';
                const saturacion = g.filas.length ? Math.max(...g.filas.map(x => {
                    const c = toNum(x.Turnos_Cargados,2); const t = Math.max(1,toNum(x.TotalDia || x.Turnos_Calc,2)); return c/t;
                })) : 0;
                ...
                const children = (g.filas || []).sort((a,b) => {
                    const ta = String(a.Tipo_Carga || '').toUpperCase();
                    const tb = String(b.Tipo_Carga || '').toUpperCase();
                    const ia = ORDEN_TIPOS.includes(ta) ? ORDEN_TIPOS.indexOf(ta) : 999;
                    const ib = ORDEN_TIPOS.includes(tb) ? ORDEN_TIPOS.indexOf(tb) : 999;
                    return ia - ib;
                }).map(row => {
                    ...
                    const hidden = isOpen ? '' : 'hidden-row';
