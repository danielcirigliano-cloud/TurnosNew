    ...
    <section id="s7" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-6 rounded-xl w-full">
            <div class="overflow-x-auto h-[500px]">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-[10px] uppercase text-slate-400 font-bold sticky top-0 shadow">
                        <tr><th class="py-3 px-3">ID</th><th class="px-3">Nombre y Apellido</th><th class="px-3">Email</th><th class="px-3">Planta</th><th class="px-3">Reporte</th><th class="px-3">Estado</th><th class="px-3 text-center">Accion</th></tr>
                    </thead>
                    <tbody id="tabla-usuarios" class="divide-y divide-slate-700/50">
                        <tr><td colspan="7" class="p-6 text-center text-slate-500 text-sm">Sin datos.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <div class="w-full grid grid-cols-2 gap-8">
            ...
            <div class="glass p-8 rounded-xl flex flex-col h-[600px]">
                ...
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-4 font-mono text-xs overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit v4.4...</div></div>
            </div>
        </div>
    </section>
...
<script>
    ...
    function aplicarFiltrosMonitor() {
        ...
        // La grilla principal ya no filtra por fecha. Muestra toda la base ordenada estructuralmente.
    ...
    async function cargarTurnos() {
        ...
        try {
            ...
            // APLICACIÓN DEL ORDENAMIENTO ESTRUCTURAL (Planta -> Fecha -> Estado)
            ordenarEstructuralmenteBase(listaTurnosCache);
    ...
    function conmutarFechaDesdeCalendario(nuevaFecha) {
        ...
        renderizarGrilaCalendario(); 
        // La grilla principal no se filtra por fecha, pero esto actualiza Disponibilidad y Resumen
        aplicarFiltrosMonitor(); 
    ...
    function renderizarBaseUsuarios() {
        ...
        tb.innerHTML = listaUsuariosCache.map((u, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm cursor-pointer" onclick="cargarUsuForm(${idx})">
                ...
                <td class="p-3 font-mono font-bold ${u.reporte === 'SI' ? 'text-sky-400' : 'text-slate-500'}">${escapeHtml(u.reporte)}</td>
    ...
    function cargarUsuForm(idx) {
        ...
        document.getElementById('u_reporte').value = u.reporte || 'SI';
    ...
    async function guardarEdicionUsuario() {
        ...
        const obj = { 
            ...
            reporte: document.getElementById('u_reporte').value,
        ...
        document.getElementById('u_reporte').value = 'SI';
