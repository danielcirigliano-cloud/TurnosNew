    ...
    <nav class="space-y-1 flex-1 overflow-y-auto" id="main-nav">
        ...
        <div class="mt-2">
            <button onclick="toggleMenuConfig()" class="w-full text-left rounded tab-btn flex justify-between items-center" id="btn-menu-config">
                ...
                <span id="icon-menu-config" class="text-slate-400 text-xs">▶</span>
            </button>
...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
            ...
            <div class="col-span-9 glass p-4 rounded-xl flex flex-col h-full">
                <div class="flex gap-2 mb-3 items-end bg-slate-900/50 p-3 rounded-lg border border-slate-700 shrink-0 flex-wrap">
                    ...
                    <div>
                        ...
                        <input type="text" id="filtro_patente" oninput="aplicarFiltrosMonitor()" placeholder="AAA000" maxlength="7"
                            class="bg-slate-950 border border-slate-700 p-1.5 text-xs text-white rounded outline-none w-24 uppercase">
                    </div>
                    <div>
                        <label class="block text-[10px] uppercase text-slate-400 font-bold mb-1">Proveedor</label>
                        <input type="text" id="filtro_proveedor" oninput="aplicarFiltrosMonitor()" placeholder="Buscar..."
                            class="bg-slate-950 border border-slate-700 p-1.5 text-xs text-white rounded outline-none w-36">
                    </div>
    ...
    <section id="s1" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-8 rounded-xl w-full grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <!-- Cambio 2: mismas opciones y comportamiento que el filtro del monitor.
                     onchange dispara handleHorariosChange para recalcular slots. -->
    ...
    <section id="sFlota" class="tab-content mt-8 w-full">
        ...
        <p class="text-xs text-slate-400 mb-4">Conectado a: <span class="font-mono text-sky-400">/sites/GestiondeTurnos/Lists/Vehiculos</span></p>
        <div class="glass p-6 rounded-xl mb-6 border border-slate-700 w-full text-sm">
            ...
            <div class="grid grid-cols-12 gap-4 items-end">
                ...
                <div class="col-span-2">
                    <label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Habilitacion Vto.</label>
                    <input type="date" id="fl_vto" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none focus:border-sky-500">
                </div>
                <div class="col-span-1">
                    <label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Estado</label>
        ...
        <div class="glass p-6 rounded-xl mb-6 border border-slate-700 w-full text-sm">
            ...
            <div class="grid grid-cols-12 gap-4 items-end">
                ...
                <div class="col-span-2">
                    ...
                    <input type="number" id="z_capacidad" min="1" max="99" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none focus:border-sky-500" placeholder="10">
                </div>
                <div class="col-span-2">
                    <label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Estado</label>
    ...
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <!-- Panel de Sincronizacion de Maestros CSV a SharePoint -->
        <div class="glass p-6 rounded-xl mb-6 border border-slate-700">
            <div class="flex items-center justify-between mb-4 border-b border-slate-700 pb-3">
                <div>
                    <h3 class="text-base font-bold text-sky-300 uppercase tracking-wider">Sincronizacion de Maestros a SharePoint</h3>
                    <p class="text-xs text-slate-400 mt-1">Sube los archivos CSV actuales (Proveedores y Usuarios) a la carpeta <span class="font-mono text-sky-400">Documentos compartidos</span> del sitio. Sobreescribe el archivo existente.</p>
                </div>
                <button onclick="sincronizarAmbosCSV()" class="bg-sky-700 hover:bg-sky-600 text-white px-5 py-2.5 text-sm font-bold rounded shadow transition flex items-center gap-2">
                    Sincronizar Todo
                </button>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-xs text-slate-400 font-mono">proveedores.csv</div>
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-xs text-slate-400 font-mono">Usuarios.csv</div>
...
<script>
    ...
    async function sincronizarCSVConFeedback(tipo) {
        ...
        if (!statusEl) { await actualizarCSVSharePoint(tipo); return; }

        const cache = tipo === 'prov' ? listaProveedoresCache : listaUsuariosCache;
        if (cache.length === 0) {
            statusEl.textContent = `Sin registros en memoria para sincronizar.`;
            statusEl.className = 'text-xs mt-1.5 font-semibold text-amber-400';
            showToast(`Base de ${label} vacia, no hay nada que subir.`, 'err');
            return;
        }

        statusEl.textContent = `Subiendo ${cache.length} registros...`;
        statusEl.className = 'text-xs mt-1.5 font-semibold text-sky-400 animate-pulse';

        try {
            await actualizarCSVSharePoint(tipo);
            const ahora = new Date().toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });
            statusEl.textContent = `OK — ${cache.length} registros sincronizados a las ${ahora}`;
            statusEl.className = 'text-xs mt-1.5 font-semibold text-emerald-400';
            logLog(`[ETL CSV] ${label}: ${cache.length} registros subidos a SharePoint Documentos.`);
        } catch(e) {
            statusEl.textContent = `Error al sincronizar: ${e.message}`;
            statusEl.className = 'text-xs mt-1.5 font-semibold text-rose-400';
        }
    }

    // Sincroniza ambas bases (Proveedores y Usuarios) en secuencia.
    async function sincronizarAmbosCSV() {
    ...
    // ---- ABM FLOTA ----
    // Conectado a: /sites/GestiondeTurnos/Lists/Vehiculos
    function renderizarBaseFlota() {
        const tbody = document.getElementById('tabla-flota');
        if(!tbody) return;
        if(listaFlotaCache.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="p-6 text-center text-slate-500 text-sm">Sin vehiculos cargados. Guarda el primero con el formulario.</td></tr>';
            return;
        }
        tbody.innerHTML = listaFlotaCache.map((v, i) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm">
                ...
                <td class="px-3 font-mono text-slate-400">${escapeHtml(v.cuit)}</td>
                <td class="px-3 text-slate-400">${escapeHtml(v.vto)}</td>
                ...
                <td class="px-3 text-center"><button onclick="cargarFlotaForm(${i})" class="text-sky-400 text-xs hover:underline">Editar</button></td>
            </tr>`).join('');
    }
    function cargarFlotaForm(idx) {
        ...
        document.getElementById('fl_vto').value     = v.vto     || '';
    ...
    function limpiarFormularioFlota() {
        ...
        ['fl_patente','fl_empresa','fl_cuit','fl_vto'].forEach(id => document.getElementById(id).value = '');
    ...
    async function guardarVehiculo() {
        ...
        const obj = {
            ...
            cuit:    document.getElementById('fl_cuit').value.replace(/[^0-9\-]/g,''),
            vto:     document.getElementById('fl_vto').value,
            estado:  document.getElementById('fl_estado').value
        };
        if(idx === '') listaFlotaCache.push(obj);
        else listaFlotaCache[parseInt(idx)] = obj;
        renderizarBaseFlota();
        limpiarFormularioFlota();
        showToast('Vehiculo guardado.', 'ok');
    }

    // ---- ABM ZONAS DE DESCARGA ----
    // Conectado a: /sites/GestiondeTurnos/Lists/Zonas descarga
    function renderizarBaseZonas() {
        // Poblar select de planta en el form de zonas
        ...
        tbody.innerHTML = listaZonasCache.map((z, i) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm">
                ...
                <td class="px-3 text-center text-slate-300">${escapeHtml(String(z.capacidad||''))}</td>
                <td class="px-3 font-bold ${z.estado==='Activo'?'text-emerald-400':'text-rose-400'}">${escapeHtml(z.estado)}</td>
                <td class="px-3 text-center"><button onclick="cargarZonaForm(${i})" class="text-sky-400 text-xs hover:underline">Editar</button></td>
            </tr>`).join('');
    }
    function cargarZonaForm(idx) {
        const z = listaZonasCache[idx];
        document.getElementById('z_item_id').value  = idx;
    ...
    async function guardarZona() {
        ...
        const idx = document.getElementById('z_item_id').value;
        const obj = {
            nombre:    nombre,
            planta:    document.getElementById('z_planta').value,
            tipo:      document.getElementById('z_tipo').value,
        ...
        else listaZonasCache[parseInt(idx)] = obj;
        renderizarBaseZonas();
        limpiarFormularioZona();
        showToast('Zona guardada.', 'ok');
    }
    ...
    // Punto 5: ventana habilitada para Alta/Reprogramacion = hoy + mañana
    // (2 dias). Desde pasado mañana en adelante queda bloqueado para alta.
    ...
    function validarFormularioTurno(modo) {
        ...
        if(modo === 'ALTA' || modo === 'REPROGRAMAR') {
            ...
            if(fechaIngresada > maxHabilitada) return marcarError('f_fecha', "Solo se permiten Altas/Reprogramaciones para hoy o mañana.");
        }
    ...
    function setModoIndicador(texto, claseColor) {
    ...
    function renderBotonesABM(modo) {
        ...
        if(!panel) return;
        if (modo === 'ALTA') {
            setModoIndicador('ALTA DE TURNO','bg-emerald-900 text-emerald-300 border-emerald-700');
            ...
            setModoIndicador('CONSULTA','bg-sky-900 text-sky-300 border-sky-700');
            // 2.1: si esta Aprobado, solo Ver + controles In/Out (sin Modificar ni Eliminar)
            // 2.7: no hay boton "Volver al Monitor" - el Cancelar cumple esa funcion
            ...
            setModoIndicador('MODIFICANDO','bg-amber-900 text-amber-300 border-amber-700');
            panel.innerHTML = `
            ...
            setModoIndicador('APROBACION','bg-emerald-900 text-emerald-300 border-emerald-700');
            ...
            setModoIndicador('REPROGRAMACION','bg-indigo-900 text-indigo-300 border-indigo-700');
    ...
    function cargarTurnoEnFormulario(id) {
        ...
        const esPendiente = est === 'PENDIENTE';

        // 2.3 y 2.4: si el turno es Pendiente, Planta y Zona muestran "Seleccione..."
        // en vez de el valor guardado, para que el usuario lo confirme al Modificar.
        const selPD = document.getElementById('f_planta_descarga');
        const selP  = document.getElementById('f_planta');
        // Limpiar opcion vacia previa si existe
        if(selPD.options[0]?.value === '') selPD.remove(0);
        if(selP.options[0]?.value === '')  selP.remove(0);

        if(esPendiente) {
            selPD.insertBefore(new Option('-- Seleccione Planta --',''), selPD.firstChild);
            selP.insertBefore(new Option('-- Seleccione Zona --',''), selP.firstChild);
            selPD.selectedIndex = 0;
            selP.selectedIndex = 0;
        } else {
            selPD.value = t.PlantaDescarga || selPD.options[0]?.value || '';
            selP.value  = t.Planta || selP.options[0]?.value || '';
        }
    ...
    function autocompletarProveedor() {
        ...
        if (!provEncontrado) {
            // 2.8: CUIT no existe → abrir modal de nuevo proveedor
            marcarError('f_cuit', "CUIT no encontrado. Se abrira el formulario de alta de proveedor.");
    ...
    // Disparo automatico a las 18:00 SOLO si la pestaña esta abierta en ese
