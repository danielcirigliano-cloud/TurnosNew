...
<aside class="w-64 glass flex flex-col p-4 border-r border-slate-700 h-full z-10">
    ...
    <div class="mb-5 text-xs inline-flex items-center gap-2 p-1.5 rounded-full" style="background:rgba(0,0,0,.4); border:1px solid #334155;">
        ...
        <span class="font-bold text-emerald-400">Online REST API</span>
    </div>
...
<main class="flex-1 p-6 overflow-y-auto relative">
    <div id="header-user-box" class="absolute top-6 right-8 glass px-5 py-2.5 rounded-full flex items-center gap-2 text-sm border border-slate-700 bg-slate-900/50 z-20">
        ...
        <span id="app-user-name" class="text-sky-400 font-bold font-mono">Conectando...</span>
    </div>
    ...
    <section id="s2" class="tab-content active mt-8">
        ...
        <div class="grid grid-cols-6 gap-4">
            <div class="col-span-4 glass p-4 rounded-xl h-[720px] flex flex-col">
            ...
            <div class="col-span-2 glass p-4 rounded-xl flex flex-col h-[720px] overflow-hidden">
                <h3 class="text-sm font-bold text-sky-400 uppercase tracking-wider text-center mb-2">Disponibilidad Logistica</h3>
        ...
        <div class="glass p-6 rounded-xl max-w-4xl grid grid-cols-2 gap-4">
            ...
            <div class="col-span-2 grid grid-cols-3 gap-4 border-b border-slate-700 pb-4 mb-2">
                <div>
                    ...
                    <input type="text" id="f_patente" placeholder="AAA111" class="bg-slate-900 border border-slate-700 p-2 w-full font-mono text-amber-300 uppercase focus:border-sky-500 outline-none f-control">
                </div>
                <div>
                    <label class="block text-[10px] uppercase text-slate-400 font-bold">N Proveedor / CUIT</label>
                    <input type="text" id="f_cuit" placeholder="ID Proveedor" onblur="autocompletarProveedor()" class="bg-slate-900 border border-slate-700 p-2 w-full text-amber-300 font-bold focus:border-sky-500 outline-none f-control">
                </div>
                <div>
                    <label class="block text-[10px] uppercase text-slate-400 font-bold">Num OC</label>
                    <input type="text" id="f_oc" placeholder="Ej: 698765" class="bg-slate-900 border border-slate-700 p-2 w-full text-white focus:border-sky-500 outline-none f-control">
                </div>
            </div>

            <div class="col-span-2">
                <label class="block text-[10px] uppercase text-slate-400 font-bold">Razon Social Proveedor</label>
                <input type="text" id="f_proveedor" placeholder="Proveedor" class="bg-slate-900 border border-slate-700 p-2 w-full text-white focus:border-sky-500 outline-none f-control">
            </div>
            <div class="col-span-2">
                <label class="block text-[10px] uppercase text-slate-400 font-bold">Email de Contacto</label>
                <input type="email" id="f_email" placeholder="correo@proveedor.com" class="bg-slate-900 border border-slate-700 p-2 w-full text-white focus:border-sky-500 outline-none f-control">
            </div>

            <div>
                <label class="block text-[10px] uppercase text-slate-400 font-bold">Planta Fisica (Ubicacion)</label>
                <select id="f_planta_descarga" onchange="regenerarHorariosABM(esModoFiltradoActivo())" class="bg-slate-900 border border-slate-700 p-2 w-full text-sky-400 font-bold focus:border-sky-500 outline-none f-control">
                    </select>
            </div>
            <div>
                <label class="block text-[10px] uppercase text-slate-400 font-bold">Tipo Carga (Area)</label>
                <select id="f_planta" class="bg-slate-900 border border-slate-700 p-2 w-full text-white focus:border-sky-500 outline-none f-control">
            ...
            <div>
                ...
                <input type="date" id="f_fecha" onchange="regenerarHorariosABM(esModoFiltradoActivo()); autoDia();" class="bg-slate-900 border border-slate-700 p-2 w-full text-white focus:border-sky-500 outline-none f-control f-reprog">
            </div>
            <div>
                <label class="block text-[10px] uppercase text-slate-400 font-bold">Slot Horario Calculado</label>
                <select id="f_horario" class="bg-slate-900 border border-slate-700 p-2 w-full text-sky-300 font-mono font-bold focus:border-sky-500 outline-none f-control f-reprog"></select>
            </div>
            ...
            <div class="col-span-2 flex gap-3 mt-2" id="panel-botones-abm">
                </div>
            ...
            <div class="col-span-2 mt-2 border-t border-slate-700 pt-3 flex items-center justify-between" id="panel-tracking">
                <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <input type="text" id="f_hora_ing" readonly class="bg-transparent text-emerald-400 w-16 text-xs text-center border-b border-emerald-900 ml-2" placeholder="--:--">
                        <input type="text" id="f_hora_sal" readonly class="bg-transparent text-rose-400 w-16 text-xs text-center border-b border-rose-900" placeholder="--:--">
                    </div>
                </div>
    ...
    <section id="s8" class="tab-content mt-8">
        ...
        <p class="text-xs text-slate-400 mb-4">Ajuste las ventanas operativas y duracion de turnos. Los sabados poseen reglas especiales integradas.</p>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-xs uppercase text-slate-400 font-bold border-b border-slate-700">
                        <tr>
                            <th class="py-3 px-3">Planta Fisica</th>
                            <th class="px-3">Lunes a Viernes (Ventana)</th>
                            <th class="px-3">Sabados (Ventana)</th>
                            <th class="px-3">Tiempo x Turno</th>
                            <th class="px-3 text-center">Accion</th>
                        </tr>
                    </thead>
                    <tbody id="tabla-config-horarios" class="divide-y divide-slate-700/50">
                        </tbody>
                </table>
            </div>
            <div class="mt-4 flex gap-2">
                <button onclick="guardarConfiguracionHorarios()" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-xs font-bold rounded shadow transition text-white">💾 Guardar Configuracion General</button>
            </div>
        </div>
    </section>
    ...
    <section id="s6" class="tab-content mt-8">
        ...
        <div class="glass p-4 rounded mb-6 border border-slate-700 flex gap-3 items-end max-w-5xl text-xs">
            ...
            <div class="w-1/6">
                <label class="block uppercase text-slate-400 font-bold mb-1">N Proveedor</label>
            ...
            <div class="overflow-x-auto h-96">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-[10px] uppercase text-slate-400 font-bold sticky top-0">
                        <tr><th class="py-3 px-3">N Proveedor</th><th class="px-3">Razon Social</th><th class="px-3">Tipo Insumo</th><th class="px-3">Responsable</th><th class="px-3">Estado</th><th class="px-3">Accion</th></tr>
                    </thead>
    ...
    <section id="s9" class="tab-content mt-8">
        <h2 class="text-2xl font-bold mb-4 text-sky-400">Modulo ETL & Auditoria de Procesos</h2>
        
        <div class="max-w-5xl grid grid-cols-2 gap-6">
            <div class="glass p-6 rounded-xl">
                <h3 class="text-sm font-bold text-slate-300 uppercase mb-4 border-b border-slate-700 pb-2">Flujo de Automatizacion (En vivo)</h3>
                <div class="flex flex-col gap-4 relative">
                    ...
                    <div id="etl-extract" class="etl-step flex items-center gap-4 z-10 bg-slate-900 p-3 rounded border border-slate-700">
                        ...
                        <div>
                            <div class="font-bold text-sky-400 text-sm">Extraccion (E)</div>
                            <div class="text-xs text-slate-400">Lectura de buffers y Formularios HTML.</div>
                        </div>
                    </div>
                    ...
                    <div id="etl-transform" class="etl-step flex items-center gap-4 z-10 bg-slate-900 p-3 rounded border border-slate-700">
                        ...
                        <div>
                            <div class="font-bold text-amber-400 text-sm">Transformacion (T)</div>
                            <div class="text-xs text-slate-400">Mapeo OData, validacion de campos e Internal Names.</div>
                        </div>
                    </div>
                    ...
                    <div id="etl-load" class="etl-step flex items-center gap-4 z-10 bg-slate-900 p-3 rounded border border-slate-700">
                        ...
                        <div>
                            <div class="font-bold text-emerald-400 text-sm">Carga (L)</div>
                            <div class="text-xs text-slate-400">POST/MERGE a SharePoint REST API.</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section id="s5" class="tab-content mt-8">
        <div class="flex justify-between items-center mb-4 max-w-5xl">
            ...
            <div class="flex gap-2">
                <button onclick="escanearCamposSharePoint()" class="bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 text-xs font-bold rounded shadow transition">Escanear SP</button>
                <button onclick="testConexion()" class="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 text-xs font-bold rounded shadow transition">Test API</button>
            </div>
        </div>
...
<script>
    /* =====================================================================
       CHANGELOG V14 (sobre V13) - Mejoras de validacion Form <-> App
       - No se modifico la capa de conexion SharePoint (relativeSiteUrl,
         targetListName, getDigest, ejecutarTransaccion, testConexion,
         cargarTurnos: misma estructura REST/Digest que en V13).
       1) Fix: chequeo de fecha vacia en crearTurno/ejecutarReprogramacion
          (el bug de "T00:00:00Z" siempre truthy ya no puede pasar).
       2) Fix: al cambiar Planta Fisica o Fecha durante Alta/Reprogramacion,
          se mantiene el filtro de "solo horarios disponibles".
       3) Fix: guardia anti bucle-infinito en getSlotsCalculados() ante
          duraciones <= 0, + validacion atomica en Config. Horarios.
       4) Nuevo: validarFormularioTurno() centraliza validaciones de
          Fecha, Horario, CUIT (formato + padron + estado), Proveedor,
          Patente, Email y N OC, con marcado visual de campos invalidos.
       5) Nuevo: revalidarDisponibilidad() reduce la ventana de carrera
          (doble reserva de un mismo slot) re-sincronizando contra SP
          justo antes de Alta/Reprogramacion, usando cargarTurnos().
       6) Nuevo: ejecutarAccionFlujo() resincroniza antes de aplicar
          Aprobar/Cancelar para evitar pisar cambios de otro usuario.
       7) Fix: escapeHtml() aplicado en todo render via innerHTML para
          mitigar XSS desde campos de texto libre (Proveedor, Patente,
          OC, Email, Usuarios).
       ===================================================================== */
    ...
    // CONFIGURACION MAESTRA DE PLANTAS (Requisito Nuevo)
    let configPlantas = [
    ...
    // Modo actual del formulario ABM: null | 'ALTA' | 'VER' | 'APROBAR' | 'REPROGRAMAR'
    // Se usa para decidir si al cambiar Planta/Fecha se debe seguir
    // filtrando solo horarios disponibles (fix item 2 del changelog).
    let modoActualABM = null;

    // ---- FLAGS DE VALIDACION (ajustables segun reglas de negocio) ----
    const VALIDAR_CUIT_MOD11 = false;   // true = valida tambien el digito verificador del CUIT
    const PATENTE_OBLIGATORIA = true;   // false = la patente pasa a ser opcional en el Alta
    const EMAIL_OBLIGATORIO = false;    // true = exige email de contacto en el Alta

    // --- ARRANQUE OPTIMISTA ---
    function initApp() {
        try {
            logDebug("Inicializando Core ERP Logistico...");
            const hoy = new Date();
            // Correccion de Zona Horaria a Local para la fecha "hoy"
            ...
            // Evita seleccionar fechas pasadas en el formulario de Alta/Reprogramacion
            const fFecha = document.getElementById('f_fecha');
            if (fFecha) fFecha.min = fechaSeleccionadaGlobal;

            setTimeout(() => { testConexion(); }, 50);
            setTimeout(() => { refrescarDBAbsoluto(); }, 150); // Carga todo independiente de la fecha
    ...
    function guardarConfiguracionHorarios() {
        // Validacion previa de TODAS las plantas antes de aplicar nada
        // (todo-o-nada). Evita configuraciones inconsistentes que luego
        // provocarian bucles infinitos o agendas invertidas en
        // getSlotsCalculados().
        const nuevasConfigs = [];
        for (let idx = 0; idx < configPlantas.length; idx++) {
            const p = configPlantas[idx];
            const inlv = parseInt(document.getElementById(`cfg_inlv_${idx}`).value);
            const outlv = parseInt(document.getElementById(`cfg_outlv_${idx}`).value);
            const dur = parseInt(document.getElementById(`cfg_dur_${idx}`).value);
            const sab = document.getElementById(`cfg_sab_${idx}`).checked;
            const ins = parseInt(document.getElementById(`cfg_ins_${idx}`).value);
            const outs = parseInt(document.getElementById(`cfg_outs_${idx}`).value);

            if (isNaN(inlv) || isNaN(outlv) || inlv < 0 || inlv > 24 || outlv < 0 || outlv > 24 || inlv >= outlv) {
                showToast(`Ventana Lunes a Viernes invalida en "${p.id}" (Inicio debe ser menor a Fin, 0-24h).`, "err");
                return;
            }
            if (isNaN(dur) || dur <= 0) {
                showToast(`El "Tiempo x Turno" de "${p.id}" debe ser mayor a 0.`, "err");
                return;
            }
            if (sab && (isNaN(ins) || isNaN(outs) || ins < 0 || ins > 24 || outs < 0 || outs > 24 || ins >= outs)) {
                showToast(`Ventana de Sabado invalida en "${p.id}" (Inicio debe ser menor a Fin, 0-24h).`, "err");
                return;
            }

            nuevasConfigs.push({
                hInLV: inlv, hOutLV: outlv,
                sabado: sab, hInS: sab ? ins : 0, hOutS: sab ? outs : 0,
                duracion: dur
            });
        }

        nuevasConfigs.forEach((nc, idx) => Object.assign(configPlantas[idx], nc));
        showToast("Configuracion de Horarios Actualizada", "ok");
        regenerarHorariosABM(esModoFiltradoActivo()); // Refresca si hay un form abierto
        calcularDisponibilidadOffline(); // Refresca panel
    }
    ...
    // Calcula slots segun reglas dinamicas y turnos ocupados
    function getSlotsCalculados(plantaId, fechaStr) {
        ...
        if (day === 6 && !pConf.sabado) return slots; // Sabado cerrado

        let startMins = (day === 6 ? pConf.hInS : pConf.hInLV) * 60;
        let endMins = (day === 6 ? pConf.hOutS : pConf.hOutLV) * 60;
        let duration = pConf.duracion;

        // Guardia anti bucle-infinito: una duracion <= 0 o una ventana
        // horaria invertida/vacia harian que el while siguiente nunca
        // termine (current nunca alcanzaria endMins). Se devuelve "sin
        // operacion" en vez de colgar el navegador.
        if (!duration || duration <= 0 || endMins <= startMins) return slots;
    ...
    function regenerarHorariosABM(modoFiltroDisponibles = false) {
        ...
        const itemIdActual = document.getElementById('f_item_id').value;
        ...
        if(allSlots.length === 0) {
            select.innerHTML = '<option value="">Sin Operacion / Cerrado</option>';
            return;
        }

        // Si es alta o reprogramacion, cruzamos contra la BD para ver cuales estan ocupados.
        // En Reprogramacion, el propio turno (f_item_id) no debe contarse como
        // "ocupado" contra si mismo, para poder re-seleccionar su slot actual.
        let slotsOcupados = [];
        if (modoFiltroDisponibles) {
            slotsOcupados = listaTurnosCache
                .filter(t => t.field_6 && t.field_6.startsWith(fecha) && t.PlantaDescarga === pFisica
                    && String(t.field_7).toLowerCase() !== 'cancelado'
                    && (!itemIdActual || t.Id !== parseInt(itemIdActual)))
        ...
        allSlots.forEach(s => {
            ...
            let estaOcupado = slotsOcupados.includes(sNorm);
            if (modoFiltroDisponibles && estaOcupado) {
                // No lo agregamos, o lo mostramos deshabilitado. Requerimiento: "solo los horarios disponibles"
            } else {
    ...
    // --- LECTURA PRINCIPAL (REQUERIMIENTO: Carga TODO independiente de la fecha) ---
    ...
    async function cargarTurnos() {
        ...
        try {
            ...
            logETLStep(2, "Aplicando filtros de visualizacion...");
            
            poblarFiltrosEstaticos();
            aplicarFiltrosMonitor(); // Renderiza la tabla completa
            calcularDisponibilidadOffline(); // Renderiza paneles laterales
            
            logETLStep(2, "OK", true);
            logLog(`BD Sincronizada. ${listaTurnosCache.length} turnos analizados.`);
        } catch (err) {
            logLog(`[ERROR API]: ${err.message}`);
        }
    }
    ...
    function renderizarFilas(items) {
        ...
        if(!tbody) return;
        if(items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="8" class="p-6 text-center text-slate-500 text-xs">No hay turnos para los filtros seleccionados.</td></tr>`;
            return;
        }
        ...
        items.forEach(t => {
            ...
            let slot = escapeHtml(t.field_5 || '-');
            let estadoSeguro = escapeHtml(estado);
            ...
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-xs">
                ...
                <td class="p-2 text-amber-300 font-mono">${slot}</td>
                <td class="p-2 font-mono tracking-widest text-slate-300 bg-black/30 text-center rounded">${pat}</td>
                <td class="p-2 ${colorEst} text-center">${estadoSeguro}<br><span class="text-[9px] text-slate-500 font-normal truncate block max-w-[100px] mx-auto" title="Resp">${modPor}</span></td>
                <td class="p-2 text-center flex justify-center gap-1">
                    <button onclick="modoABM_Ver(${t.Id})" class="bg-sky-700 hover:bg-sky-600 text-white px-2 py-1 rounded transition text-[10px] uppercase font-bold">Ver</button>
                    ${!esAprobado && estado.toLowerCase() !== 'cancelado' ? `<button onclick="modoABM_Aprobar(${t.Id})" class="bg-emerald-700 hover:bg-emerald-600 text-white px-2 py-1 rounded transition text-[10px] uppercase font-bold">Aprobar</button>` : ''}
                </td>
            </tr>`;
        });
    ...
    // --- LOGICA DE ESTADOS ABM (VER / APROBAR / ALTA) ---
    function abrirAltaTurno() {
        limpiarFormularioABM();
        modoActualABM = 'ALTA';
        document.getElementById('f_fecha').value = fechaSeleccionadaGlobal;
        autoDia();
        regenerarHorariosABM(true); // Filtra los ocupados para que ofrezca solo libres
    ...
    function modoABM_Ver(id) {
        ...
        document.getElementById('form-mode-indicator').textContent = "CONSULTA DE TURNO";
        ...
        // El requerimiento dice: "Opcion de ver debe abrir... con todos los campos cargados" pero no aclara si editable. Dejamos readonly por seguridad visual, o editable parcial.
    ...
    function modoABM_Aprobar(id) {
        ...
        // Requisito: "sin la opcion de modificar ningun campo. Solo habilitados Aprobar | Cancelar | Reprogramar"
        habilitarCamposABM(false);
    ...
    function renderBotonesABM(modo) {
        ...
        let html = '';
        if (modo === 'ALTA') {
            html = `<button onclick="crearTurno()" class="bg-sky-600 hover:bg-sky-500 px-6 py-3 font-bold rounded text-white flex-1 transition">Registrar Alta</button>`;
        } else if (modo === 'VER') {
            html = `<button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded text-white flex-1 transition">Volver a Monitor</button>`;
        } else if (modo === 'APROBAR_PANEL') {
            html = `
                <button onclick="ejecutarAccionFlujo('Aprobado')" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-3 font-bold rounded text-white flex-1 transition">✅ Aprobar Turno</button>
                <button onclick="ejecutarAccionFlujo('Cancelado')" class="bg-rose-600 hover:bg-rose-500 px-4 py-3 font-bold rounded text-white flex-1 transition">❌ Cancelar</button>
                <button onclick="modoABM_Reprogramar()" class="bg-amber-600 hover:bg-amber-500 px-4 py-3 font-bold rounded text-white flex-1 transition">📅 Reprogramar</button>
            `;
        } else if (modo === 'REPROGRAMAR') {
            html = `
                <button onclick="ejecutarReprogramacion()" class="bg-indigo-600 hover:bg-indigo-500 px-6 py-3 font-bold rounded text-white flex-1 transition">Confirmar Reprogramacion</button>
                <button onclick="modoABM_Ver(document.getElementById('f_item_id').value)" class="bg-slate-700 hover:bg-slate-600 px-4 py-3 font-bold rounded text-white transition">Abortar</button>
            `;
        }
    ...
    function cargarTurnoEnFormulario(id) {
        ...
        // Regenera sin filtro para ver el slot original
    ...
    // =====================================================================
    // --- VALIDACION DE FORMULARIO DE TURNOS (Form <-> App) ---
    // =====================================================================
    ...
    // Devuelve true si, en el modo actual del formulario, los selects de
    // Planta Fisica / Fecha deben seguir filtrando solo horarios libres
    // al ser modificados por el usuario (fix item 2 del changelog).
    function esModoFiltradoActivo() {
        return modoActualABM === 'ALTA' || modoActualABM === 'REPROGRAMAR';
    }

    // Deja solo digitos (para comparar/validar CUIT sin importar guiones)
    function normalizarCUIT(valor) {
        return String(valor || '').replace(/[^0-9]/g, '');
    }

    // Valida formato de CUIT (11 digitos). Si VALIDAR_CUIT_MOD11 esta
    // activo, ademas verifica el digito verificador real.
    function validarFormatoCUIT(cuitNorm) {
        if (!/^\d{11}$/.test(cuitNorm)) return false;
        if (!VALIDAR_CUIT_MOD11) return true;

        const mults = [5, 4, 3, 2, 7, 6, 5, 4, 3, 2];
        let suma = 0;
        for (let i = 0; i < 10; i++) suma += parseInt(cuitNorm[i], 10) * mults[i];
        let resto = suma % 11;
        let dv = 11 - resto;
        if (dv === 11) dv = 0;
        if (dv === 10) dv = 9;
        return dv === parseInt(cuitNorm[10], 10);
    }

    // Patente Argentina: formato viejo (AAA111) o Mercosur (AA111AA)
    ...
    // Quita el marcado visual de error de todos los campos del formulario
    function limpiarErroresValidacion() {
        document.querySelectorAll('.f-control').forEach(el => el.classList.remove('border-rose-500', 'ring-1', 'ring-rose-500'));
    }

    // Valida el formulario de Turnos antes de Alta o Reprogramacion.
    // modo: 'ALTA' | 'REPROGRAMAR'
    // Devuelve true si es valido; si no, marca el/los campos invalidos
    // (borde rojo), muestra un toast con el primer error y hace foco
    // en el primer campo invalido.
    function validarFormularioTurno(modo) {
        ...
        const fechaVal = document.getElementById('f_fecha').value;
        if (!fechaVal) {
            marcarError('f_fecha', 'Debe seleccionar una fecha.');
        } else if (fechaVal < obtenerFechaHoyISO()) {
            marcarError('f_fecha', 'No se pueden registrar turnos en fechas pasadas.');
        }

        // --- HORARIO ---
        const horarioVal = document.getElementById('f_horario').value;
        if (!horarioVal) {
            marcarError('f_horario', 'Debe seleccionar un horario disponible para esa planta/fecha.');
        }
        ...
        if (modo === 'ALTA') {
            ...
            let provEncontrado = null;
            if (!cuit) {
                marcarError('f_cuit', 'Debe indicar el N de Proveedor / CUIT.');
            } else if (!validarFormatoCUIT(cuitNorm)) {
                marcarError('f_cuit', VALIDAR_CUIT_MOD11
                    ? 'El CUIT ingresado no es valido (verifique digito verificador).'
                    : 'El CUIT debe tener 11 digitos numericos.');
            } else if (listaProveedoresCache.length > 0) {
                provEncontrado = listaProveedoresCache.find(p => normalizarCUIT(p.id) === cuitNorm);
                if (!provEncontrado) {
                    marcarError('f_cuit', 'El proveedor no existe en el padron (Base Proveedores). Verifique el numero o cargue el alta.');
                } else if (provEncontrado.estado !== 'Habilitado') {
                    marcarError('f_cuit', `Proveedor INHABILITADO (${provEncontrado.nombre}). No se puede asignar turno.`);
                }
            }

            // --- RAZON SOCIAL ---
            if (!document.getElementById('f_proveedor').value.trim()) {
                marcarError('f_proveedor', 'Debe indicar la Razon Social del proveedor.');
            }

            // --- PATENTE ---
            const patente = document.getElementById('f_patente').value.trim().toUpperCase();
            if (!patente) {
                if (PATENTE_OBLIGATORIA) marcarError('f_patente', 'Debe indicar la patente del vehiculo.');
            } else if (!validarFormatoPatente(patente)) {
                marcarError('f_patente', 'Formato de patente invalido (Ej: AAA111 o AA111AA).');
            }

            // --- EMAIL (opcional salvo flag) ---
            const email = document.getElementById('f_email').value.trim();
            if (!email) {
                if (EMAIL_OBLIGATORIO) marcarError('f_email', 'Debe indicar un email de contacto.');
            } else if (!validarFormatoEmail(email)) {
                marcarError('f_email', 'El formato del email no es valido.');
            }

            // --- N OC (opcional, numerico) ---
            const oc = document.getElementById('f_oc').value.trim();
            if (oc && !/^[0-9]+$/.test(oc)) {
                marcarError('f_oc', 'El N de OC debe contener solo numeros.');
            }
        }

        if (errores.length > 0) {
            showToast(errores[0], "err");
    ...
    // Revalida disponibilidad de un slot contra SharePoint justo antes de
    // confirmar Alta/Reprogramacion, re-sincronizando listaTurnosCache via
    // cargarTurnos() (misma capa REST existente, sin nuevos endpoints).
    // Reduce -no elimina del todo- la ventana de doble reserva.
    async function revalidarDisponibilidad(plantaId, fechaStr, slotStr, excludeId = null) {
        try {
            await cargarTurnos();
        } catch (e) {
            // Si falla la resincronizacion, no bloqueamos el alta: se
            // sigue con la cache local (comportamiento equivalente a V13).
            logLog(`[WARN] No se pudo resincronizar antes de confirmar: ${e.message}`);
            return true;
        }

        const slotNorm = normalizarStringHorario(slotStr);
        const excludeIdNum = excludeId ? parseInt(excludeId) : null;

        const conflicto = listaTurnosCache.find(t =>
            t.field_6 && t.field_6.startsWith(fechaStr) &&
            t.PlantaDescarga === plantaId &&
            normalizarStringHorario(t.field_5) === slotNorm &&
            String(t.field_7).toLowerCase() !== 'cancelado' &&
            (excludeIdNum === null || t.Id !== excludeIdNum)
        );
        return !conflicto;
    }

    // --- TRANSACCIONES SP (CORE) ---
    async function ejecutarTransaccion(itemId, payload, msg, skipRefresh = false) {
        logETLStep(3, `Iniciando POST/MERGE para Item: ${itemId || 'NUEVO'}`);
        try {
            ...
            if (!res.ok && res.status !== 204 && res.status !== 201) throw new Error("Falla API HTTP " + res.status);
            
            logETLStep(3, "Carga Exitosa", true);
            showToast(msg, "ok");
            generarLogArchivoSP("Transaccion_Turno", msg); // Auditoria
            
            if(!skipRefresh) await cargarTurnos(); 
        } catch(e) {
            logLog(`[ERROR ESCRITURA SP]: ${e.message}`);
            // Mock local por supervivencia
            if(itemId) {
                let obj = listaTurnosCache.find(x=>x.Id==itemId);
                if(obj) Object.assign(obj, payload);
            }
            showToast(msg + " (Mock Local)", "ok");
            aplicarFiltrosMonitor();
            calcularDisponibilidadOffline();
        }
    }

    // --- ACCIONES ESPECIFICAS DEL FLUJO ---
    async function crearTurno() {
        ...
        // Revalida contra SP justo antes de confirmar (mitiga doble reserva)
        const slotLibre = await revalidarDisponibilidad(pFisica, fecha, horario);
        if (!slotLibre) {
            showToast("Ese horario fue tomado por otro usuario mientras completaba el formulario. Elija otro.", "err");
            regenerarHorariosABM(true);
            return;
        }
    ...
    async function ejecutarAccionFlujo(nuevoEstado) {
        ...
        // Resincroniza contra SP antes de aplicar la transicion de estado,
        // para no pisar un cambio que ya hizo otro usuario (ej. ya
        // Aprobado/Cancelado desde otra sesion).
        try { await cargarTurnos(); } catch (e) { /* seguimos con cache local si falla */ }

        const turnoActual = listaTurnosCache.find(t => t.Id === parseInt(id));
        if (turnoActual) {
            const estadoActual = String(turnoActual.field_7 || '').toLowerCase().trim();
            if (estadoActual === 'cancelado' || estadoActual === 'aprobado') {
                showToast(`Este turno ya fue actualizado a "${turnoActual.field_7}" por otro usuario.`, "err");
                volverTurnosDiarios();
                return;
            }
        }

        const prov = document.getElementById('f_proveedor').value;
        ...
        // Gatillo Mail Simulado (Abrir cliente)
        if (email) {
    ...
    async function ejecutarReprogramacion() {
        ...
        // Revalida contra SP justo antes de confirmar, excluyendo el
        // propio turno (que sigue figurando como "ocupado" hasta que
        // se confirme la reprogramacion).
        const slotLibre = await revalidarDisponibilidad(pFisica, nfec, nhor, id);
        if (!slotLibre) {
            showToast("Ese horario fue tomado por otro usuario mientras reprogramaba. Elija otro.", "err");
            regenerarHorariosABM(true);
            return;
        }

        const payload = {
            field_6: nfec + "T00:00:00Z",
            field_5: nhor,
            field_7: "Reprogramado"
        };
        await ejecutarTransaccion(id, payload, "Turno Reprogramado");
        volverTurnosDiarios();
    }
    ...
    async function marcarTrackingForm(tipo) {
        ...
        await ejecutarTransaccion(id, payload, `Check-${tipo} Guardado`, true); // Skip refresh to avoid UI jump
    }

    // --- FILTROS TABLA MONITOR ---
    function poblarFiltrosEstaticos() {
        ...
        if (selectP && selectP.options.length <= 1) {
            // Ya es cargado estaticamente por config, pero si queremos dynamically by DB:
    ...
    function aplicarFiltrosMonitor() {
        // La grilla principal muestra TODOS los turnos (Requerimiento)
        // PERO el calendario filtra el Panel de Disponibilidad.
        // Espera, el requerimiento 4: "Cuando se de Refresh debe recargar todos independientemente del filtro de disponibilidad".
        // Entonces la Tabla filtra por Planta y Modificador, pero NO por fecha por defecto.
    ...
    function getResponsableName(t) {
        return t.Author ? t.Author.Title : (t.Editor ? t.Editor.Title : (t.Gestor || 'Desconocido'));
    }

    // --- CALENDARIO Y DISPONIBILIDAD (PANEL LATERAL MÚLTIPLE) ---
    ...
    function calcularDisponibilidadOffline() {
        ...
        configPlantas.forEach(plantaConfig => {
            // Requerimiento: "En el filtro de Planta Fisica cuando selecciono la planta debe adecuarse la disponibilidad conforme a esta"
            ...
            if (slotsTotales.length > 0) {
                ...
                slotsTotales.forEach(s => {
                    ...
                    if (ocupado) {
                        ...
                        htmlPanel += `<div class="flex justify-between p-1.5 border border-red-900 bg-red-950/30 rounded text-[9px] font-mono text-rose-300 shadow">
                            <span>${s.split(' ')[0]}</span><span class="font-bold tracking-wider">${patente}</span>
                        </div>`;
                    } else {
                        htmlPanel += `<div class="flex justify-between p-1.5 border border-emerald-900 bg-emerald-950/30 rounded text-[9px] font-mono text-emerald-400 shadow opacity-70">
                            <span>${s.split(' ')[0]}</span><span class="font-bold tracking-wider opacity-50">LIBRE</span>
                        </div>`;
                    }
                });
                htmlPanel += `</div></div>`;
        ...
        if (htmlPanel === '') htmlPanel = '<div class="text-center text-slate-500 text-xs mt-4">Sin turnos configurados.</div>';
    ...
    function procesarCSVProveedores() {
        ...
        r.onload = function(e) {
            try {
                ...
                showToast(`Padrón Importado: ${listaProveedoresCache.length} regs`, "ok");
    ...
    function renderizarBaseProveedores() {
        ...
        tb.innerHTML = listaProveedoresCache.map((p, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-xs cursor-pointer" onclick="cargarProvForm(${idx})">
                ...
                <td class="p-2 ${p.estado === 'Habilitado' ? 'text-emerald-400' : 'text-rose-400'}">${p.estado}</td>
    ...
    function cargarProvForm(idx) {
        ...
        generarLogArchivoSP("Acceso_Prov", `Consulta de ABM Prov: ${p.nombre}`);
    }
    ...
    function autocompletarProveedor() {
        ...
        if(provEncontrado && provEncontrado.estado === 'Habilitado') {
            ...
            // Inferir area (Packaging o Insumos)
        ...
        steps.forEach((s, idx) => {
            if(idx === stepNum-1) {
                ...
                if(isDone) {
                    ...
                    document.getElementById(ic[idx]).innerHTML = '✔';
                } else {
    ...
    function logLog(msg) {
        ...
        const db = document.getElementById('debug-log');
        ...
        if(db) { db.insertAdjacentHTML('beforeend', txt); db.scrollTop = db.scrollHeight; }
        console.log(msg);
    }

    // Simula crear log en SP Documents
    async function generarLogArchivoSP(modulo, detalle) {
        const msjLog = `Modulo: ${modulo} | Accion: ${detalle} | Usuario: ${currentLoggedUser.name}`;
        logLog(`Audit Log Request -> ${msjLog}`);
        // Logica para _api/web/GetFolderByServerRelativeUrl('Shared Documents')/Files/add iria aqui.
    }

    // --- UTILIDADES ---
    // Escapa caracteres especiales de HTML. Se aplica a todo dato de
    // texto libre (proveedor, patente, OC, email, usuarios, etc.) antes
    // de insertarlo via innerHTML, para mitigar XSS (fix item 7).
    ...
    function procesarCSVUsuarios() {
        ...
        r.onload = function(e) {
            ...
            document.getElementById('tabla-usuarios').innerHTML = listaUsuariosCache.map(u => `<tr class="border-b border-slate-700 text-xs"><td class="p-2">${u.usuario}</td><td class="p-2">${u.nombre}</td><td class="p-2">${u.email}</td><td class="p-2">${u.planta}</td></tr>`).join('');
