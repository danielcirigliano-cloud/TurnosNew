        ...
        configPlantas.forEach(plantaConfig => {
            ...
            if (slotsTotales.length > 0) {
                ...
                slotsTotales.forEach(s => {
                    ...
                    if (ocupado) { let estado = String(ocupado.field_7||'').toLowerCase(); let claseBg='bg-red-950/30 border-red-900'; if(estado.includes('pendiente')) claseBg='bg-yellow-900 border-yellow-500'; rounded text-sm font-mono shadow-md">
...
// ====================== MOLCA PRO EXTENSIONS ======================

// --- CONFIG CUPOS DINAMICOS ---
const limitesCupo = {
    "Parque Canuelas": { "Insumos":3, "Packaging":5, "Materia Prima":2 },
    "Predio Canuelas": { "Insumos":4, "Packaging":6, "Materia Prima":3 }
};

function proveedorSinCupoDinamico(cuit) {
    const planta = document.getElementById('f_planta_descarga')?.value;
    const tipo = document.getElementById('f_planta')?.value;
    const fecha = document.getElementById('f_fecha')?.value;

    const limite = limitesCupo[planta]?.[tipo] || 3;

    const turnos = listaTurnosCache.filter(t =>
        t.field_1 === cuit &&
        t.PlantaDescarga === planta &&
        t.Planta === tipo &&
        t.field_6?.startsWith(fecha)
    );

    return turnos.length >= limite;
}

// --- SEMAFORO ---
function calcularSemaforo(planta, fecha) {
    const total = getSlotsCalculados(planta, fecha).length;
    const ocupados = listaTurnosCache.filter(t =>
        t.PlantaDescarga === planta &&
        t.field_6?.startsWith(fecha)
    ).length;

    const ratio = ocupados/total;
...
// --- SCORE ---
function calcularScoreProveedor(cuit) {
    const turnos = listaTurnosCache.filter(t => t.field_1 === cuit);
    if (turnos.length === 0) return 100;

    const cancelados = turnos.filter(t => String(t.field_7).toLowerCase().includes('cancelado')).length;
    return Math.max(0, 100 - cancelados * 10);
}

// --- PREDICCION DEMORA ---
function predecirDemora(planta, fecha, slot) {
    const sem = calcularSemaforo(planta, fecha);
    let base = sem === "ROJO" ? 25 : sem === "AMARILLO" ? 10 : 0;
    return base;
}

// --- AUTO ASIGNADOR ---
async function autoAsignarHorarioPro() {
    const planta = document.getElementById('f_planta_descarga')?.value;
    const fecha = document.getElementById('f_fecha')?.value;
    const cuit = document.getElementById('f_cuit')?.value;
    
    let slots = getSlotsCalculados(planta, fecha);
    let mejor = null;
    let mejorScore = -9999;

    slots.forEach(slot => {
        const ocupado = listaTurnosCache.find(t => t.field_5 === slot);
        if (ocupado) return;

        let score = 100;
        score -= predecirDemora(planta, fecha, slot);
        score += calcularScoreProveedor(cuit);
    ...
    if(mejor) document.getElementById('f_horario').value = mejor;
}

// --- REPROGRAMACION AUTOMATICA ---
function reprogramarTurnosAutomaticoPro() {
    const fecha = fechaSeleccionadaGlobal;
    configPlantas.forEach(p => {
        if(calcularSemaforo(p.id, fecha)!=="ROJO") return;

        listaTurnosCache.forEach(t => {
            if(t.PlantaDescarga===p.id && t.field_7==='Pendiente') {
                t.field_5 = getSlotsCalculados(p.id, fecha)[0];
                t.field_7 = "Reprogramado";
            }
        });
    });
}
...
// --- PLANIFICACION 6AM ---
function planificacion6AM() {
    if(new Date().getHours() !== 6) return;

    configPlantas.forEach(p => {
        const slots = getSlotsCalculados(p.id, fechaSeleccionadaGlobal);
        slots.forEach((s,i)=>{
            listaTurnosCache.push({Id:'AUTO_'+i, PlantaDescarga:p.id, field_5:s, field_7:'PreAsignado'});
        });
    });
}

setInterval(planificacion6AM,60000);

// --- DASHBOARD CONGESTION ---
function renderCongestionPro() {
    const ctx = document.getElementById('chartCongestion');
    if(!ctx) return;

    let labels = configPlantas.map(p=>p.id);
    let data = labels.map(p => {
        const sem = calcularSemaforo(p, fechaSeleccionadaGlobal);
        return sem==="ROJO"?100:sem==="AMARILLO"?70:30;
    });

    new Chart(ctx,{type:'bar',data:{labels,datasets:[{label:'% Congestion',data}]}});
}
