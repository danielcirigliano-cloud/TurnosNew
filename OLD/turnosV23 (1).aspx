...
<script>
    ...
    // FIX V23: targetListName ya NO se muta dentro de fetchSharePoint en
    // caliente (eso hacia que un unico 404 transitorio, de cualquier
    // llamada concurrente, dejara fijo para siempre un nombre de lista
    // potencialmente incorrecto, afectando a todas las transacciones
    // posteriores de todos los usuarios en esa sesion).
    //
    // En su lugar, el nombre real se resuelve UNA SOLA VEZ al arranque
    // (resolverNombreListaTurnos, llamado desde initApp) contra un
    // endpoint de solo lectura ($top=0). Una vez resuelto, targetListName
    // queda fijo durante el resto de la sesion.
    //
    // Si por algun motivo la resolucion de arranque no llego a ejecutarse
    // (falla de red puntual al iniciar), fetchSharePoint conserva un
    // fallback de emergencia, pero ahora es LOCAL a cada llamada: prueba
    // el segundo nombre solo para esa peticion puntual, sin pisar la
    // variable global ni afectar al resto de la app.
    async function resolverNombreListaTurnos() {
        for (const nombre of NOMBRES_LISTA_POSIBLES) {
            try {
                ...
                if (res.ok) {
                    targetListName = nombre;
                    logLog(`[ETL] Lista de Turnos resuelta: "${nombre}"`);
                    return;
                }
    ...
    async function fetchSharePoint(query, options = {}) {
        ...
        if (res.status === 404) {
            // Fallback de emergencia LOCAL: no muta targetListName global,
            // solo reintenta esta llamada puntual con el otro nombre posible.
    ...
    function aplicarFiltrosMonitor() {
        ...
        // Base completa del dia, ya ordenada estructuralmente por
        // Planta -> Fecha de Solicitud -> Estado. Los filtros de planta y
        // responsable se aplican DESPUES, sobre esa base ordenada, para
        // que el orden se mantenga sin importar que filtro este activo.
        let base = ordenarTurnosMonitor(
            listaTurnosCache.filter(t => t.field_6 && String(t.field_6).split('T')[0] === fechaSeleccionadaGlobal)
        );
    ...
    // Orden de prioridad operativa de Estados: lo que requiere accion
    // primero (Pendiente), luego lo confirmado (Aprobado), luego lo
    // reprogramado, y al final lo cerrado (Cancelado/Rechazado).
    const ORDEN_ESTADOS_MONITOR = { 'pendiente': 0, 'aprobado': 1, 'reprogramado': 2, 'cancelado': 3, 'rechazado': 3 };

    // Ordena turnos por Planta (alfabetico) -> Fecha de Solicitud
    // (cronologico) -> Estado (segun ORDEN_ESTADOS_MONITOR). No muta el
    // arreglo recibido; devuelve una copia ordenada.
    function ordenarTurnosMonitor(items) {
        return [...items].sort((a, b) => {
            const plantaA = String(a.PlantaDescarga || '').toLowerCase();
            const plantaB = String(b.PlantaDescarga || '').toLowerCase();
            if (plantaA !== plantaB) return plantaA.localeCompare(plantaB);

            const fecA = String(a.field_6 || '');
            const fecB = String(b.field_6 || '');
            if (fecA !== fecB) return fecA.localeCompare(fecB);

            const estA = ORDEN_ESTADOS_MONITOR[String(a.field_7 || 'pendiente').toLowerCase().trim()] ?? 1;
            const estB = ORDEN_ESTADOS_MONITOR[String(b.field_7 || 'pendiente').toLowerCase().trim()] ?? 1;
            return estA - estB;
        });
    }
    ...
    async function cargarTurnos() {
        ...
        try {
            ...
            if(document.getElementById('s10').classList.contains('active')) updateDashboard(); // Refresca dash si esta activo
        } catch (err) { 
    ...
    function renderizarFilas(items) {
        ...
        items.forEach(t => {
            ...
            let colorEst = esAprobado ? 'text-emerald-400 font-bold' : (estado.toLowerCase() === 'cancelado' ? 'text-rose-400' : (estado.toLowerCase() === 'reprogramado' ? 'text-sky-400 font-bold' : 'text-amber-400'));
    ...
    function modoABM_Reprogramar() {
        ...
        document.getElementById('abm-mensaje-estado').className = "hidden"; // Oculta el banner
    ...
    async function ejecutarAccionFlujo(nuevoEstado) {
        ...
        if (email) {
            const responsable = t ? getResponsableName(t) : currentLoggedUser.name;
    ...
    function cargarSelectoresDashboard() {
        // Carga selectores unicos para el dashboard
    ...
    function updateDashboard() {
        ...
        // Setup Options globales Chart.js
        Chart.defaults.color = '#94a3b8';
        Chart.defaults.font.family = 'Syne';

        // Grafico 1: Estados Globales
        ...
        // Grafico 2: Proveedores (Top Ranking)
