...
<main class="flex-1 p-8 overflow-y-auto relative">
    ...
    <section id="s1" class="tab-content mt-10">
        ...
        <div class="glass p-6 rounded-xl max-w-4xl grid grid-cols-2 gap-4 text-base relative">
            ...
            <div id="warning-slot" class="col-span-2 hidden mt-2 p-3 bg-red-950/40 border border-red-500/50 rounded-lg text-red-400 text-sm font-bold flex items-center gap-3 transition-all">
                <span class="text-xl">⚠️</span> 
                <span>Atencion: El modulo horario seleccionado ya se encuentra Ocupado/Aprobado para esta planta.</span>
            </div>
    ...
    <section id="s7" class="tab-content mt-10">
        ...
        <div class="glass p-6 rounded-xl max-w-4xl space-y-6">
            <p class="text-sm text-slate-400">
                Selecciona un archivo Excel <code class="text-sky-400 font-mono">.xlsx</code> para importar de manera directa el pool logistico. El sistema mapeara y sanitizara las columnas de forma automatica:
            </p>
...
<script>
    ...
    let proveedoresImportadosExcel = []; 
    const slotsHorarios = [];
    ...
    function initApp() {
        try {
            ...
            document.getElementById('f_fecha')?.addEventListener('change', validarSlotEnTiempoReal);
            document.getElementById('f_planta')?.addEventListener('change', validarSlotEnTiempoReal);
            document.getElementById('f_horario')?.addEventListener('change', validarSlotEnTiempoReal);
    ...
    function renderizarFilas(items) {
        ...
        if(!tbody) return;
        if(items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="8" class="p-4 text-center text-slate-500 text-xs">No se encontraron turnos logisticos.</td></tr>`;
            return;
        }
    ...
    function validarSlotEnTiempoReal() {
        ...
        if (slotOcupado) {
            ...
            btnAlta.classList.add('opacity-40', 'cursor-not-allowed');
        } else {
            warnBox.classList.add('hidden');
            warnBox.classList.remove('flex');
            btnAlta.disabled = false;
            btnAlta.classList.remove('opacity-40', 'cursor-not-allowed');
        }
    }
    ...
    function procesarArchivoExcel(e) {
        ...
        reader.onload = function(evt) {
            try {
                ...
                if (!worksheet) {
                    showToast("Error: No se pudo leer la estructura del archivo", "err");
                    return;
                }
                ...
                if (!rows || rows.length <= 1) {
                    showToast("El archivo de proveedores esta vacio o tiene un formato invalido", "err");
                    return;
                }
