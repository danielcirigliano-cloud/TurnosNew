...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s1" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-8 rounded-xl w-full grid grid-cols-2 gap-6">
            ...
            <div class="col-span-2 mt-4 border-t border-slate-700 pt-4 flex justify-end items-center" id="panel-tracking">
...
<script>
    ...
    async function actualizarCSVSharePoint(tipo) {
        ...
        try {
            ...
            if (tipo === 'usu') {
                ...
                csvContent += "Usuario;Responsable;Mail Responsable;Tipo Insumo;Num_Proveedor;CUIT;Nombre proveedor;Estado\n";
                listaProveedoresCache.forEach(p => { csvContent += `${p.usuario};${p.responsable};${p.mail};${p.tipo};${p.id};${p.cuit};${p.nombre};${p.estado}\n`; });
            }
    ...
    function aplicarFiltrosMonitor() {
        ...
        // Punto 5: la grilla principal se recorta segun la fecha
        // seleccionada en el calendario (fechaSeleccionadaGlobal), igual
        // que ya hacian el resumen vertical y la disponibilidad logistica.
        ...
        if(tt !== 'TODOS') {
            if(tt === 'Sobreturno') {
                filtrados = filtrados.filter(t => t.EsSobreturno === true || t.EsSobreturno === 'true' || t.EsSobreturno === 1);
            } else {
                filtrados = filtrados.filter(t => !(t.EsSobreturno === true || t.EsSobreturno === 'true' || t.EsSobreturno === 1));
            }
        }
    ...
    function renderBotonesABM(modo) {
        ...
        else if (modo === 'VER') panel.innerHTML = `
        ...
        else if (modo === 'MODIFICAR') panel.innerHTML = `
    ...
    function cargarTurnoEnFormulario(id) {
        ...
        document.getElementById('f_patente').value = t.Patente || ''; document.getElementById('f_hora_ing').value = t.HoraIngresoReal || '';
        document.getElementById('f_hora_sal').value = t.HoraSalidaReal || '';
        document.getElementById('f_cuit').value = t.field_1 || ''; document.getElementById('f_oc').value = t.N_x00b0__x0020_de_x0020_OC || ''; document.getElementById('f_proveedor').value = t.field_2 || '';
        ...
        autoDia(); document.getElementById('f_planta').value = t.Planta || 'Packaging'; document.getElementById('f_planta_descarga').value = t.PlantaDescarga ||
        'Parque Canuelas';

        // Bug fix: el checkbox de Sobreturno no se estaba cargando al ver/
        // editar un turno existente, quedando siempre desmarcado.
        ...
        // Los botones In/Out de tracking ahora estan en la grilla del monitor (renderizarFilas)
        // y no en el formulario ABM, por lo que no hay elementos btn-track-in/out que manejar aqui.
    }
    ...
    async function marcarTracking(itemId, tipo) {
        ...
        // Actualiza cache local inmediatamente para que el render inline
        // muestre la hora sin esperar la resincronizacion completa de SP.
        const obj = listaTurnosCache.find(x => x.Id === parseInt(itemId));
        if (obj) Object.assign(obj, payload);
        aplicarFiltrosMonitor(); // Re-renderiza la fila con la hora visible
        ...
        for(let i=1; i<lines.length; i++) {
            ...
            if(cols.length === 7) {
                ...
                listaProveedoresCache.push({ 
                    ...
                    tipo: cols[3].trim(),
                    id: cols[4].replace(/[^0-9]/g, ''), 
                    cuit: cols[5].replace(/[^0-9\-]/g, ''),
                    nombre: cols[6].trim(), 
                    estado: cols[7].trim() || 'Habilitado' 
                });
            }
        }
    ...
    function autocompletarProveedor() {
        ...
        if (!provEncontrado) {
            if (listaProveedoresCache.length > 0) marcarError('f_cuit', "Proveedor no encontrado en el padron.");
            return;
        }
        ...
        // Email de contacto desde el campo mail del responsable del proveedor
        if (provEncontrado.mail) document.getElementById('f_email').value = provEncontrado.mail;

        // Zona de Descarga (Tipo Insumo del proveedor)
        ...
        // Si el tipo no coincide con ninguno se deja sin seleccion (no forzar valor)

        // Planta Fisica: no se preselecciona (el usuario elige segun la operacion)
        // para evitar asignaciones automaticas incorrectas entre plantas.

        // Responsable del Turno = usuario conectado (no depende del proveedor)
        const nombreResponsable = currentLoggedUser.name || 'S/D';
        document.getElementById('f_usuario_gestor').textContent = escapeHtml(nombreResponsable);
