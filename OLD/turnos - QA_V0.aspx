...
<aside class="w-64 glass flex flex-col p-4 border-r h-full z-10" style="border-color: var(--sp-border)">
    <h1 class="text-xl font-bold text-white mb-1">Auto<span style="color:var(--sp-blue)">Turnos</span> M365</h1>
...
<main class="flex-1 p-8 overflow-y-auto">
    ...
    <section id="s2" class="tab-content">
        <h2 class="text-2xl font-bold mb-4" style="color:var(--sp-blue)">Monitor de SharePoint en Linea</h2>
        <div class="glass p-6 rounded-xl">
            <div class="flex justify-between mb-4 items-center">
                <span class="text-xs text-slate-400">Lista conectada: <strong>Turnos Proveedores</strong></span>
                <button onclick="cargarTurnos()" class="bg-slate-700 px-4 py-2 rounded text-xs font-bold text-white hover:bg-slate-600">↻ Refrescar Datos</button>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-300">
                    ...
                    <tbody id="tabla-turnos" class="divide-y divide-slate-700/50">
                        <tr><td colspan="6" class="p-6 text-center text-slate-500 text-xs">Aguardando conexion con la lista...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <section id="s5" class="tab-content">
        ...
        <div class="glass p-4 rounded-xl">
            <div id="sys-logs" class="mono bg-black/80 h-96 overflow-y-auto p-4 rounded text-xs border border-slate-700" style="color:#4ade80"></div>
        </div>
    </section>
...
// Armado del objeto JSON con los nombres internos EXACTOS mapeados
        const payload = {
            ...
            // "field_4": dia,   <--- COMENTAMOS O BORRAMOS ESTA LÍNEA TEMPORALMENTE
            "field_5": horario,                        
            "field_7": "Pendiente"                     
        };
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Nuevo Capturador de Error de SharePoint
    ...
    async function cargarTurnos() {
        ...
        tbody.innerHTML = `<tr><td colspan="6" class="p-4 text-center text-xs">Cargando datos desde M365... <div class="spin ml-2"></div></td></tr>`;
        
        try {
            const endpoint = `${relativeSiteUrl}/_api/web/lists/getbytitle('${targetListName}')/items?$select=Id,Title,field_1,field_2,N_x00b0__x0020_de_x0020_OC,field_6,field_5,field_7&$orderby=Created desc&$top=50`;
            ...
            if(items.length === 0) {
                tbody.innerHTML = `<tr><td colspan="6" class="p-4 text-center text-slate-500 text-xs">La lista esta vacia.</td></tr>`;
                return;
            }

            tbody.innerHTML = items.map(t => {
                const statusColor = t.field_7 === 'Pendiente' ? 'text-amber-400' : 'text-emerald-400';
                return `
                <tr class="border-b border-slate-700/50 hover:bg-slate-800/30">
                    <td class="py-3 px-3 font-bold text-slate-400">${t.Title || '-'}</td>
                    <td class="px-3 text-white">${t.field_2 || '-'}</td>
            ...
            sysLog(`Sincronizacion exitosa. ${items.length} filas renderizadas.`, "INFO");
        } catch (err) {
            tbody.innerHTML = `<tr><td colspan="6" class="p-4 text-center text-red-400 text-xs font-bold">Error conectando con la API de SharePoint. Ver Log.</td></tr>`;
