...
<main class="flex-1 p-6 overflow-y-auto relative">
    ...
    <section id="s2" class="tab-content active mt-8">
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex gap-2">
                ...
                <button onclick="enviarResumenDiario()" class="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 text-xs font-bold rounded shadow transition">Enviar Resumen a Usuarios</button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-7 gap-4">
            <div class="col-span-4 glass p-4 rounded-xl h-[720px] flex flex-col">
                <div class="flex gap-2 mb-4 items-end bg-slate-900/50 p-3 rounded-lg border border-slate-700">
                    <div>
                        ...
                        <select id="filtro_planta_descarga" onchange="sincronizarFiltrosYMonitor()" class="bg-slate-950 border border-slate-700 p-1.5 text-xs text-white rounded outline-none w-32">
                            <option value="TODAS">-- Todas --</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-[10px] uppercase text-slate-400 font-bold mb-1">Responsable</label>
                        <select id="filtro_usuario" onchange="aplicarFiltrosMonitor()" class="bg-slate-950 border border-slate-700 p-1.5 text-xs text-white rounded outline-none w-32">
            ...
            <div class="col-span-3 glass p-4 rounded-xl flex flex-col h-[720px] overflow-hidden">
                ...
                <div class="bg-slate-950/80 p-2 rounded border border-slate-800 mb-2 text-xs">
                    <div class="flex justify-between items-center mb-1 px-1">
                        ...
                        <div class="text-[9px] text-slate-400 font-semibold font-mono">SELECCION DE FECHA</div>
                    </div>
    ...
    <section id="s6" class="tab-content mt-8">
        ...
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('prov')" class="bg-slate-700 hover:bg-slate-600 text-white px-4 py-1.5 font-bold rounded text-xs transition border border-slate-600">📥 Importar desde SharePoint</button>
        </div>
    ...
    <section id="s7" class="tab-content mt-8">
        ...
        <div class="glass p-4 rounded mb-6 border border-slate-700 flex gap-3 items-end max-w-5xl text-xs">
            ...
            <div class="w-1/6">
                ...
                <input type="text" id="u_id" class="w-full bg-slate-900 border border-slate-700 p-2 text-amber-300 font-bold rounded outline-none">
            </div>
            <div class="w-2/6">
                <label class="block uppercase text-slate-400 font-bold mb-1">Nombre (Responsable)</label>
                <input type="text" id="u_nombre" class="w-full bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none">
            </div>
            <div class="w-2/6">
                <label class="block uppercase text-slate-400 font-bold mb-1">Email</label>
                <input type="text" id="u_email" class="w-full bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none">
            </div>
            <div class="w-1/6">
                <label class="block uppercase text-slate-400 font-bold mb-1">Planta</label>
                <input type="text" id="u_planta" class="w-full bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none">
            </div>
            <div class="w-1/6">
                <label class="block uppercase text-slate-400 font-bold mb-1">Estado</label>
                <select id="u_estado" class="w-full bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none">
                    <option value="Habilitado">Habilitado</option>
                    <option value="Deshabilitado">No Habilitado</option>
                </select>
            </div>
            <div>
                <button onclick="guardarEdicionUsuario()" class="bg-sky-600 hover:bg-sky-500 text-white px-4 py-2 font-bold rounded transition">Guardar</button>
            </div>
        </div>
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('usu')" class="bg-slate-700 hover:bg-slate-600 text-white px-4 py-1.5 font-bold rounded text-xs transition border border-slate-600">📥 Importar desde SharePoint</button>
        </div>
        <div class="glass p-4 rounded-xl overflow-x-auto max-w-5xl">
            <div class="overflow-x-auto h-96">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-[10px] uppercase text-slate-400 font-bold">
                        <tr><th class="py-3 px-3">ID</th><th class="px-3">Nombre</th><th class="px-3">Email</th><th class="px-3">Planta</th><th class="px-3">Estado</th><th class="px-3">Accion</th></tr>
                    </thead>
                    <tbody id="tabla-usuarios">
                        <tr><td colspan="6" class="p-4 text-center text-slate-500 text-xs">Sin datos.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="max-w-5xl grid grid-cols-2 gap-6">
            <div class="glass p-6 rounded-xl">
                ...
                <div class="flex flex-col gap-4 relative">
                    ...
                    <div id="etl-extract" class="etl-step flex items-center gap-4 z-10 bg-slate-900 p-3 rounded border border-slate-700"><div class="w-8 h-8 rounded-full bg-slate-800 border-2 border-slate-500 flex items-center justify-center font-bold text-xs" id="etl-ic1">1</div><div><div class="font-bold text-sky-400 text-sm">Extraccion (E)</div><div class="text-xs text-slate-400">Lectura OData API REST.</div></div></div>
                    <div id="etl-transform" class="etl-step flex items-center gap-4 z-10 bg-slate-900 p-3 rounded border border-slate-700"><div class="w-8 h-8 rounded-full bg-slate-800 border-2 border-slate-500 flex items-center justify-center font-bold text-xs" id="etl-ic2">2</div><div><div class="font-bold text-amber-400 text-sm">Transformacion (T)</div><div class="text-xs text-slate-400">Validacion Defensiva.</div></div></div>
                    <div id="etl-load" class="etl-step flex items-center gap-4 z-10 bg-slate-900 p-3 rounded border border-slate-700"><div class="w-8 h-8 rounded-full bg-slate-800 border-2 border-slate-500 flex items-center justify-center font-bold text-xs" id="etl-ic3">3</div><div><div class="font-bold text-emerald-400 text-sm">Carga (L)</div><div class="text-xs text-slate-400">POST/MERGE a SharePoint.</div></div></div>
                </div>
            </div>
            <div class="glass p-6 rounded-xl flex flex-col h-[400px]">
                <h3 class="text-sm font-bold text-slate-300 uppercase mb-2 border-b border-slate-700 pb-2">Archivos de Log</h3>
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-3 font-mono text-[10px] overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit...</div></div>
            </div>
        </div>
    </section>
...
<script>
    ...
    async function importarCSVSharePoint(tipo) {
        // Usa la ruta interna asumiendo que "Documentos compartidos" es la libreria por defecto
        const url1 = `${relativeSiteUrl}/Documentos compartidos/${tipo === 'prov' ? 'proveedores.csv' : 'Usuarios.csv'}`;
        const url2 = `${relativeSiteUrl}/Shared Documents/${tipo === 'prov' ? 'proveedores.csv' : 'Usuarios.csv'}`;
        
        try {
            let res = await fetch(url1);
            if (!res.ok) res = await fetch(url2);
            if (!res.ok) throw new Error("Archivo no encontrado en SharePoint.");
            
            const text = await res.text();
            if(tipo === 'prov') parsearCSVProveedores(text);
            else parsearCSVUsuarios(text);
            
            showToast("Importado correctamente de SP", "ok");
        } catch (err) {
            logLog(`[ERROR CSV SP]: ${err.message}`);
            showToast("Error importando archivo SP", "err");
        }
    }
    ...
    function sincronizarFiltrosYMonitor() {
        ...
        // Relacionar planta con usuarios habilitados
        let usuariosValidos = listaUsuariosCache.filter(u => u.estado !== 'Deshabilitado');
        if (plantaSelect !== 'TODAS') {
            // Asume que en CSV Usuarios hay una columna planta que coincida (o muestra todos si no coincide exacto, pero prioriza)
        ...
        if (selectU) {
            // Extraer nombres unicos
            const usr = [...new Set(usuariosValidos.map(u => u.nombre).filter(Boolean))].sort();
            // Agregar responsables historicos que ya esten en los turnos
    ...
    function aplicarFiltrosMonitor() {
        ...
        calcularDisponibilidadOffline(); // Sincroniza el panel de disponibilidad
    }

    function restablecerFiltros() {
        document.getElementById('filtro_planta_descarga').value = 'TODAS';
        sincronizarFiltrosYMonitor(); // Resetea usuario a TODOS
    }
    ...
    async function cargarTurnos() {
        ...
        try {
            ...
            sincronizarFiltrosYMonitor(); // Refresca dropdowns y tabla
            logETLStep(2, true); logLog(`BD Sincronizada.`);
        } catch (err) { 
            document.getElementById('tabla-turnos').innerHTML = `<tr><td colspan="7" class="p-6 text-center text-rose-500 font-bold text-xs">Error de conexion.</td></tr>`;
        }
    }
            ...
            if (slotsTotales.length > 0) {
                ...
                slotsTotales.forEach(s => {
                    ...
                    if (ocupado) {
                        ...
                        htmlPanel += `
                        <div class="flex flex-col p-1.5 border border-red-900 bg-red-950/30 rounded text-[9px] font-mono shadow">
                            <div class="flex justify-between text-rose-300">
                                <span>${s.split(' ')[0]}</span><span class="font-bold tracking-wider">${pat}</span>
                            </div>
                            <div class="flex justify-between text-slate-400 mt-1 truncate">
                                <span class="truncate pr-1">${raz}</span><span class="text-sky-500 text-[8px] whitespace-nowrap">Resp: ${rsp}</span>
                            </div>
                        </div>`;
                    } else {
    ...
    function renderizarFilas(items) {
        ...
        if(!tbody) return;
        if(items.length === 0) { tbody.innerHTML = `<tr><td colspan="7" class="p-6 text-center text-slate-500 text-xs">No hay turnos.</td></tr>`; return; }
        ...
        items.forEach(t => {
            ...
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-xs">
                ...
                <td class="p-2 text-slate-300 font-bold">${escapeHtml(t.PlantaDescarga || '-')}</td>
                <td class="p-2 text-white font-semibold">${escapeHtml(t.field_2 || '-')}</td>
                <td class="p-2 font-mono tracking-widest text-slate-300 bg-black/20 text-center rounded">${escapeHtml(t.Patente || '-')}</td>
                <td class="p-2 text-amber-300 font-mono">${escapeHtml(t.field_5 || '-')}</td>
                <td class="p-2 ${colorEst} text-center">${estado}<br><span class="text-[9px] text-slate-500 font-normal truncate block max-w-[100px] mx-auto">${escapeHtml(getResponsableName(t))}</span></td>
    ...
    function renderizarABMHorarios() {
        ...
        configPlantas.forEach((p, idx) => {
            ...
            h += `<tr class="text-xs"><td class="p-2 font-bold text-sky-400">${p.id}</td><td class="p-2"><input type="number" id="cfg_inlv_${idx}" value="${p.hInLV}" class="w-12 bg-slate-900 border border-slate-700 text-center rounded text-white"> a <input type="number" id="cfg_outlv_${idx}" value="${p.hOutLV}" class="w-12 bg-slate-900 border border-slate-700 text-center rounded text-white"></td><td class="p-2"><label class="mr-2"><input type="checkbox" id="cfg_sab_${idx}" ${chkSab} onchange="toggleSabado(${idx})"> Hab.</label><input type="number" id="cfg_ins_${idx}" value="${p.hInS}" class="w-12 bg-slate-900 border border-slate-700 text-center rounded text-white ${!p.sabado ? 'opacity-30' : ''}" ${!p.sabado ? 'disabled' : ''}> a <input type="number" id="cfg_outs_${idx}" value="${p.hOutS}" class="w-12 bg-slate-900 border border-slate-700 text-center rounded text-white ${!p.sabado ? 'opacity-30' : ''}" ${!p.sabado ? 'disabled' : ''}></td><td class="p-2"><input type="number" id="cfg_dur_${idx}" value="${p.duracion}" class="w-16 bg-slate-900 border border-slate-700 text-center rounded text-white"></td><td class="p-2 text-center text-slate-500 font-mono">OK</td></tr>`;
        });
        tb.innerHTML = h;
    }
    ...
    // --- MAESTROS (PROVEEDORES Y USUARIOS) CON ABM ---
    ...
    function renderizarBaseProveedores() {
        ...
        tb.innerHTML = listaProveedoresCache.map((p, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-xs cursor-pointer" onclick="cargarProvForm(${idx})">
                <td class="p-2 font-mono text-amber-400 font-bold">${escapeHtml(p.id)}</td><td class="p-2 text-white font-semibold">${escapeHtml(p.nombre)}</td><td class="p-2 text-slate-400">${escapeHtml(p.tipo)}</td><td class="p-2 text-slate-400">${escapeHtml(p.responsable)}</td><td class="p-2 font-bold ${p.estado === 'Habilitado' ? 'text-emerald-400' : 'text-rose-400'}">${escapeHtml(p.estado)}</td><td class="p-2 text-sky-400 underline">Editar</td>
            </tr>`).join('');
    }
    ...
    function renderizarBaseUsuarios() {
        ...
        tb.innerHTML = listaUsuariosCache.map((u, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-xs cursor-pointer" onclick="cargarUsuForm(${idx})">
                <td class="p-2 font-mono text-amber-400">${escapeHtml(u.usuario)}</td><td class="p-2 text-white font-semibold">${escapeHtml(u.nombre)}</td><td class="p-2 text-slate-400">${escapeHtml(u.email)}</td><td class="p-2 text-slate-400">${escapeHtml(u.planta)}</td><td class="p-2 font-bold ${u.estado === 'Habilitado' ? 'text-emerald-400' : 'text-rose-400'}">${escapeHtml(u.estado)}</td><td class="p-2 text-sky-400 underline">Editar</td>
            </tr>`).join('');
    }
    ...
    function logETLStep(stepNum, isDone=false) {
        ...
        steps.forEach((s, idx) => {
            ...
            if(idx === stepNum-1) { el.classList.add('active'); if(isDone) { el.classList.add('done'); document.getElementById(ic[idx]).innerHTML = '✔'; } } 
