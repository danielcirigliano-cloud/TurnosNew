...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s6" class="tab-content mt-8 w-full">
        ...
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('prov')" class="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2 font-bold rounded text-sm transition border border-slate-600">[+] Sincronizar desde SharePoint</button>
        </div>
        <div class="glass p-6 rounded-xl text-base w-full">
            <div class="overflow-x-auto h-[500px]">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-xs uppercase text-slate-400 font-bold sticky top-0 shadow">
    ...
    <section id="s7" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-6 rounded mb-6 border border-slate-700 flex gap-4 items-end w-full text-sm">
            ...
            <div class="flex-1">
                ...
                <input type="text" id="u_id" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-amber-300 font-bold rounded outline-none" placeholder="Usu01">
            </div>
        ...
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('usu')" class="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2 font-bold rounded text-sm transition border border-slate-600">[+] Sincronizar desde SharePoint</button>
        </div>
    ...
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <div class="w-full grid grid-cols-2 gap-8">
            ...
            <div class="glass p-8 rounded-xl flex flex-col h-[600px]">
                ...
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-4 font-mono text-xs overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit v3.0...</div></div>
            </div>
        </div>
    </section>
...
<script>
    ...
    async function fetchSharePoint(query, options = {}) {
        ...
        logLog(`[HTTP GET] Solicitando datos a: ${targetListName}`);
        let res = await fetch(url, options);
        if (res.status === 404 && targetListName === 'Turnos Proveedores') {
            logLog(`[HTTP 404] Lista no encontrada. Intentando Auto-Discovery en _1...`);
    ...
    async function getDigest() {
        try {
            logLog(`[HTTP POST] Solicitando Security Token (X-RequestDigest)...`);
    ...
    async function ejecutarTransaccion(itemId, payload, msg, skipRefresh = false) {
        ...
        try {
            ...
            logLog(`[ETL] Enviando POST/MERGE a DB Sharepoint...`);
            let res = await fetchSharePoint(query, options);
            if (res.status === 400) {
                logLog(`[HTTP 400] Sanitizando columnas no creadas y reintentando...`);
    ...
    async function initApp() {
        try {
            ...
            // iniciarRecordatorioMailing(); // <-- COMENTADO POR SOLICITUD: Quitar barras (//) para activar envio automatico a las 16hs
    ...
    async function importarCSVSharePoint(tipo, isSilent = false) {
        ...
        logLog(`[ETL] Conectando a repositorio remoto: ${urlDescarga}...`);
        ...
        try {
            ...
            logLog(`[ETL] ${fileName} extraido y mapeado con exito en memoria.`);
            if(!isSilent) showToast(`Padron ${fileName} Importado`, "ok");
        } catch (err) {
            logLog(`[ERROR CSV SP]: ${err.message}`);
            if(!isSilent) showToast("Falla importando CSV. Ver Logs.", "err");
        }
    }
    ...
    async function testConexion() {
        logLog("[ETL] Identificando usuario actual contra SharePoint...");
        try {
            ...
            if (res.ok) {
                ...
                logLog(`[ETL] Handshake exitoso. Usuario: ${currentLoggedUser.name}`);
            }
        } catch (err) { logLog(`[ERROR API USUARIO]: ${err.message}`); }
    }
    ...
    async function cargarTurnos() {
        ...
        try {
            ...
            logLog(`[ERROR LECTURA TURNOS]: ${err.message}`);
        ...
        for(let i=1; i<lines.length; i++) {
            ...
            if(cols.length >= 4) {
                listaProveedoresCache.push({ 
                    id: cols[0].replace(/[^0-9]/g, ''), 
                    nombre: cols[1].trim(), 
                    tipo: cols[2].trim(), 
                    responsable: cols[3].trim(), 
                    estado: 'Habilitado' 
                });
            }
        }
    ...
    // --- ABM USUARIOS (MAPEADO A 5 COLUMNAS) ---
    ...
    function enviarResumenDiario() {
        ...
        if(!para) return showToast("No se detectaron los usuarios autorizados (Usu01-08) en la base", "err");
    ...
    // --- CRON JOB: RECORDATORIO 16:00 HS ---
    function iniciarRecordatorioMailing() {
        setInterval(() => {
            const now = new Date();
            if(now.getHours() === 16 && now.getMinutes() === 0 && now.getSeconds() < 10) {
                showToast("16:00 HS - Generando envio automatico de resumen...", "ok");
                logLog("[CRON] Disparando evento programado 16:00 hs...");
                enviarResumenDiario();
            }
        }, 10000); 
    }
