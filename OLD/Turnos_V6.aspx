            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-300">
                    ...
                    <tbody id="tabla-proveedores" class="divide-y divide-slate-700/50">
                        <tr><td colspan="3" class="p-6 text-center text-slate-500 text-xs">Procesando pool de datos...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
<script>
    ...
    // --- ARQUITECTURA: RENDERIZADO OPTIMISTA (UI-FIRST) ---
    function initApp() {
        try {
            logDebug("Iniciando Renderizado Optimista (UI-First)...");
            
            // 1. Configuracion Estática Sincrónica
            const hoy = new Date();
            ...
            // 2. Renderizado Inmediato de Grillas (Evita la pantalla colgada)
            ...
            logDebug("UI interactiva renderizada con éxito al milisegundo 0.");

            // 3. Ejecución Asíncrona de Red (No bloquea la pantalla si SharePoint se cuelga)
    ...
    function logDebug(msg) {
        ...
        area.insertAdjacentHTML('beforeend', `<div>[${ts}] ${msg}</div>`);
    ...
    function userLog(msg, tipoIngreso = 'MANUAL') {
        ...
        logUserArea.insertAdjacentHTML('beforeend', `<div class="mb-1"><span class="text-slate-500">[${tsComplete}]</span> <span class="text-sky-400 font-bold">[${tipoIngreso}]</span> <span class="text-white font-semibold">${currentLoggedUser.name}:</span> ${msg}</div>`);
    ...
    async function cargarTurnos() {
        ...
        try {
            ...
            logDebug("SharePoint bloqueó petición o la red expiró. Interfaz sobrevive con datos locales.");
        }
    }
    ...
    function consultarTurnoAlFormulario(itemId) {
        ...
        document.getElementById('f_proveedor').value = t.field_2 ? String(t.field_2).trim() : '';
    ...
    function renderizarBaseProveedores() {
        ...
        listaTurnosCache.forEach(t => {
            ...
            if (cuitLimpio && !cuitsMapeados.has(cuitLimpio)) {
                ...
                proveedoresUnicos.push({ cuit: cuitLimpio, text_social: t.field_2 ? String(t.field_2) : 'Sin razon social', email: t.field_3 ? String(t.field_3) : '-' });
            }
        });
