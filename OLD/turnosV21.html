...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s6" class="tab-content mt-8 w-full">
        ...
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('prov')" class="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2 font-bold rounded text-sm transition border border-slate-600">[+] Descargar Padron de SharePoint</button>
        </div>
    ...
    <section id="s7" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-6 rounded-xl mb-6 border border-slate-700 w-full text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                ...
                <div class="col-span-1">
                    <label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Area</label>
                    <input type="text" id="u_area" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none transition-colors focus:border-sky-500">
                </div>
                <div class="col-span-2">
                ...
                <div class="col-span-1">
                    <label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Recibe Reporte</label>
                    <select id="u_reporte" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none transition-colors focus:border-sky-500">
                ...
                <div class="col-span-2">
                    <label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Responsable Directo</label>
                    <input type="text" id="u_resp" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none transition-colors focus:border-sky-500">
                </div>
                <div class="col-span-1">
                    <label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Estado</label>
        ...
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('usu')" class="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2 font-bold rounded text-sm transition border border-slate-600">[+] Descargar Padron de SharePoint</button>
        </div>
        ...
        <div class="glass p-6 rounded-xl w-full">
            <div class="overflow-x-auto h-[500px]">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-[10px] uppercase text-slate-400 font-bold sticky top-0 shadow">
                        <tr><th class="py-3 px-3">ID</th><th class="px-3">Nombre y Apellido</th><th class="px-3">Area</th><th class="px-3">Email</th><th class="px-3">Reporte</th><th class="px-3">Planta</th><th class="px-3">Responsable</th><th class="px-3">Estado</th><th class="px-3 text-center">Accion</th></tr>
                    </thead>
                    <tbody id="tabla-usuarios" class="divide-y divide-slate-700/50">
                        <tr><td colspan="9" class="p-6 text-center text-slate-500 text-sm">Sin datos.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <div class="w-full grid grid-cols-2 gap-8">
            ...
            <div class="glass p-8 rounded-xl flex flex-col h-[600px]">
                ...
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-4 font-mono text-xs overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit v4.2...</div></div>
            </div>
        </div>
    </section>
...
<script>
    ...
    async function actualizarCSVSharePoint(tipo) {
        ...
        try {
            ...
            if (tipo === 'usu') {
                csvContent += "ID;Nombre y apellido;Area;Email;Mail Reporte Diario;Planta;Responsable;Estado\n";
                listaUsuariosCache.forEach(u => { csvContent += `${u.id};${u.nombre};${u.area};${u.email};${u.reporte};${u.planta};${u.responsable};${u.estado}\n`; });
            } else {
            ...
            logLog(`[ETL] Archivo ${fileName} sincronizado bidireccionalmente en SP.`);
            logETLStep(3, true);
        ...
        items.forEach(t => {
            ...
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-xs">
                ...
                <td class="p-3 text-center flex justify-center gap-2">
                    <a href="${relativeSiteUrl}/Lists/${targetListName.replace(' ', '%20')}/DispForm.aspx?ID=${t.Id}" target="_blank" class="bg-indigo-700 hover:bg-indigo-600 text-white px-2 py-1.5 rounded transition text-[10px] uppercase font-bold shadow">SP</a>
    ...
    async function guardarEdicionUsuario() {
        ...
        document.getElementById('u_planta').value = ''; 
