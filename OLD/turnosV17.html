...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <!-- SECCION 2: MONITOR DE TURNOS DIARIOS (AMPLIADO) -->
    <section id="s2" class="tab-content active mt-8 w-full">
        ...
        <div class="flex flex-col gap-4">
            <!-- Bloque Superior: Tablas + Calendario (Grid 12 columnas) -->
            <div class="grid grid-cols-12 gap-4 h-[650px]">
                
                <!-- TABLA PRINCIPAL (8 Columnas, super ancha) -->
                <div class="col-span-8 glass p-4 rounded-xl flex flex-col h-full">
                    ...
                    <div class="overflow-x-auto text-base flex-1">
                        <table class="w-full text-left text-sm text-slate-300">
                            ...
                            <tbody id="tabla-turnos" class="divide-y divide-slate-700/50">
                                <tr><td colspan="7" class="p-6 text-center text-slate-500 text-xs">Cargando base de datos...</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>

                <!-- PANEL DISPONIBILIDAD (4 Columnas, letra mas grande, calendario angosto) -->
                <div class="col-span-4 glass p-4 rounded-xl flex flex-col h-full overflow-hidden">
                    <h3 class="text-sm font-bold text-sky-400 uppercase tracking-wider text-center mb-3">Disponibilidad Logistica</h3>
                    
                    <!-- CALENDARIO ANGOSTO -->
                    <div class="max-w-xs mx-auto w-full bg-slate-950/80 p-2 rounded border border-slate-800 mb-4 text-xs shadow-inner">
                        <div class="flex justify-between items-center mb-1 px-1">
                            <span id="calendar-month-title" class="font-bold text-white uppercase tracking-wide">MES</span>
                            <div class="text-[9px] text-slate-400 font-semibold font-mono">SELECCION FECHA</div>
                        </div>
            ...
            <!-- TABLA RESUMEN DIARIO (Bloque Inferior) -->
            <div class="glass p-5 rounded-xl flex flex-col w-full shadow-lg">
                <div class="flex justify-between items-center mb-3 border-b border-slate-700 pb-2">
                    <h3 class="text-sm font-bold text-emerald-400 uppercase tracking-wider">Resumen de Operacion Diaria</h3>
                    <span id="resumen-fecha-label" class="text-xs font-mono text-slate-400">Calculando...</span>
                </div>
                <div class="overflow-x-auto w-full">
                    <table class="w-full text-left text-sm text-slate-300">
                        ...
                        <tbody id="tabla-resumen-diario" class="divide-y divide-slate-700/50">
                            <!-- JS Inject -->
                        </tbody>
                    </table>
                </div>
    ...
    <!-- SECCION 1: ABM DE TURNOS (Ancho total) -->
        ...
        <div class="glass p-8 rounded-xl w-full grid grid-cols-2 gap-6">
            ...
            <div class="col-span-2 grid grid-cols-3 gap-6 border-b border-slate-700 pb-6 mb-2">
                <div>
                    ...
                    <input type="text" id="f_patente" placeholder="AA111AA / AAA111" class="bg-slate-900 border border-slate-700 p-3 w-full font-mono text-amber-300 uppercase outline-none f-control transition-colors" maxlength="7">
                </div>
                <div>
                    <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Hora Ingreso Real</label>
                    <input type="time" id="f_hora_ing" class="bg-slate-900 border border-slate-700 p-3 w-full text-emerald-400 outline-none f-control transition-colors">
                </div>
                <div>
                    <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Hora Salida Real</label>
                    <input type="time" id="f_hora_sal" class="bg-slate-900 border border-slate-700 p-3 w-full text-rose-400 outline-none f-control transition-colors">
                </div>
            </div>

            <div>
                <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Num Proveedor / CUIT *</label>
                <input type="text" id="f_cuit" placeholder="Solo numeros" onblur="autocompletarProveedor()" class="bg-slate-900 border border-slate-700 p-3 w-full text-amber-300 font-bold outline-none f-control transition-colors">
            </div>
            <div>
                <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Num OC</label>
                <input type="text" id="f_oc" placeholder="Ej: 698765" class="bg-slate-900 border border-slate-700 p-3 w-full text-white outline-none f-control transition-colors">
            </div>
            <div class="col-span-2">
                <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Razon Social Proveedor *</label>
                <input type="text" id="f_proveedor" placeholder="Razon Social" class="bg-slate-900 border border-slate-700 p-3 w-full text-white outline-none f-control transition-colors">
            </div>
            <div class="col-span-2">
                <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Email de Contacto</label>
                <input type="email" id="f_email" placeholder="correo@proveedor.com" class="bg-slate-900 border border-slate-700 p-3 w-full text-white outline-none f-control transition-colors">
            </div>

            <div>
                <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Planta Fisica (Ubicacion)</label>
                <select id="f_planta_descarga" onchange="handleHorariosChange()" class="bg-slate-900 border border-slate-700 p-3 w-full text-sky-400 font-bold outline-none f-control transition-colors"></select>
            </div>
            <div>
                <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Tipo Carga (Area)</label>
                <select id="f_planta" class="bg-slate-900 border border-slate-700 p-3 w-full text-white outline-none f-control transition-colors">
            ...
            <div>
                ...
                <input type="date" id="f_fecha" onchange="handleHorariosChange()" class="bg-slate-900 border border-slate-700 p-3 w-full text-white outline-none f-control transition-colors">
            </div>
            <div>
                <label class="block text-xs uppercase text-slate-400 font-bold mb-1">Slot Horario Calculado *</label>
                <select id="f_horario" class="bg-slate-900 border border-slate-700 p-3 w-full text-sky-300 font-mono font-bold outline-none f-control transition-colors"></select>
            </div>

            <div class="col-span-2 bg-slate-950 p-4 rounded text-sm border border-slate-800 text-slate-400 flex justify-between mt-4">
    ...
    <!-- SECCION 6: BASE PROVEEDORES (CON ABM) (Ancho total) -->
    <section id="s6" class="tab-content mt-8 w-full">
        <h2 class="text-2xl font-bold mb-2 text-sky-400">Base Maestro de Proveedores</h2>
        <div class="glass p-6 rounded mb-6 border border-slate-700 flex gap-4 items-end w-full text-sm">
            <input type="hidden" id="p_index">
            <div class="flex-1">
                <label class="block uppercase text-slate-400 font-bold mb-1">Num Proveedor</label>
                <input type="text" id="p_cuit" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-amber-300 font-bold rounded outline-none">
            </div>
            <div class="flex-2 w-1/3">
                <label class="block uppercase text-slate-400 font-bold mb-1">Razon Social</label>
                <input type="text" id="p_razon" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
            </div>
            <div class="flex-1">
                <label class="block uppercase text-slate-400 font-bold mb-1">Tipo Insumo</label>
                <input type="text" id="p_tipo" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
            </div>
            <div class="flex-1">
                <label class="block uppercase text-slate-400 font-bold mb-1">Responsable</label>
                <input type="text" id="p_resp" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
            </div>
            <div class="flex-1">
                <label class="block uppercase text-slate-400 font-bold mb-1">Estado</label>
                <select id="p_estado" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
                    <option value="Habilitado">Habilitado</option>
                    <option value="Deshabilitado">No Habilitado</option>
                </select>
            </div>
            <div>
                <button onclick="guardarEdicionProveedor()" class="bg-sky-600 hover:bg-sky-500 text-white px-6 py-2.5 font-bold rounded transition">Guardar</button>
            </div>
        </div>
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('prov')" class="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2 font-bold rounded text-sm transition border border-slate-600">[+] Importar desde SharePoint</button>
        </div>
            ...
            <div class="overflow-x-auto h-[600px]">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-xs uppercase text-slate-400 font-bold sticky top-0">
                        <tr><th class="py-3 px-3">Num Proveedor</th><th class="px-3">Razon Social</th><th class="px-3">Tipo Insumo</th><th class="px-3">Responsable</th><th class="px-3">Estado</th><th class="px-3 text-center">Accion</th></tr>
                    </thead>
                    <tbody id="tabla-proveedores" class="divide-y divide-slate-700/50">
                        <tr><td colspan="6" class="p-6 text-center text-slate-500 text-sm">Sin padron de proveedores cargado.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <!-- SECCION 7: BASE USUARIOS (CON ABM) (Ancho total) -->
    <section id="s7" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-6 rounded mb-6 border border-slate-700 flex gap-4 items-end w-full text-sm">
            ...
            <div class="flex-1">
                <label class="block uppercase text-slate-400 font-bold mb-1">ID Usuario</label>
                <input type="text" id="u_id" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-amber-300 font-bold rounded outline-none">
            </div>
            <div class="flex-2 w-1/4">
                <label class="block uppercase text-slate-400 font-bold mb-1">Nombre (Responsable)</label>
                <input type="text" id="u_nombre" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
            </div>
            <div class="flex-2 w-1/4">
                <label class="block uppercase text-slate-400 font-bold mb-1">Email Corporativo</label>
                <input type="text" id="u_email" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
            </div>
            <div class="flex-1">
                <label class="block uppercase text-slate-400 font-bold mb-1">Planta Asociada</label>
                <input type="text" id="u_planta" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
            </div>
            <div class="flex-1">
                <label class="block uppercase text-slate-400 font-bold mb-1">Estado</label>
                <select id="u_estado" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-white rounded outline-none">
                    <option value="Habilitado">Habilitado</option>
                    <option value="Deshabilitado">No Habilitado</option>
                </select>
            </div>
            <div>
                <button onclick="guardarEdicionUsuario()" class="bg-sky-600 hover:bg-sky-500 text-white px-6 py-2.5 font-bold rounded transition">Guardar</button>
            </div>
        </div>
        <div class="flex gap-4 mb-4 items-center">
            <button onclick="importarCSVSharePoint('usu')" class="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2 font-bold rounded text-sm transition border border-slate-600">[+] Importar desde SharePoint</button>
        </div>
        <div class="glass p-6 rounded-xl w-full">
            <div class="overflow-x-auto h-[600px]">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-xs uppercase text-slate-400 font-bold sticky top-0">
                        <tr><th class="py-3 px-3">ID Usuario</th><th class="px-3">Nombre</th><th class="px-3">Email</th><th class="px-3">Planta</th><th class="px-3">Estado</th><th class="px-3 text-center">Accion</th></tr>
                    </thead>
    ...
    <!-- SECCION 8: CONFIGURACION DE HORARIOS (Ancho total) -->
    ...
    <!-- SECCION 9: MODULO ETL Y LOGS (Ancho total) -->
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <div class="w-full grid grid-cols-2 gap-8">
            ...
            <div class="glass p-8 rounded-xl flex flex-col h-[600px]">
                <h3 class="text-base font-bold text-slate-300 uppercase mb-4 border-b border-slate-700 pb-3">Archivos de Log (Virtual Audit)</h3>
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-4 font-mono text-xs overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit...</div></div>
            </div>
        </div>
    </section>
...
<script>
    // =========================================================================
    // ZONA INTACTA DE CONEXION A SHAREPOINT
    // =========================================================================
    ...
    function initApp() {
        try {
            ...
            setTimeout(() => { testConexion(); refrescarDBAbsoluto(); }, 150); 
        } catch (err) {}
    }

    // --- IMPORTACION DIRECTA DE SHAREPOINT ---
    async function importarCSVSharePoint(tipo) {
        // Enlaces provistos por el usuario para extraer data RAW. 
        // Nota: Los enlaces web de visualizacion de SP suelen estar ofuscados. 
        // Lo ideal es apuntar al archivo fisico en la libreria.
        let urlDescarga = `${relativeSiteUrl}/Documentos compartidos/${tipo === 'prov' ? 'proveedores.csv' : 'Usuarios.csv'}`;
        
        try {
            let res = await fetch(urlDescarga);
            if (!res.ok) {
                // Intento fallback ingles
                urlDescarga = `${relativeSiteUrl}/Shared Documents/${tipo === 'prov' ? 'proveedores.csv' : 'Usuarios.csv'}`;
                res = await fetch(urlDescarga);
            }
            if (!res.ok) throw new Error("No se encontro archivo CSV en Documentos Compartidos.");
            
            const text = await res.text();
            if(tipo === 'prov') parsearCSVProveedores(text);
            else parsearCSVUsuarios(text);
            
            showToast("Padron de SP Importado", "ok");
        } catch (err) {
            logLog(`[ERROR CSV SP]: ${err.message}`);
            showToast("Falla importando CSV. Ver terminal.", "err");
        }
    }
    ...
    // --- FILTROS SINCRONIZADOS (PLANTA <-> RESPONSABLES) ---
    function sincronizarFiltrosYMonitor() {
        ...
        if (plantaSelect !== 'TODAS') {
            let usuariosPlanta = usuariosValidos.filter(u => String(u.planta).toLowerCase().includes(plantaSelect.toLowerCase()));
        ...
        if (selectU) {
            ...
            const historicos = [...new Set(listaTurnosCache.map(t => getResponsableName(t)).filter(Boolean))];
            const combinados = [...new Set([...usr, ...historicos])].sort();

            let h = '<option value="TODOS">-- Todos --</option>';
            combinados.forEach(u => { h += `<option value="${escapeHtml(u)}">${escapeHtml(u)}</option>`; });
            selectU.innerHTML = h;
        }
    ...
    // --- TABLA RESUMEN DIARIO (NUEVO) ---
    function generarResumenDiario() {
        const tbody = document.getElementById('tabla-resumen-diario');
        ...
        configPlantas.forEach(p => {
            ...
            if(trnP.length > 0) {
                ...
                let canc = trnP.filter(t => (t.field_7||'').toLowerCase().includes('cancelado')).length;
        ...
        if(htmlResumen === '') htmlResumen = '<tr><td colspan="6" class="p-6 text-center text-slate-500 text-sm">No hay actividad logistica en esta fecha.</td></tr>';
    ...
    async function cargarTurnos() {
        ...
        try {
            ...
            sincronizarFiltrosYMonitor(); 
            logETLStep(2, true); logLog(`BD Sincronizada.`);
        } catch (err) { 
            document.getElementById('tabla-turnos').innerHTML = `<tr><td colspan="7" class="p-6 text-center text-rose-500 font-bold text-sm">Error de conexion.</td></tr>`;
        }
    }
    ...
    function calcularDisponibilidadOffline() {
        ...
        configPlantas.forEach(plantaConfig => {
            ...
            if (slotsTotales.length > 0) {
                htmlPanel += `<div class="mb-5"><h4 class="text-xs font-bold text-sky-300 uppercase tracking-wider text-center mb-2 bg-sky-950/40 py-2 rounded border border-sky-900/30">${plantaConfig.id}</h4><div class="space-y-2">`;
                slotsTotales.forEach(s => {
                    ...
                    if (ocupado) {
                        ...
                        htmlPanel += `
                        <div class="flex flex-col p-2.5 border border-red-900 bg-red-950/30 rounded text-[11px] font-mono shadow-md">
                            <div class="flex justify-between text-rose-300 mb-1">
                                <span class="font-bold">${s.split(' ')[0]}</span><span class="font-bold tracking-widest bg-black/40 px-1 rounded">${pat}</span>
                            </div>
                            <div class="flex justify-between text-slate-300 items-end">
                                <span class="truncate pr-2 w-2/3 font-sans font-semibold">${raz}</span>
                                <span class="text-sky-400 text-[9px] whitespace-nowrap bg-sky-900/30 px-1 rounded">Resp: ${rsp}</span>
                            </div>
                        </div>`;
                    } else {
                        htmlPanel += `<div class="flex justify-between items-center p-2.5 border border-emerald-900 bg-emerald-950/30 rounded text-[11px] font-mono text-emerald-400 shadow-md opacity-70">
                            <span class="font-bold">${s.split(' ')[0]}</span><span class="font-bold tracking-widest opacity-60">LIBRE</span>
                        </div>`;
                    }
                });
                htmlPanel += `</div></div>`;
    ...
    function renderizarFilas(items) {
        ...
        if(!tbody) return;
        if(items.length === 0) { tbody.innerHTML = `<tr><td colspan="7" class="p-6 text-center text-slate-500 text-sm">No hay turnos para estos filtros.</td></tr>`; return; }
        ...
        items.forEach(t => {
            ...
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-xs">
                ...
                <td class="p-3 text-white font-semibold">${escapeHtml(t.field_2 || '-')}</td>
                ...
                <td class="p-3 text-center flex justify-center gap-2">
                    ...
                    ${!esAprobado && estado.toLowerCase() !== 'cancelado' ? `<button onclick="modoABM_Aprobar(${t.Id})" class="bg-emerald-700 hover:bg-emerald-600 text-white px-3 py-1.5 rounded transition text-[10px] uppercase font-bold shadow">Aprobar</button>` : ''}
                </td>
            </tr>`;
        });
    ...
    function renderizarGrilaCalendario() {
        ...
        for (let dia = 1; dia <= totalDiasMes; dia++) {
            ...
            calHtml += `<button onclick="conmutarFechaDesdeCalendario('${strDia}')" class="p-1.5 rounded ${claseEstilo}">${dia}</button>`;
        }
        gridContainer.innerHTML = calHtml;
    }
    function conmutarFechaDesdeCalendario(nuevaFecha) {
        fechaSeleccionadaGlobal = nuevaFecha;
        renderizarGrilaCalendario(); aplicarFiltrosMonitor(); 
    }
    ...
    function regenerarHorariosABM(modoFiltro = false) {
        ...
        let oc = modoFiltro ? listaTurnosCache.filter(t => t.field_6 && t.field_6.startsWith(f) && t.PlantaDescarga === pf && String(t.field_7).toLowerCase() !== 'cancelado').map(t => normalizarStringHorario(t.field_5)) : [];
        let htmlOpts = '';
    ...
    async function checkSlotLibreConcurrencia(planta, fecha, slot) {
        logETLStep(2, false);
        try {
            const res = await fetchSharePoint(`items?$filter=PlantaDescarga eq '${planta}' and startswith(field_6, '${fecha}') and field_5 eq '${slot}' and field_7 ne 'Cancelado'&$select=Id`);
    ...
    function abrirAltaTurno() {
        ...
        document.getElementById('f_fecha').value = fechaSeleccionadaGlobal >= hoyIso ? fechaSeleccionadaGlobal : hoyIso;
        document.getElementById('f_fecha').min = hoyIso; 
        autoDia(); handleHorariosChange();
    ...
    function modoABM_Ver(id) {
        ...
        habilitarCamposABM(false); renderBotonesABM('VER'); document.querySelector("[data-target='s1']").click();
    }
    function modoABM_Aprobar(id) {
        ...
        habilitarCamposABM(false); renderBotonesABM('APROBAR_PANEL'); document.querySelector("[data-target='s1']").click();
    }
    ...
    function renderBotonesABM(modo) {
        ...
        else if (modo === 'APROBAR_PANEL') panel.innerHTML = `<button onclick="ejecutarAccionFlujo('Aprobado')" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-3 font-bold rounded text-white flex-1 transition shadow">[OK] Aprobar</button><button onclick="ejecutarAccionFlujo('Cancelado')" class="bg-rose-600 hover:bg-rose-500 px-4 py-3 font-bold rounded text-white flex-1 transition shadow">[X] Cancelar</button><button onclick="modoABM_Reprogramar()" class="bg-amber-600 hover:bg-amber-500 px-4 py-3 font-bold rounded text-white flex-1 transition shadow">Reprogramar</button>`;
    ...
    function limpiarFormularioABM() {
        limpiarErroresVisuales();
        document.getElementById('f_item_id').value = ''; document.querySelectorAll('.f-control').forEach(i => { i.value = ''; i.disabled = true; });
    ...
    function parsearCSVProveedores(text) {
        ...
        for(let i=1; i<lines.length; i++) {
            ...
            if(cols.length >= 4) listaProveedoresCache.push({ tipo: cols[0].trim(), nombre: cols[1].trim(), id: cols[2].replace(/[^0-9]/g, ''), responsable: cols[3].trim(), estado: 'Habilitado' });
        }
    ...
    function renderizarBaseProveedores() {
        ...
        tb.innerHTML = listaProveedoresCache.map((p, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm cursor-pointer" onclick="cargarProvForm(${idx})">
                <td class="p-3 font-mono text-amber-400 font-bold">${escapeHtml(p.id)}</td><td class="p-3 text-white font-semibold">${escapeHtml(p.nombre)}</td><td class="p-3 text-slate-400">${escapeHtml(p.tipo)}</td><td class="p-3 text-slate-400">${escapeHtml(p.responsable)}</td><td class="p-3 font-bold ${p.estado === 'Habilitado' ? 'text-emerald-400' : 'text-rose-400'}">${escapeHtml(p.estado)}</td><td class="p-3 text-sky-400 underline text-center">Editar</td>
            </tr>`).join('');
    }

    function cargarProvForm(idx) {
        const p = listaProveedoresCache[idx];
        document.getElementById('p_index').value = idx; document.getElementById('p_cuit').value = p.id; document.getElementById('p_razon').value = p.nombre;
        document.getElementById('p_tipo').value = p.tipo; document.getElementById('p_resp').value = p.responsable; document.getElementById('p_estado').value = p.estado || 'Habilitado';
    }

    function guardarEdicionProveedor() {
        const idx = document.getElementById('p_index').value; const pId = document.getElementById('p_cuit').value.replace(/[^0-9]/g, '');
        if(!pId) return showToast("Falta CUIT", "err");
        const obj = { id: pId, nombre: document.getElementById('p_razon').value, tipo: document.getElementById('p_tipo').value, responsable: document.getElementById('p_resp').value, estado: document.getElementById('p_estado').value };
        if(idx === '') listaProveedoresCache.push(obj); else listaProveedoresCache[idx] = obj;
        renderizarBaseProveedores(); showToast("Proveedor Guardado", "ok");
    }
    ...
    function parsearCSVUsuarios(text) {
        ...
        for(let i=1; i<lines.length; i++) {
            ...
            if(!row) continue;
            const cols = row.split(/,|;/); 
            if(cols.length >= 4) listaUsuariosCache.push({ usuario: cols[0].trim(), nombre: cols[1].trim(), email: cols[2].trim(), planta: cols[3].trim(), estado: 'Habilitado' });
        }
    ...
    function renderizarBaseUsuarios() {
        ...
        tb.innerHTML = listaUsuariosCache.map((u, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm cursor-pointer" onclick="cargarUsuForm(${idx})">
                <td class="p-3 font-mono text-amber-400">${escapeHtml(u.usuario)}</td><td class="p-3 text-white font-semibold">${escapeHtml(u.nombre)}</td><td class="p-3 text-slate-400">${escapeHtml(u.email)}</td><td class="p-3 text-slate-400">${escapeHtml(u.planta)}</td><td class="p-3 font-bold ${u.estado === 'Habilitado' ? 'text-emerald-400' : 'text-rose-400'}">${escapeHtml(u.estado)}</td><td class="p-3 text-sky-400 underline text-center">Editar</td>
            </tr>`).join('');
    }

    function cargarUsuForm(idx) {
        const u = listaUsuariosCache[idx];
        document.getElementById('u_index').value = idx; document.getElementById('u_id').value = u.usuario; document.getElementById('u_nombre').value = u.nombre;
        document.getElementById('u_email').value = u.email; document.getElementById('u_planta').value = u.planta; document.getElementById('u_estado').value = u.estado || 'Habilitado';
    }

    function guardarEdicionUsuario() {
        const idx = document.getElementById('u_index').value; const uId = document.getElementById('u_id').value.trim();
        if(!uId) return showToast("Falta ID", "err");
        const obj = { usuario: uId, nombre: document.getElementById('u_nombre').value, email: document.getElementById('u_email').value, planta: document.getElementById('u_planta').value, estado: document.getElementById('u_estado').value };
        if(idx === '') listaUsuariosCache.push(obj); else listaUsuariosCache[idx] = obj;
        renderizarBaseUsuarios(); showToast("Usuario Guardado", "ok");
    }

    function enviarResumenDiario() {
        const bcc = listaUsuariosCache.filter(u => u.estado !== 'Deshabilitado').map(u => u.email).filter(Boolean).join(';');
        if(!bcc) return showToast("No hay correos habilitados", "err");
        const fFiltro = fechaSeleccionadaGlobal; const turnosHoy = listaTurnosCache.filter(t => t.field_6 && t.field_6.startsWith(fFiltro));
        ...
        window.location.href = `mailto:?bcc=${bcc}&subject=Resumen Molca - ${fFiltro}&body=${encodeURIComponent(body)}`;
    }
    ...
    async function generarLogArchivoSP(modulo, detalle) { logLog(`Audit Log -> Mod: ${modulo} | ${detalle} | User: ${currentLoggedUser.name}`); }

    function showToast(msg, type) { const t = document.getElementById('toast'); if(!t) return; t.textContent = msg; t.className = `show ${type}`; setTimeout(() => t.className = '', 3000); }
