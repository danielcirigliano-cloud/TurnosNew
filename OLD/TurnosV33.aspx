...
<head>
    ...
    <style>
        ...
        body { font-family: 'Inter', system-ui, -apple-system, sans-serif; background-color: var(--sp-bg); color: #e2e8f0; margin: 0; font-size: 1rem; }
...
<aside class="w-64 glass flex flex-col p-4 border-r border-slate-700 h-full z-10">
    ...
    <nav class="space-y-1 flex-1 overflow-y-auto" id="main-nav">
        ...
        <button class="w-full text-left rounded tab-btn" onclick="syncMaestrosSharePoint()">⚡ Sync Maestros</button>
    </nav>
</aside>
...
<script>
    ...
    function mostrarBannerABM(id, modo) {
        ...
        if(modo === 'VER') {
            ...
            banner.textContent = `Visualizando turno ${id}`;
            ...
            banner.textContent = `Editando turno ${id}`;
        }
    }
    ...
    function renderBotonesABM(modo) {
        ...
        <button onclick="habilitarEdicionTurno()" class="bg-amber-600 hover:bg-amber-500 px-4 py-3 font-bold rounded text-white flex-1 transition shadow">Modificar</button>
        <button onclick="eliminarTurno()" class="bg-rose-600 hover:bg-rose-500 px-4 py-3 font-bold rounded text-white flex-1 transition shadow">Eliminar</button>
        <button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-4 py-3 font-bold rounded text-white flex-1 transition shadow">Volver</button>` class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded text-white flex-1 transition shadow">Volver al Monitor</button>`;
...
async function syncMaestrosSharePoint() {
    showToast('Sincronizando maestros...','ok');
    await Promise.all([importarCSVSharePoint('prov'),importarCSVSharePoint('usu')]);
    renderizarBaseProveedores();
    renderizarBaseUsuarios();
    sincronizarFiltrosYMonitor();
}

function habilitarEdicionTurno(){
 document.getElementById('form-mode-indicator').textContent='EDICION';
...
async function eliminarTurno(){
 ...
 await ejecutarTransaccion(id,{field_7:'Cancelado'},'Turno eliminado');
 volverTurnosDiarios();
}
