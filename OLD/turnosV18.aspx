        ...
        <div class="flex flex-col gap-4">
            <div class="grid grid-cols-12 gap-4 h-[650px]">
                ...
                <div class="col-span-4 glass p-4 rounded-xl flex flex-col h-full overflow-hidden">
                    <h3 class="text-base font-bold text-sky-400 uppercase tracking-wider text-center mb-3">Disponibilidad Logistica</h3>
                    
                    <div class="w-full bg-slate-950/80 p-3 rounded border border-slate-800 mb-4 text-sm shadow-inner">
                        ...
                        <div id="calendar-days-grid" class="grid grid-cols-7 gap-2 text-center font-mono font-bold text-sm"></div>
                    </div>
            ...
            <div class="overflow-x-auto h-[600px]">
                <table class="w-full text-left text-sm text-slate-300">
                    ...
                    <tbody id="tabla-proveedores" class="divide-y divide-slate-700/50">
                        <tr><td colspan="6" class="p-6 text-center text-slate-500 text-sm">Sin padron cargado.</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
    ...
    <section id="s7" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-6 rounded mb-6 border border-slate-700 flex gap-4 items-end w-full text-sm">
            ...
            <div class="flex-2 w-1/4">
                <label class="block uppercase text-slate-400 font-bold mb-1">Nombre y Apellido</label>
        ...
        <div class="glass p-6 rounded-xl w-full">
            <div class="overflow-x-auto h-[600px]">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-xs uppercase text-slate-400 font-bold sticky top-0">
                        <tr><th class="py-3 px-3">ID Usuario</th><th class="px-3">Nombre y Apellido</th><th class="px-3">Email</th><th class="px-3">Planta</th><th class="px-3">Estado</th><th class="px-3 text-center">Accion</th></tr>
                    </thead>
...
<script>
    ...
    async function ejecutarTransaccion(itemId, payload, msg, skipRefresh = false) {
        ...
        try {
            ...
            logLog(`[ETL] POST hacia BD Sharepoint (${targetListName})...`);
    ...
    async function initApp() {
        try {
            logLog("> Iniciando auto-sincronizacion de bases maestras...");
            const hoy = new Date();
            ...
            // Auto-Sync Paralelo y silencioso (No frena UI)
            Promise.all([
    ...
    async function importarCSVSharePoint(tipo, isSilent = false) {
        ...
        logLog(`[ETL] Conectando a repositorio remoto: ${fileName}...`);
        ...
        try {
            ...
            logLog(`[ETL] ${fileName} extraido y transformado con exito.`);
            if(!isSilent) showToast("Padron de SP Importado", "ok");
        } catch (err) {
            logLog(`[ERROR CSV SP]: ${err.message}`);
            if(!isSilent) showToast("Falla importando CSV. Ver terminal.", "err");
        }
    }
    ...
    function sincronizarFiltrosYMonitor() {
        ...
        if (selectU) {
            // Nombre y apellido mapeado en "nombre" de base usuarios
    ...
    async function testConexion() {
        logLog("[ETL] Conectando a SharePoint API: /_api/web/currentUser");
        try {
    ...
    async function cargarTurnos() {
        logETLStep(1, false);
        try {
            logLog(`[ETL] Solicitando turnos OData a ${targetListName}...`);
            ...
            sincronizarFiltrosYMonitor(); 
            logETLStep(2, true); logLog(`[ETL] BD Sincronizada.`);
        } catch (err) { 
                    ...
                    if (ocupado) {
                        ...
                        <div class="flex flex-col p-3 border border-red-900 bg-red-950/30 rounded text-sm font-mono shadow-md">
                            <div class="flex justify-between text-rose-300 mb-1">
                                <span class="font-bold">${s.split(' ')[0]}</span><span class="font-bold tracking-widest bg-black/40 px-2 rounded">${pat}</span>
                            </div>
                            <div class="flex justify-between text-slate-300 items-end mt-1">
                                <span class="truncate pr-2 w-2/3 font-sans font-semibold">${raz}</span>
                                <span class="text-sky-400 text-xs whitespace-nowrap bg-sky-900/30 px-1 rounded">Resp: ${rsp}</span>
                            </div>
                        </div>`;
                    } else {
    ...
    function renderizarGrilaCalendario() {
        ...
        for (let dia = 1; dia <= totalDiasMes; dia++) {
            ...
            calHtml += `<button onclick="conmutarFechaDesdeCalendario('${strDia}')" class="p-2 rounded text-sm ${claseEstilo}">${dia}</button>`;
        }
    ...
    function enviarResumenDiario() {
        const usuariosPermitidos = ['Usu01', 'Usu02', 'Usu03', 'Usu04', 'Usu05', 'Usu06', 'Usu07', 'Usu08'];
        const bcc = listaUsuariosCache.filter(u => u.estado !== 'Deshabilitado' && usuariosPermitidos.includes(u.usuario)).map(u => u.email).filter(Boolean).join(';');
        if(!bcc) return showToast("No hay correos validos para envio", "err");
    ...
    async function generarLogArchivoSP(modulo, detalle) { logLog(`[Audit Log] -> Modulo: ${modulo} | Accion: ${detalle} | Usuario: ${currentLoggedUser.name}`); }
