...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s1" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-8 rounded-xl w-full grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <!-- Cambio 3: quitado; ahora usa Inter igual que el resto de los campos -->
    ...
    <section id="s8" class="tab-content mt-8 w-full">
        ...
        <p class="text-sm text-slate-400 mb-6">Pantalla alineada al formato de <span class="text-sky-400">capacidad_turnos_por_planta.csv</span>. Se muestra una fila por <strong>Planta + Tipo_Carga</strong>, con <strong>Habilitado</strong>, <strong>Turnos_Cargados</strong>, <strong>Turnos_Calc</strong>, <strong>Turnos_Sabado</strong>, <strong>Min_LV</strong>, <strong>Min_SAB</strong>, <strong>Minutos_Turno</strong> y <strong>TotalDia</strong>.</p>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-800 text-sm uppercase text-slate-400 font-bold border-b border-slate-700">
                        <tr><th class="py-4 px-4">Planta</th><th class="px-4">Tipo_Carga</th><th class="px-4 text-center">Habilitado</th><th class="px-4 text-center">Turnos_Cargados</th><th class="px-4 text-center">Turnos_Calc</th><th class="px-4 text-center">Turnos_Sabado</th><th class="px-4 text-center">Min_LV</th><th class="px-4 text-center">Min_SAB</th><th class="px-4 text-center">Minutos_Turno</th><th class="px-4 text-center">TotalDia</th><th class="px-4 text-center">Estatus</th></tr>
                    </thead>
                    <tbody id="tabla-config-horarios" class="divide-y divide-slate-700/50"></tbody>
                </table>
            </div>
            <div class="mt-6 flex gap-2">
                <button onclick="cargarCapacidadTurnosCSV(); renderizarABMHorarios(); showToast('Pantalla recargada desde CSV', 'ok');" class="bg-emerald-600 hover:bg-emerald-500 px-6 py-3 font-bold rounded shadow transition text-white">Refrescar desde CSV</button>
            </div>
        </div>
    </section>
    ...
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <div class="w-full grid grid-cols-2 gap-8">
            ...
            <div class="glass p-8 rounded-xl flex flex-col h-[600px]">
                ...
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-4 text-xs overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit v6.3...</div></div>
            </div>
        </div>
    </section>
...
<script>
    ...
    async function actualizarCSVSharePoint(tipo) {
        ...
        try {
            const digest = await getDigest();
            let fileName = tipo === 'prov' ? 'proveedores.csv' : (tipo === 'usu' ? 'Usuarios.csv' : 'Flota.csv');
    ...
    function normalizarTipoZona(valor) {
        const v = String(valor || '').trim().toUpperCase();
        if (!v) return '';
        if (v === 'INSUMOS') return 'INSUMOS';
        if (v === 'PACKAGING') return 'PACKAGING';
        if (v === 'NO PRODUCTIVO' || v === 'NO_PRODUCTIVO' || v === 'NO-PRODUCTIVO') return 'NO_PRODUCTIVO';
        return v.replace(/\s+/g, '_');
    }
    ...
    function calcularTurnosDesdeVentana(minutosDisponibles, minutosTurno) {
        if (!minutosTurno || minutosTurno <= 0) return 0;
        return Number((minutosDisponibles / minutosTurno).toFixed(2));
    }
    ...
    function construirIndiceCapacidad() {
        ...
        capacidadPorPlantaTipoCache.forEach(r => {
            ...
            if (!idx[planta]) idx[planta] = { tipos: {}, resumen: null };
            idx[planta].tipos[tipo] = r;
            if (!idx[planta].resumen) {
                idx[planta].resumen = {
                    Planta: planta,
                    ...
                    LV_Desde: toNum(r.LV_Desde, 0),
                    LV_Hasta: toNum(r.LV_Hasta, 0),
                    Sab_Habilitado: toNum(r.Sab_Habilitado, 0),
                    Sab_Desde: toNum(r.Sab_Desde, 0),
                    Sab_Hasta: toNum(r.Sab_Hasta, 0)
                };
            }
        });
    ...
    function convertirConfigDesdeCSV() {
        ...
        capacidadPorPlantaTipoCache.forEach(r => {
            ...
            const minLV = toNum(r.Min_LV, 2);
            const minS = toNum(r.Min_SAB, 2);
            const minutosTurno = toNum(r.Minutos_Turno, 2) || base.duracion || 1;
            const tieneHorLV = String(r.LV_Desde || '').trim() !== '' && String(r.LV_Hasta || '').trim() !== '';
            const tieneHorS = String(r.Sab_Desde || '').trim() !== '' && String(r.Sab_Hasta || '').trim() !== '';
            baseMap.set(planta, {
                id: planta,
                hInLV: tieneHorLV ? toNum(r.LV_Desde, 0) : base.hInLV,
                hOutLV: tieneHorLV ? toNum(r.LV_Hasta, 0) : base.hOutLV,
                hInS: tieneHorS ? toNum(r.Sab_Desde, 0) : base.hInS,
                hOutS: tieneHorS ? toNum(r.Sab_Hasta, 0) : base.hOutS,
                duracion: minutosTurno,
                sabado: (String(r.Sab_Habilitado || '').trim() !== '' ? toNum(r.Sab_Habilitado, 0) === 1 : base.sabado),
                totalDiaLV: ('TotalDia' in r && String(r.TotalDia).trim() !== '') ? toNum(r.TotalDia, 2) : calcularTurnosDesdeVentana(minLV, minutosTurno),
                totalSabado: ('Turnos_Sabado' in r && String(r.Turnos_Sabado).trim() !== '') ? toNum(r.Turnos_Sabado, 2) : calcularTurnosDesdeVentana(minS, minutosTurno)
            });
        });
    ...
    async function cargarCapacidadTurnosCSV() {
        try {
            let res = await fetch(CAPACIDAD_CSV_SERVER_RELATIVE_URL);
            if (!res.ok) {
                res = await fetch(CAPACIDAD_CSV_SERVER_RELATIVE_URL.replace('/Documentos compartidos/', '/Shared Documents/'));
            }
            if (!res.ok) throw new Error('Archivo no encontrado: capacidad_turnos_por_planta.csv');
            const text = await res.text();
            const rows = parseCsvSemicolon(text).map(r => {
                const tieneLVDesdeHasta = String(r.LV_Desde || '').trim() !== '' && String(r.LV_Hasta || '').trim() !== '';
                const tieneSabDesdeHasta = String(r.Sab_Desde || '').trim() !== '' && String(r.Sab_Hasta || '').trim() !== '';
                const minLV = ('Min_LV' in r && String(r.Min_LV).trim() !== '')
                    ? toNum(r.Min_LV, 2)
                    : (tieneLVDesdeHasta ? ((toNum(r.LV_Hasta, 0) - toNum(r.LV_Desde, 0)) * 60) : 0);
                const minSAB = ('Min_SAB' in r && String(r.Min_SAB).trim() !== '')
                    ? toNum(r.Min_SAB, 2)
                    : ((String(r.Sab_Habilitado || '').trim() !== '' ? toNum(r.Sab_Habilitado, 0) === 1 : tieneSabDesdeHasta)
                        ? ((toNum(r.Sab_Hasta, 0) - toNum(r.Sab_Desde, 0)) * 60) : 0);
                const minutosTurno = toNum(r.Minutos_Turno, 2);
                const turnosCalc = ('Turnos_Calc' in r && String(r.Turnos_Calc).trim() !== '') ? toNum(r.Turnos_Calc, 2) : calcularTurnosDesdeVentana(minLV, minutosTurno);
                const turnosSabado = ('Turnos_Sabado' in r && String(r.Turnos_Sabado).trim() !== '') ? toNum(r.Turnos_Sabado, 2) : calcularTurnosDesdeVentana(minSAB, minutosTurno);
                const totalDia = ('TotalDia' in r && String(r.TotalDia).trim() !== '') ? toNum(r.TotalDia, 2) : turnosCalc;
                return {
                    Planta: String(r.Planta || '').trim(),
                    Tipo_Carga: normalizarTipoCarga(r.Tipo_Carga),
                    Habilitado: toNum(r.Habilitado, 0),
                    Turnos_Cargados: toNum(r.Turnos_Cargados, 2),
                    Turnos_Calc: turnosCalc,
                    Turnos_Sabado: turnosSabado,
                    Min_LV: minLV,
                    Min_SAB: minSAB,
                    Minutos_Turno: minutosTurno,
                    TotalDia: totalDia,
                    LV_Desde: tieneLVDesdeHasta ? toNum(r.LV_Desde, 0) : '',
                    LV_Hasta: tieneLVDesdeHasta ? toNum(r.LV_Hasta, 0) : '',
                    Sab_Habilitado: String(r.Sab_Habilitado || '').trim() !== '' ? toNum(r.Sab_Habilitado, 0) : '',
                    Sab_Desde: tieneSabDesdeHasta ? toNum(r.Sab_Desde, 0) : '',
                    Sab_Hasta: tieneSabDesdeHasta ? toNum(r.Sab_Hasta, 0) : '',
                    Estatus: String(r.Estatus || 'OK').trim()
                };
            }).filter(r => r.Planta && r.Tipo_Carga);

            capacidadPorPlantaTipoCache = rows;
            construirIndiceCapacidad();
            convertirConfigDesdeCSV();
            cargarSelectPlantasFisicas();
            renderizarABMHorarios();
            renderizarResumenCapacidad();
            logLog(`[ETL CSV] capacidad_turnos_por_planta.csv: ${rows.length} registros cargados.`);
            return rows;
        } catch (e) {
            logLog(`[WARN] No se pudo cargar capacidad_turnos_por_planta.csv: ${e.message}`);
            return [];
        }
    }
    ...
    function calcularCapacidadPlantaPorTipo(plantaConfig, esSabado) {
        ...
        const slots = resumen
            ? (esSabado ? toNum(resumen.Turnos_Sabado, 2) : toNum(resumen.TotalDia || resumen.Turnos_Calc, 2))
            : calcularTurnosTeoricos(plantaConfig, esSabado);
        ...
        TIPOS_ZONA_CALCULO.forEach(tipo => {
            ...
            let cap = 0;
            if (reg && reg.Habilitado === 1) {
                cap = esSabado
                    ? toNum(reg.Turnos_Sabado, 2)
                    : toNum(reg.Turnos_Cargados || reg.TotalDia || reg.Turnos_Calc, 2);
            } else if (!reg) {
                cap = slots;
            }
            res[tipo] = cap;
    ...
    function renderizarResumenCapacidad() {
        ...
        configPlantas.forEach(p => {
            ...
            TIPOS_ZONA_CALCULO.forEach(tipo => {
                ...
                if (teo === 0) return;
                const ocup = listaTurnosCache.filter(t =>
                    t.field_6 && String(t.field_6).startsWith(fechaSeleccionadaGlobal) &&
                    t.PlantaDescarga === p.id &&
                    normalizarTipoZona(t.Planta) === normalizarTipoCarga(tipo) &&
                    !['cancelado', 'rechazado'].includes(String(t.field_7 || '').toLowerCase())
                ).length;
                const disp = Math.max(0, teo - ocup);
                html += `<div class="flex justify-between gap-1 py-0.5 border-b border-slate-800/50">
                    <span class="text-sky-300 truncate">${escapeHtml(p.id)} / ${escapeHtml(tipo)}</span>
                    <span class="text-slate-400 shrink-0">${teo}|${ocup}|<span class="text-emerald-400 font-bold">${disp}</span></span>
                </div>`;
            });
        });
    ...
    // ---- ABM FLOTA -- Origen SharePoint CSV: Flota.csv ----
    const LISTA_FLOTA   = 'Vehculos'; // compatibilidad visual, sin uso de lectura REST
    const LISTA_ZONAS   = 'Zonas descarga';
    ...
    function parsearCSVFlota(text) {
        ...
        renderizarBaseFlota();
        actualizarDatalistFlota();
        logLog(`[ETL CSV] Flota: ${listaFlotaCache.length} vehiculos cargados desde Flota.csv.`);
    }
    ...
    async function cargarFlotaSP() {
        try {
            ...
            logLog(`[WARN] Flota CSV no disponible: ${e.message}. Usando cache local.`);
            renderizarBaseFlota();
        }
    }
    ...
    function cargarFlotaForm(idx) {
        ...
        document.getElementById('fl_tipo').value    = v.tipo || 'Camion';
        ...
        document.getElementById('fl_estado').value  = v.estado || 'Habilitado';
    }
    ...
    function limpiarFormularioFlota() {
        ...
        document.getElementById('fl_tipo').value   = 'Camion';
        document.getElementById('fl_estado').value = 'Habilitado';
    }
    ...
    async function guardarVehiculo() {
        ...
        raw[15] = document.getElementById('fl_tipo').value;
        const obj = mapearFilaFlota(raw);
        obj.cuit = document.getElementById('fl_cuit').value.replace(/[^0-9\-]/g,'');
        obj.estado = document.getElementById('fl_estado').value;
        obj.chofer = chofer;
        ...
        logLog(`[ETL CSV] Flota: vehiculo ${pat} actualizado en Flota.csv.`);
    }
    ...
    function limpiarFormularioZona() {
        ...
        const selP = document.getElementById('z_planta');
        if (selP) selP.selectedIndex = 0;
        document.getElementById('z_tipo').selectedIndex = 0;
        document.getElementById('z_estado').selectedIndex = 0;
    }

    async function guardarZona() {
        const nombre = document.getElementById('z_nombre').value.trim();
        if(!nombre) return showToast('Nombre de zona requerido.', 'err');
        const planta = document.getElementById('z_planta').value;
        const tipo = document.getElementById('z_tipo').value;
        const estado = document.getElementById('z_estado').value;
        if(!planta) return showToast('Seleccione Planta Fisica.', 'err');
        if(!tipo) return showToast('Seleccione Tipo de Insumo.', 'err');
        if(!estado) return showToast('Seleccione Estado.', 'err');
        const spId = document.getElementById('z_item_id').value;
        const payload = {
            NombreZona:   nombre,
            PlantaFisica: document.getElementById('z_planta').value,
            TipoInsumo:   document.getElementById('z_tipo').value,
            Capacidad:    parseInt(document.getElementById('z_capacidad').value)||0,
            Estado:       document.getElementById('z_estado').value,
            '__metadata': { 'type': 'SP.Data.Zonas_x0020_descargaListItem' }
        };
        try {
            const digest = await getDigest();
            ...
            await cargarZonasSP();
            logLog(`[ETL] Zonas: "${nombre}" guardada en SP.`);
        } catch(e) {
    ...
    async function initApp() {
        try {
            ...
            Promise.all([
                ...
                importarCSVSharePoint('flota', true)
            ]).then(async () => {
                logLog("[ETL] Maestros sincronizados. Cargando Flota.csv y Zonas...");
                await Promise.all([cargarFlotaSP(), cargarZonasSP()]);
                poblarSelectsABMInicial();
                logLog("[ETL] Disparando carga de Turnos...");
                refrescarDBAbsoluto();
            });
    ...
    async function importarCSVSharePoint(tipo, isSilent = false) {
        ...
        try {
            ...
            else if (tipo === 'usu') parsearCSVUsuarios(text);
            else parsearCSVFlota(text);
    ...
    function renderizarABMHorarios() {
        ...
        if (capacidadPorPlantaTipoCache && capacidadPorPlantaTipoCache.length) {
            const filas = capacidadPorPlantaTipoCache
                .slice()
                .sort((a, b) => {
                    const pa = String(a.Planta || '').localeCompare(String(b.Planta || ''));
                    if (pa !== 0) return pa;
                    return String(a.Tipo_Carga || '').localeCompare(String(b.Tipo_Carga || ''));
                })
                .map(r => {
                    const habilitado = Number(r.Habilitado || 0);
                    const colorHab = habilitado === 1 ? 'text-emerald-400' : 'text-rose-400';
                    const estatus = String(r.Estatus || 'OK');
                    const colorEst = estatus === 'OK' ? 'text-emerald-400' : 'text-amber-400';
                    return `<tr class="text-sm hover:bg-slate-800/30">
                        <td class="py-3 px-4 font-bold text-sky-400">${escapeHtml(r.Planta)}</td>
                        <td class="px-4 text-white font-semibold">${escapeHtml(r.Tipo_Carga)}</td>
                        <td class="px-4 text-center font-bold ${colorHab}">${habilitado}</td>
                        <td class="px-4 text-center text-slate-200">${toNum(r.Turnos_Cargados, 2)}</td>
                        <td class="px-4 text-center text-emerald-400 font-bold">${toNum(r.Turnos_Calc, 2)}</td>
                        <td class="px-4 text-center text-amber-400 font-bold">${toNum(r.Turnos_Sabado, 2)}</td>
                        <td class="px-4 text-center text-slate-300">${toNum(r.Min_LV, 2)}</td>
                        <td class="px-4 text-center text-slate-300">${toNum(r.Min_SAB, 2)}</td>
                        <td class="px-4 text-center text-slate-300">${toNum(r.Minutos_Turno, 2)}</td>
                        <td class="px-4 text-center text-sky-300 font-bold">${toNum(r.TotalDia, 2)}</td>
                        <td class="px-4 text-center font-bold ${colorEst}">${escapeHtml(estatus)}</td>
                    </tr>`;
                }).join('');

            tb.innerHTML = filas || '<tr><td colspan="11" class="p-6 text-center text-slate-500 text-sm">Sin datos en capacidad_turnos_por_planta.csv.</td></tr>';
            return;
        }

        let h = '';
        configPlantas.forEach((p) => {
            const turnosLV = calcularTurnosTeoricos(p, false);
            const turnosSab = calcularTurnosTeoricos(p, true);
            h += `<tr class="text-sm hover:bg-slate-800/30">
                <td class="py-3 px-4 font-bold text-sky-400">${p.id}</td>
                <td class="px-4 text-white font-semibold">-</td>
                <td class="px-4 text-center text-slate-300">-</td>
                <td class="px-4 text-center text-slate-300">-</td>
                <td class="px-4 text-center text-emerald-400 font-bold">${Number(turnosLV.toFixed(2))}</td>
                <td class="px-4 text-center text-amber-400 font-bold">${Number(turnosSab.toFixed(2))}</td>
                <td class="px-4 text-center text-slate-300">${(p.hOutLV - p.hInLV) * 60}</td>
                <td class="px-4 text-center text-slate-300">${p.sabado ? (p.hOutS - p.hInS) * 60 : 0}</td>
                <td class="px-4 text-center text-slate-300">${p.duracion}</td>
                <td class="px-4 text-center text-sky-300 font-bold">${Number(turnosLV.toFixed(2))}</td>
                <td class="px-4 text-center text-emerald-400">OK</td>
            </tr>`;
        });
        tb.innerHTML = h || '<tr><td colspan="11" class="p-6 text-center text-slate-500 text-sm">Sin configuración disponible.</td></tr>';
    }
    ...
    function previewHorariosConfig() {
        configPlantas.forEach((p, idx) => {
            ...
            const tmp = {
                id: p.id,
                hInLV: parseInt(document.getElementById(`cfg_inlv_${idx}`)?.value, 10) || p.hInLV,
                hOutLV: parseInt(document.getElementById(`cfg_outlv_${idx}`)?.value, 10) || p.hOutLV,
                hInS: parseInt(document.getElementById(`cfg_ins_${idx}`)?.value, 10) || p.hInS,
                hOutS: parseInt(document.getElementById(`cfg_outs_${idx}`)?.value, 10) || p.hOutS,
                duracion: dur < 10 ? 10 : dur,
                sabado: document.getElementById(`cfg_sab_${idx}`)?.checked || false
            };
            const capLV = calcularCapacidadPlantaPorTipo(tmp, false);
            const capSab = calcularCapacidadPlantaPorTipo(tmp, true);
            const elLv = document.getElementById(`cfg_prev_lv_${idx}`);
            const elSab = document.getElementById(`cfg_prev_sab_${idx}`);
            const elTip = document.getElementById(`cfg_prev_tip_${idx}`);
            const elTot = document.getElementById(`cfg_prev_tot_${idx}`);
            if (elLv) elLv.textContent = capLV.slots;
            if (elSab) elSab.textContent = capSab.slots;
            if (elTip) elTip.textContent = `${capLV.porTipo['Insumos']||0}/${capLV.porTipo['Packaging']||0}/${capLV.porTipo['No Productivo']||0}`;
            if (elTot) elTot.textContent = capLV.total;
        });
    }
    ...
    function getSlotsCalculados(plantaId, fechaStr) {
        let slots = [];
        if(!fechaStr || !plantaId) return slots;
        let pConf = configPlantas.find(p => p.id === plantaId);
        if (!pConf) return slots;
    ...
    function regenerarHorariosABM(modoFiltro = false) {
        const pf = document.getElementById('f_planta_descarga').value;
        const zona = document.getElementById('f_planta').value;
        const f = document.getElementById('f_fecha').value;
        const sel = document.getElementById('f_horario');
        if(!sel) return;
        sel.innerHTML = '';
        ...
        let oc = modoFiltro ?
        listaTurnosCache.filter(t => t.field_6 && t.field_6.startsWith(f) && t.PlantaDescarga === pf && t.Planta === zona && String(t.field_7).toLowerCase() !== 'cancelado' && String(t.field_7).toLowerCase() !== 'rechazado').map(t => normalizarStringHorario(t.field_5)) : [];
        let htmlOpts = '';
    ...
    function validarFormularioTurno(modo) {
        ...
        const fPlanta = document.getElementById('f_planta_descarga').value;
        const fZona = document.getElementById('f_planta').value;
    ...
    async function checkSlotLibreConcurrencia(planta, zona, fecha, slot) {
        logETLStep(2, false);
        try {
            const res = await fetchSharePoint(`items?$filter=PlantaDescarga eq '${planta}' and Planta eq '${zona}' and startswith(field_6, '${fecha}') and field_5 eq '${slot}' and field_7 ne 'Cancelado' and field_7 ne 'Rechazado'&$select=Id`);
    ...
    function abrirAltaTurno() {
        ...
        document.getElementById('f_horario').innerHTML = '<option value="">-</option>';
    ...
    function modoABM_Ver(id) {
        cargarTurnoEnFormulario(id, 'VER');
    ...
    function modoABM_Aprobar(id) {
        cargarTurnoEnFormulario(id, 'APROBAR_PANEL');
    ...
    function modoABM_Reprogramar() {
        ...
        document.getElementById('f_fecha').disabled = false; aplicarVentanaFechaAlta();
    ...
    function volverTurnosDiarios() {
        modoABMActual = null;
        cerrarModalNuevoProveedor(); // Fix: cerrar modal si quedo abierto
        document.getElementById('btn-tab-monitor').click();
    }
    
    function habilitarCamposABM(estado) { document.querySelectorAll('.f-control').forEach(e=>e.disabled=!estado); 
    ...
    function renderBotonesABM(modo) {
        ...
        if (modo === 'ALTA') {
            ...
            panel.innerHTML = `<button onclick="crearTurno()" class="bg-sky-600 hover:bg-sky-500 px-6 py-3 font-bold rounded text-white flex-1 transition shadow">Registrar Alta</button>
            ...
            if(esAprobado) {
                panel.innerHTML = `<button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded text-white flex-1 transition shadow">Cancelar</button>`;
            } else {
                panel.innerHTML = `
                    ...
                    <button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-5 py-3 font-bold rounded text-white transition shadow">Cancelar</button>`;
            }
            ...
            panel.innerHTML = `<button onclick="ejecutarAccionFlujo('Aprobado')" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-3 font-bold rounded text-white flex-1 transition shadow">Aprobar</button>
                ...
                <button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-4 py-3 font-bold rounded text-white transition shadow">Volver al Monitor</button>`;
        } else if (modo === 'REPROGRAMAR') {
            setModoIndicador('REPROGRAMACION','bg-indigo-900 text-indigo-300 border-indigo-700', 'REPROGRAMAR');
            panel.innerHTML = `<button onclick="ejecutarReprogramacion()" class="bg-indigo-600 hover:bg-indigo-500 px-6 py-3 font-bold rounded text-white flex-1 transition shadow">Confirmar</button>
                <button onclick="modoABM_Ver(document.getElementById('f_item_id').value)" class="bg-slate-700 hover:bg-slate-600 px-4 py-3 font-bold rounded text-white transition shadow">Cancelar</button>`;
        }
    }
    ...
    function modoABM_Modificar() {
        const id = document.getElementById('f_item_id').value;
        cargarTurnoEnFormulario(id, 'MODIFICAR');
    ...
    async function eliminarTurno() {
        ...
        try {
            ...
            volverTurnosDiarios();
            aplicarFiltrosMonitor(); generarResumenDiario(); calcularDisponibilidadOffline(); renderizarResumenCapacidad();
        } catch(e) {
    ...
    function cargarTurnoEnFormulario(id, modo) {
        ...
        if (!t) return;
        const modoActual = modo || modoABMActual || 'VER';
        limpiarErroresVisuales();
        ...
        if (mostrarValoresGuardados) {
            asegurarOptionSelect(selPD, t.PlantaDescarga);
            poblarSelectZonasPorPlanta(t.PlantaDescarga, t.Planta);
        } else if (modoActual === 'MODIFICAR' || modoActual === 'REPROGRAMAR') {
            if (t.PlantaDescarga) {
                asegurarOptionSelect(selPD, t.PlantaDescarga);
                poblarSelectZonasPorPlanta(t.PlantaDescarga, t.Planta);
            } else {
                selPD.selectedIndex = 0;
                poblarSelectZonasPorPlanta('', '');
            }
        } else {
            selPD.selectedIndex = 0;
        ...
        if(t.field_5 && !Array.from(slotSelect.options).some(opt => opt.value === t.field_5))
    ...
    function limpiarFormularioABM() {
        ...
        document.querySelectorAll('.f-control').forEach(i => {
            if (i.tagName === 'SELECT') {
                if (i.id === 'f_horario') i.innerHTML = '<option value="">-</option>';
                else if (i.selectedIndex >= 0) i.selectedIndex = 0;
            } else if (i.type === 'checkbox') i.checked = false;
            else i.value = '';
            i.disabled = true;
        });
        document.getElementById('f_fecha').min = '';
        document.getElementById('f_fecha').max = '';
        document.getElementById('f_dia').textContent = '-';
    ...
    function renderizarBaseProveedores() {
        ...
        tb.innerHTML = listaProveedoresCache.map((p, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm cursor-pointer" onclick="cargarProvForm(${idx})">
                <td class="p-3 text-slate-300">${escapeHtml(p.usuario)}</td>
                <td class="p-3 text-slate-400">${escapeHtml(p.responsable)}</td>
                <td class="p-3 text-amber-400 font-bold">${escapeHtml(p.id)}</td>
                <td class="p-3 text-amber-400 font-bold">${escapeHtml(p.cuit || '-')}</td>
    ...
    function abrirModalNuevoProveedor(cuitPrecargado) {
        if (_modalProvAbierto) return;
        _modalProvAbierto = true;
        limpiarErroresVisuales();
    ...
    function autocompletarProveedor() {
        ...
        if (listaProveedoresCache.length === 0) {
            // Padron no cargado aun -- no bloquear, solo avisar
        ...
        if (!provEncontrado) {
            if (_modalProvAbierto) return;
            showToast('CUIT no encontrado. Complete el alta de proveedor.', 'ok');
    ...
    function renderizarBaseUsuarios() {
        ...
        tb.innerHTML = listaUsuariosCache.map((u, idx) => `
            <tr class="border-b border-slate-700/50 hover:bg-slate-800/30 text-sm cursor-pointer" onclick="cargarUsuForm(${idx})">
                <td class="p-3 text-amber-400">${escapeHtml(u.id)}</td>
    ...
    function limpiarFormularioUsuario() {
        ...
        if(document.getElementById('u_estado')) document.getElementById('u_estado').selectedIndex = 0;
    ...
    async function guardarEdicionUsuario() {
        ...
        const uPlanta = document.getElementById('u_planta').value;
        const uEstado = document.getElementById('u_estado').value;
        if(!uId) return showToast("Falta ID", "err");
        if(!uPlanta) return showToast("Seleccione Planta Asociada", "err");
        if(!uEstado) return showToast("Seleccione Estado", "err");
