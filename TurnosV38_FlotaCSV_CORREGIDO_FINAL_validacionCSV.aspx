...
<main class="flex-1 p-6 overflow-y-auto relative w-full">
    ...
    <section id="s8" class="tab-content mt-8 w-full">
        ...
        <p class="text-sm text-slate-400 mb-4">Vista productiva alineada a <span class="text-sky-400">capacidad_turnos_por_planta.csv</span>. La fila principal resume la planta y, con el símbolo <strong>+</strong>, despliega el detalle exacto por <strong>Tipo_Carga</strong> tal como está en el CSV.</p>
...
async function cargarCapacidadTurnosCSV() {
    try {
        ...
        if (!res.ok) {
            const altServerRelative = CAPACIDAD_CSV_SERVER_RELATIVE_URL.replace('/Documentos compartidos/', '/Shared Documents/');
            const urlAlternativa = `${relativeSiteUrl}/_api/web/GetFileByServerRelativeUrl('${altServerRelative}')/$value`;
            res = await fetch(urlAlternativa, { headers: { "Accept": "text/plain" } });
            rutaUsada = altServerRelative;
        }
        ...
        if (trimmed.startsWith('<!DOCTYPE html') || trimmed.startsWith('<html')) {
            throw new Error('La URL devolvió HTML en lugar del contenido del CSV');
        }

        const rows = parseCsvSemicolon(text);
        if (!rows.length) {
            const detalle = 'El archivo fue leído pero no contiene filas útiles o está vacío.';
            logLog('[WARN] capacidad_turnos_por_planta.csv vacío o sin filas.');
            actualizarEstadoCSVVisual('warn', 'CSV vacío', detalle);
        ...
        if (faltantes.length) {
            throw new Error(`Faltan columnas obligatorias en CSV: ${faltantes.join(', ')}`);
        }
        ...
        const parsed = rows.map(r => {
            ...
            const turnosCalc = ('Turnos_Calc' in r && String(r.Turnos_Calc).trim() !== '')
                ? toNum(r.Turnos_Calc, 2)
                : calcularTurnosDesdeVentana(minLV, minutosTurno);
            const turnosSabado = ('Turnos_Sabado' in r && String(r.Turnos_Sabado).trim() !== '')
                ? toNum(r.Turnos_Sabado, 2)
                : calcularTurnosDesdeVentana(minSAB, minutosTurno);
            const totalDia = ('TotalDia' in r && String(r.TotalDia).trim() !== '')
                ? toNum(r.TotalDia, 2)
                : turnosCalc;
        ...
        actualizarEstadoCSVVisual(
            'ok',
            'CSV válido',
            `Ruta: ${rutaUsada} | Registros: ${parsed.length} | Columnas: ${headers.join(', ')}`
        );
        ...
        logLog(`[ERROR CSV CAPACIDAD]: ${detalle}`);
...
function toggleDetalleCapacidad(plantaKey) {
    let rows = Array.from(document.querySelectorAll('tr[data-parent-planta]'))
        .filter(r => r.getAttribute('data-parent-planta') === plantaKey);
    const icon = Array.from(document.querySelectorAll('[data-expander-planta]'))
        .find(el => el.getAttribute('data-expander-planta') === plantaKey);
    ...
    function calcularCapacidadPlantaPorTipo(plantaConfig, esSabado) {
        ...
        TIPOS_ZONA_CALCULO.forEach(tipo => {
            ...
            if (reg && reg.Habilitado === 1) {
                cap = esSabado ? toNum(reg.Turnos_Sabado, 2) : toNum(reg.Turnos_Cargados || reg.TotalDia || reg.Turnos_Calc, 2);
            } else if (!reg) {
                cap = slots;
            }
            res[tipo] = cap;
        ...
        if (capacidadPorPlantaTipoCache && capacidadPorPlantaTipoCache.length) {
            ...
            const htmlRows = grupos.map(g => {
                ...
                const children = (g.filas || []).sort((a, b) => String(a.Tipo_Carga || '').localeCompare(String(b.Tipo_Carga || ''))).map(row => {
                    ...
                    return `<tr class="text-sm cap-child-row hidden-row border-b border-slate-800/60" data-parent-planta="${plantaKeyAttr}"><td class="py-2.5 px-4 text-slate-500"><span class="cap-child-indent">└</span></td><td class="px-4 text-white font-semibold">${escapeHtml(row.Tipo_Carga || '-')}</td><td class="px-4 text-center font-bold ${colorHab} mono-num">${fmtNum(row.Habilitado, 0)}</td><td class="px-4 text-center text-slate-200 mono-num">${fmtNum(row.Turnos_Cargados, 2)}</td><td class="px-4 text-center text-emerald-400 font-bold mono-num">${fmtNum(row.Turnos_Calc, 2)}</td><td class="px-4 text-center text-amber-400 font-bold mono-num">${fmtNum(row.Turnos_Sabado, 2)}</td><td class="px-4 text-center text-slate-300 mono-num">${fmtNum(row.Min_LV, 0)}</td><td class="px-4 text-center text-slate-300 mono-num">${fmtNum(row.Min_SAB, 0)}</td><td class="px-4 text-center text-slate-300 mono-num">${fmtNum(row.Minutos_Turno, 0)}</td><td class="px-4 text-center text-sky-300 font-bold mono-num">${fmtNum(row.TotalDia, 2)}</td><td class="px-4 text-center font-bold ${colorEst}">${escapeHtml(row.Estatus || 'OK')}</td></tr>`;
                }).join('');
