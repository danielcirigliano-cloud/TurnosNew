...
<head>
    ...
    <style>
        ...
        #toast { position: fixed; bottom: 1.5rem; right: 1.5rem; padding: .6rem 1.2rem; border-radius: 8px; font-size: .9rem; font-weight: 600; opacity: 0; transition: opacity .3s; z-index: 9999; pointer-events: none; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
...
<div id="splash-screen">
    <div id="splash-content">
        <div class="term-header">MOLCA.ERP [Version 38.4.2]<br>Sistema de Gestion Logistica</div>
            ...
            <div class="col-span-9 glass p-4 rounded-xl flex flex-col h-full">
                <div class="flex gap-2 mb-3 items-end bg-slate-900/50 p-3 rounded-lg border border-slate-700 shrink-0 flex-wrap">
                    ...
                    <div>
                        ...
                        <input type="text" id="filtro_proveedor_busq" list="dl_proveedores" oninput="aplicarFiltrosMonitor()" placeholder="Buscar proveedor..."
    ...
    <section id="s1" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-8 rounded-xl w-full grid grid-cols-2 gap-6">
            ...
            <div class="col-span-2">
                ...
                <input type="text" id="f_proveedor" placeholder="" class="bg-slate-900 border border-slate-700 p-3 w-full text-white outline-none f-control transition-colors focus:border-sky-500" readonly>
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2 mt-4 border-t border-slate-700 pt-4 flex justify-between items-center" id="panel-tracking">
                <div class="text-left flex items-end gap-6">
                    <div>
                    ...
                    <div>
                        <span class="block text-[10px] uppercase text-slate-400 font-bold mb-1">Registro de Porteria</span>
                        <div class="flex gap-2">
                            <button id="btn-track-in" onclick="marcarTrackingForm('IN')" class="bg-slate-800 text-emerald-400 border border-slate-700 hover:bg-slate-700 px-3 py-1.5 rounded text-xs font-bold transition">In</button>
                            <button id="btn-track-out" onclick="marcarTrackingForm('OUT')" class="bg-slate-800 text-rose-400 border border-slate-700 hover:bg-slate-700 px-3 py-1.5 rounded text-xs font-bold transition">Out</button>
                        </div>
                    </div>
                </div>
        ...
        <div class="glass p-6 rounded-xl mb-6 border border-slate-700 w-full text-sm">
            ...
            <div class="grid grid-cols-12 gap-4 items-end">
                <div class="col-span-2">
                    ...
                    <input type="text" id="fl_patente" maxlength="7" class="w-full bg-slate-900 border border-slate-700 p-2.5 text-amber-300 font-bold uppercase rounded outline-none focus:border-sky-500" placeholder="Ej: AA123BB">
                </div>
                <div class="col-span-2">
    ...
    <section id="s9" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-6 rounded-xl mb-6 border border-slate-700">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
                    ...
                    <button onclick="sincronizarFlotaConFeedback()" class="bg-slate-700 hover:bg-slate-600 text-emerald-300 px-4 py-2 text-xs font-bold rounded border border-slate-600 transition">Leer CSV</button>
                </div>
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
                    <div>
                    ...
                    <button onclick="sincronizarZonasConFeedback()" class="bg-slate-700 hover:bg-slate-600 text-emerald-300 px-4 py-2 text-xs font-bold rounded border border-slate-600 transition">Leer CSV</button>
                </div>
        ...
        <div class="w-full grid grid-cols-2 gap-8">
            ...
            <div class="glass p-8 rounded-xl flex flex-col h-[600px]">
                ...
                <div class="flex-1 bg-black/80 rounded border border-slate-700 p-4 text-xs overflow-y-auto" id="etl-log-console" style="color:#4ade80"><div>> Inicializando motor ETL Audit v10.0...</div></div>
            </div>
        </div>
    </section>
    ...
    <section id="s10" class="tab-content mt-8 w-full">
        ...
        <div class="glass p-5 rounded-xl mb-6 border border-slate-700">
            <div class="flex justify-between items-center mb-3 border-b border-slate-700 pb-2">
                ...
                <span id="dash-resumen-fecha-label" class="text-xs text-slate-400">Calculando...</span>
            </div>
            <div id="dash-resumen-contadores" class="grid grid-cols-5 gap-3"></div>
        </div>
        <div class="glass p-4 rounded-xl mb-6 flex gap-4 items-end border border-slate-700 text-sm">
            <div><label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Fecha</label><input type="date" id="dash_fecha" onchange="updateDashboard()" class="bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none focus:border-sky-500"></div>
            <div><label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Planta</label><select id="dash_planta" onchange="updateDashboard()" class="bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none focus:border-sky-500"><option value="TODAS">-- Todas --</option></select></div>
            <div><label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Horario</label><select id="dash_horario" onchange="updateDashboard()" class="bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none focus:border-sky-500"><option value="TODOS">-- Todos --</option></select></div>
            <div><label class="block uppercase text-slate-400 font-bold mb-1 text-[10px]">Responsable</label><select id="dash_resp" onchange="updateDashboard()" class="bg-slate-900 border border-slate-700 p-2 text-white rounded outline-none focus:border-sky-500"><option value="TODOS">-- Todos --</option></select></div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="glass p-4 rounded-xl"><h3 class="text-sm font-bold text-slate-300 mb-2">Estados Globales</h3><canvas id="chartEstados"></canvas></div>
            <div class="glass p-4 rounded-xl"><h3 class="text-sm font-bold text-slate-300 mb-2">Ranking de Proveedores</h3><canvas id="chartProveedores"></canvas></div>
            <div class="glass p-4 rounded-xl"><h3 class="text-sm font-bold text-slate-300 mb-2">Turnos por Planta</h3><canvas id="chartPlantas"></canvas></div>
            <div class="glass p-4 rounded-xl"><h3 class="text-sm font-bold text-slate-300 mb-2">Curva de Horarios</h3><canvas id="chartHorarios"></canvas></div>
        </div>
            ...
            <div class="overflow-x-auto w-full">
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-900/50 text-xs uppercase text-slate-400 font-bold">
                        <tr><th class="py-2 px-3">Planta Fisica</th><th class="px-3 text-center">T. Asignados</th><th class="px-3 text-center text-amber-400">Pendientes</th><th class="px-3 text-center text-emerald-400">Aprobados</th><th class="px-3 text-center text-sky-400">Reprogramados</th><th class="px-3 text-center text-rose-400">Cancelados</th></tr>
                    </thead>
                    <tbody id="tabla-resumen-dashboard" class="divide-y divide-slate-700/50"></tbody>
                </table>
            </div>
        </div>
    </section>
...
<script>
    ...
    async function ejecutarTransaccion(itemId, payload, msg, skipRefresh = false) {
        ...
        try {
            ...
            ordenarEstructuralmenteBase(listaTurnosCache); aplicarFiltrosMonitor(); 
        }
    }
    ...
    async function importarCSVSharePoint(tipo, isSilent = false) {
        ...
        try {
            ...
            else if(tipo === 'flota') parsearCSVFlota(text);
            else if(tipo === 'zonas') parsearCSVZonas(text);
            else parsearCSVUsuarios(text);
            
            return true;
        } catch (err) { 
            logLog(`[ERROR CSV SP - ${fileName}]: ${err.message}`); 
            if(tipo === 'prov') renderizarBaseProveedores();
            else if(tipo === 'flota') renderizarBaseFlota();
            else if(tipo === 'zonas') renderizarBaseZonas();
            else renderizarBaseUsuarios();
            throw err; 
        }
    }
    ...
    async function sincronizarCSVConFeedback(tipo) {
        ...
        else if (tipo === 'flota') { statusId = 'sync-status-flota'; label = 'Flota'; cache = listaFlotaCache; }
    ...
    async function sincronizarFlotaConFeedback() {
        ...
        try {
            await importarCSVSharePoint('flota', true); const ahora = new Date().toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });
    ...
    async function sincronizarZonasConFeedback() {
        ...
        try {
            await importarCSVSharePoint('zonas', true); const ahora = new Date().toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });
    ...
    async function actualizarCSVSharePoint(tipo) {
        ...
        try {
            ...
            let csvContent = "\uFEFF"; 
            if (tipo === 'usu') {
                csvContent += "ID;Nombre y apellido;Email;Planta;Estado;Reporte\n";
                listaUsuariosCache.forEach(u => { csvContent += `${u.id||''};${u.nombre||''};${u.email||''};${u.planta||''};${u.estado||''};${u.reporte||''}\n`; });
            } else if (tipo === 'flota') {
    ...
    async function initApp() {
        try {
            ...
            await testConexion();
            terminalLog(`Usuario conectado: ${currentLoggedUser.name} <span class="text-emerald-400 font-bold">[OK]</span>`);
            ...
            // Si el archivo no existe, el catch mostrará el log y no abortará
            await Promise.all([
                importarCSVSharePoint('prov', true).then(() => terminalLog("Cargando Proveedores.csv <span class="text-emerald-400 font-bold">[OK]</span>")).catch(() => terminalLog("<span class="text-amber-400">Proveedores.csv no encontrado. Iniciando vacio.</span>")),
                importarCSVSharePoint('usu', true).then(() => terminalLog("Cargando Usuarios.csv <span class="text-emerald-400 font-bold">[OK]</span>")).catch(() => terminalLog("<span class="text-amber-400">Usuarios.csv no encontrado. Iniciando vacio.</span>")),
                importarCSVSharePoint('flota', true).then(() => terminalLog("Cargando Flota.csv <span class="text-emerald-400 font-bold">[OK]</span>")).catch(() => terminalLog("<span class="text-amber-400">Flota.csv no encontrado. Iniciando vacio.</span>")),
                importarCSVSharePoint('zonas', true).then(() => terminalLog("Cargando Zonas.csv <span class="text-emerald-400 font-bold">[OK]</span>")).catch(() => terminalLog("<span class="text-amber-400">Zonas.csv no encontrado. Iniciando vacio.</span>"))
            ]);
    ...
    async function cargarCapacidadTurnosCSV() {
        try {
            ...
            const detalle = e && e.message ? e.message : 'Error no identificado al leer el CSV.'; actualizarEstadoCSVVisual('err', 'CSV inválido o inaccesible', detalle); capacidadPorPlantaTipoCache = []; capacidadPorPlantaIndex = {}; renderizarABMHorarios(); return [];
        }
    }
    ...
    async function syncFlotaABM() { 
        await importarCSVSharePoint('flota', true).catch(() => {});
        actualizarDatalistFlota(); 
        showToast(`Flota sincronizada.`, 'ok'); 
    }
    ...
    function renderizarFilas(items) {
        ...
        items.forEach(t => {
            ...
            <tr class="${filaClase}">
                ...
                <td class="p-3 text-slate-300 font-medium">${fechaStr}</td>
                ...
                <td class="p-3 text-amber-300 font-semibold">${escapeHtml(t.field_5 || '-')}</td>
    ...
    function handleHorariosChange() { const filtrar = (modoABMActual === 'ALTA' || modoABMActual === 'REPROGRAMAR' || modoABMActual === 'MODIFICAR'); regenerarHorariosABM(filtrar); autoDia(); }
    ...
    function modoABM_Modificar() {
        ...
        document.getElementById('btn-modo-modificar').classList.add('hidden'); document.getElementById('form-mode-indicator').textContent = `MODIFICANDO TURNO N° ${id}`; document.getElementById('form-mode-indicator').className = "text-xs uppercase px-3 py-1.5 bg-amber-900/50 text-amber-400 border border-amber-700 rounded-md font-bold tracking-wide flex items-center";
    ...
    function setModoIndicador(texto, claseColor, modo) { modoABMActual = modo || null; const ind = document.getElementById('form-mode-indicator'); if(!ind) return; ind.textContent = texto; ind.className = `text-xs uppercase px-3 py-1.5 rounded-md font-bold border min-w-[120px] text-center ${claseColor}`; }

    function renderBotonesABM(modo, estadoStr = '') {
        const panel = document.getElementById('panel-botones-abm'); if(!panel) return;
        if (modo === 'ALTA') { setModoIndicador('ALTA DE TURNO','bg-emerald-900/50 text-emerald-400 border-emerald-800', 'ALTA'); panel.innerHTML = `<button onclick="crearTurno()" class="bg-sky-600 hover:bg-sky-500 px-8 py-3 font-bold rounded-lg text-white transition shadow-sm flex items-center justify-center gap-2 flex-1">Registrar Alta</button><button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded-lg text-white transition shadow-sm">Cancelar</button>`; } 
        else if (modo === 'VER_PENDIENTE' || modo === 'VER') { setModoIndicador('CONSULTA','bg-slate-800 text-slate-300 border-slate-600', 'VER'); panel.innerHTML = `<button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded-lg text-white flex-1 transition shadow-sm">Cancelar / Volver</button>`; } 
        else if (modo === 'MODIFICANDO') { setModoIndicador('MODIFICANDO','bg-amber-900/50 text-amber-400 border-amber-700', 'MODIFICAR'); panel.innerHTML = `<button onclick="guardarModificacionTurno()" class="bg-amber-600 hover:bg-amber-500 px-8 py-3 font-bold rounded-lg text-white transition shadow-sm flex-1">Guardar Modificacion</button><button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded-lg text-white transition shadow-sm">Cancelar</button>`; } 
        else if (modo === 'APROBAR_PANEL') { setModoIndicador('APROBACION','bg-amber-900/50 text-amber-400 border-amber-700', 'APROBAR_PANEL'); panel.innerHTML = `<button onclick="ejecutarAccionFlujo('Aprobado')" class="bg-emerald-600 hover:bg-emerald-500 px-4 py-3 font-bold rounded-lg text-white flex-1 transition shadow-sm">Aprobar</button><button onclick="ejecutarAccionFlujo('Cancelado')" class="bg-rose-600 hover:bg-rose-500 px-4 py-3 font-bold rounded-lg text-white flex-1 transition shadow-sm">Cancelar Turno</button><button onclick="modoABM_Reprogramar()" class="bg-amber-600 hover:bg-amber-500 px-4 py-3 font-bold rounded-lg text-white flex-1 transition shadow-sm">Reprogramar</button><button onclick="volverTurnosDiarios()" class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded-lg text-white transition shadow-sm">Cancelar</button>`; } 
        else if (modo === 'REPROGRAMAR') { setModoIndicador('REPROGRAMACION','bg-indigo-900/50 text-indigo-400 border-indigo-800', 'REPROGRAMAR'); panel.innerHTML = `<button onclick="ejecutarReprogramacion()" class="bg-indigo-600 hover:bg-indigo-500 px-6 py-3 font-bold rounded-lg text-white flex-1 transition shadow-sm">Confirmar Reprogramacion</button><button onclick="modoABM_Ver(document.getElementById('f_item_id').value)" class="bg-slate-700 hover:bg-slate-600 px-6 py-3 font-bold rounded-lg text-white transition shadow-sm">Cancelar</button>`; }
    }
    ...
    function cargarTurnoEnFormulario(id, modo) {
        ...
        regenerarHorariosABM(modoActual === 'ALTA' || modoActual === 'REPROGRAMAR' || modoActual === 'MODIFICAR');
    ...
    function limpiarFormularioABM() {
        ...
        document.getElementById('f_fecha').min = ''; document.getElementById('f_fecha').max = ''; document.getElementById('f_dia').textContent = '-'; document.getElementById('f_usuario_gestor').textContent = '-'; document.getElementById('f_estado_actual').textContent = 'NUEVO'; document.getElementById('panel-botones-abm').innerHTML = ''; document.getElementById('btn-track-in').disabled = true; document.getElementById('btn-track-out').disabled = true;
    }
    ...
    async function guardarModificacionTurno() {
        if(!validarFormularioTurno('MODIFICAR')) return; 
    ...
    function enviarResumenDiario() {
        ...
        if(turnosVentana.length === 0) return showToast("No hay turnos para reportar en esta fecha", "err");
